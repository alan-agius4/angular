...
load("//packages/bazel/src/api-extractor:api_extractor.bzl", "run_api_extractor")
...
def _ng_package_impl(ctx):
    ...
    for dep in deps_in_package:
        ...
        typings_entry_point = "/".join([p for p in [
            ctx.bin_dir.path,
            ctx.label.package,
            entry_point,
            "%s.d.ts" % dep.angular.flat_module_metadata.flat_module_out_file,
        ] if p])

        typings_output_filename = "/".join([p for p in [
