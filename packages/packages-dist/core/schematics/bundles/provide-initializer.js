...
function replaceProviderWithNewApi({sourceFile, node, provider, changeTracker}) {
  const {initializerCode, importInject, provideInitializerFunctionName, initializerToken} =
    provider;
  const initializerTokenSpecifier = imports.getImportSpecifier(
    sourceFile,
    angularCoreModule,
    initializerToken,
  );
...
function tryParseProviderExpression(node) {
  ...
  for (const property of node.properties) {
    if (
      ts__default['default'].isPropertyAssignment(property) &&
      ts__default['default'].isIdentifier(property.name)
    ) {
    ...
    if (
      ts__default['default'].isMethodDeclaration(property) &&
      property.name.getText() === 'useFactory'
    ) {
