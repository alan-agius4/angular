...
class NgtscCompilerHost {
  ...
  getDefaultLibFileName(options) {
    return this.fs.join(
      ...
      ts__default['default'].getDefaultLibFileName(options),
    );
  }
...
function readConfiguration(project, existingOptions, host = checker.getFileSystem()) {
  try {
    ...
    const readConfigFile = (configFile) =>
      ts__default['default'].readConfigFile(configFile, (file) =>
        host.readFile(host.resolve(file)),
      );
    ...
    const {
      ...
      fileNames: rootNames,
      projectReferences,
    } = ts__default['default'].parseJsonConfigFileContent(
      config,
      parseConfigHost,
      basePath,
...
function getExtendedConfigPathWorker(configFile, extendsValue, host, fs) {
  if (extendsValue.startsWith('.') || fs.isRooted(extendsValue)) {
    ...
    const {resolvedModule} = ts__default['default'].nodeModuleNameResolver(
      extendsValue,
      configFile,
      {
...
function createNgtscProgram(tsHost, tsconfig, optionOverrides) {
  const ngtscProgram = new program.NgtscProgram(
    tsconfig.rootNames,
    {
      ...tsconfig.options,
      ...defaultMigrationTsOptions,
  ...
  if (process.env['DEBUG_NG_SEMANTIC_DIAGNOSTICS'] === '1') {
    console.error(
      ts__default['default'].formatDiagnosticsWithColorAndContext(
        ngtscProgram.getNgSemanticDiagnostics(),
        tsHost,
      ),
    );
  }
