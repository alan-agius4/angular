...
function getMemberName(member) {
  ...
  if (
    ...
    ts__default['default'].isStringLiteralLike(member.name)
  ) {
...
class DebugElementComponentInstance {
  ...
  detect(node) {
    ...
    let typeExpr;
    if (
      ts__default['default'].isCallExpression(queryArg) &&
...
class TemplateExpressionReferenceVisitor extends checker.RecursiveAstVisitor$1 {
  ...
  _inspectPropertyAccess(ast, astPath) {
    ...
    const isWrite = !!(
      ast instanceof checker.PropertyWrite ||
  ...
  _isPartOfNarrowingTernary(read) {
    ...
    return this.insideConditionalExpressionsWithReads.some(
      (r) =>
        (r instanceof checker.PropertyRead ||
...
function traverseReceiverAndLookupSymbol(readOrWrite, componentClass, checker$1) {
  ...
  while (
    node.receiver instanceof checker.PropertyRead ||
    node.receiver instanceof checker.PropertyWrite
  ) {
    node = node.receiver;
  ...
  if (
    !(
      node.receiver instanceof checker.ImplicitReceiver ||
      node.receiver instanceof checker.ThisReceiver
    )
  ) {
    return null;
  }
  ...
  return (
    lookupPropertyAccess(checker$1, classType, path, {
...
function isTwoWayBindingNode(node) {
  return (
    (node instanceof checker.BoundAttribute && node.type === checker.BindingType.TwoWay) ||
...
function identifyHostBindingReferences(
  ...
  const angularDecorators = checker.getAngularDecorators(
    decorators,
    ['Directive', 'Component'],
    /* isAngularCore */ false,
  );
  ...
  for (const [rawName, expression] of hostMap.entries()) {
    ...
    const sourceSpan = new checker.ParseSourceSpan(
      ...
      new checker.ParseLocation({content: '', url: ''}, 0, 0, 0),
      new checker.ParseLocation({content: '', url: ''}, 0, 0, 0),
    );
...
function identifyTemplateReferences(
  ...
  const template =
    templateTypeChecker.getTemplate(node, checker.OptimizeFor.WholeProgram) ??
      ...
      result.references.push({
        ...
        from: {
          ...
          templateFile: project_paths.projectFile(
            checker.absoluteFrom(templateFilePath),
            programInfo,
          ),
...
function extractTemplateWithoutCompilerAnalysis(
  ...
  return program.extractTemplate(
    node,
    tmplDef,
    evaluator,
    null,
...
function traverseAccess(access) {
  if (
    ts__default['default'].isPropertyAccessExpression(access.parent) &&
    access.parent.name === access
  ) {
    return access.parent;
  } else if (
    ts__default['default'].isElementAccessExpression(access.parent) &&
...
function identifyPotentialTypeScriptReference(
  ...
  const isWriteReference =
    ts__default['default'].isBinaryExpression(accessParent) &&
  ...
  const visitor = (node) => {
    ...
    if (partialDirectiveInCatalyst !== null) {
      result.references.push({
        ...
        from: {
          file: project_paths.projectFile(
