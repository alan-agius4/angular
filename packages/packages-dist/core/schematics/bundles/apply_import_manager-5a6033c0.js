...
function applyImportManagerChanges(importManager, replacements, sourceFiles, info) {
    ...
    newImports.forEach((newImport) => {
      ...
      replacements.push(
        new project_paths.Replacement(
          project_paths.projectFile(checker.absoluteFrom(fileName), info),
          new project_paths.TextUpdate({position: 0, end: 0, toInsert: `${printedImport}\n`}),
        ),
      );
    });
  });
  ...
  for (const [oldBindings, newBindings] of updatedImports.entries()) {
    ...
    let formatFlags =
      ts__default['default'].ListFormat.NamedImportsOrExportsElements |
      ts__default['default'].ListFormat.Indented |
      ts__default['default'].ListFormat.Braces |
      ts__default['default'].ListFormat.PreserveLines |
      (isMultiline
        ? ts__default['default'].ListFormat.MultiLine
        : ts__default['default'].ListFormat.SingleLine);
    ...
    replacements.push(
      new project_paths.Replacement(
        project_paths.projectFile(oldBindings.getSourceFile(), info),
  ...
  for (const removedImport of deletedImports) {
    replacements.push(
      new project_paths.Replacement(
        project_paths.projectFile(removedImport.getSourceFile(), info),
        new project_paths.TextUpdate({
