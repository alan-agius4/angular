  ...
  findReferencesInProject(node) {
    ...
    for (const symbol of referencedSymbols) {
      for (const ref of symbol.references) {
        if (
          !ref.isDefinition ||
          symbol.definition.kind === ts__default['default'].ScriptElementKind.alias
        ) {
  ...
  _getLanguageService() {
    if (!this._languageService) {
      ...
      this._languageService = ts__default['default'].createLanguageService(
        ...
        ts__default['default'].createDocumentRegistry(),
        ts__default['default'].LanguageServiceMode.PartialSemantic,
      );
    }
...
function findLiteralProperty(literal, name) {
  return literal.properties.find(
    (prop) =>
      prop.name && ts__default['default'].isIdentifier(prop.name) && prop.name.text === name,
  );
}
...
function getTestingImports(sourceFile) {
  return {
    ...
    catalyst: imports.getImportSpecifier(
      sourceFile,
...
function isTestCall(typeChecker, node, testBedImport, catalystImport) {
  const isObjectLiteralCall =
    ts__default['default'].isCallExpression(node) &&
      ...
      if (importsToAdd.length > 0) {
        ...
        decorator = setPropertyOnAngularDecorator(
          ...
          ts__default['default'].factory.createArrayLiteralExpression(
            ...
            ts__default['default'].factory.createNodeArray(importsToAdd, hasTrailingComma),
            hasTrailingComma,
          ),
        );
      }
  ...
  for (const dep of templateDependencies) {
    const importLocation = findImportLocation(
      ...
      usedDependenciesInMigration.has(dep)
        ...
        : checker.PotentialImportMode.Normal,
      typeChecker,
    );
...
function potentialImportsToExpressions(potentialImports, toFile, tracker, importRemapper) {
  ...
  return processedDependencies.map((importLocation) => {
    ...
    const arrowFunction = ts__default['default'].factory.createArrowFunction(
      undefined,
      undefined,
      [],
      undefined,
    ...
    return ts__default['default'].factory.createCallExpression(forwardRefExpression, undefined, [
      arrowFunction,
    ]);
  });
}
...
function moveDeclarationsToImports(
  ...
  if (ts__default['default'].isPropertyAssignment(declarationsProp)) {
    ...
    if (ts__default['default'].isArrayLiteralExpression(declarationsProp.initializer)) {
      ...
      declarationsToCopy.push(
        ts__default['default'].factory.createSpreadElement(declarationsProp.initializer),
      );
    }
  }
  ...
  for (const prop of literal.properties) {
    ...
    if (prop === declarationsProp) {
      if (declarationsToPreserve.length > 0) {
        ...
        properties.push(
          ts__default['default'].factory.updatePropertyAssignment(
            prop,
            prop.name,
            ts__default['default'].factory.createArrayLiteralExpression(
              ts__default['default'].factory.createNodeArray(
    ...
    if (prop === importsProp && declarationsToCopy.length > 0) {
      ...
      if (ts__default['default'].isArrayLiteralExpression(prop.initializer)) {
        initializer = ts__default['default'].factory.updateArrayLiteralExpression(
          prop.initializer,
          ts__default['default'].factory.createNodeArray(
        ...
        initializer = ts__default['default'].factory.createArrayLiteralExpression(
          ts__default['default'].factory.createNodeArray(
            [
              ts__default['default'].factory.createSpreadElement(prop.initializer),
              ...declarationsToCopy,
            ],
      ...
      properties.push(
        ts__default['default'].factory.updatePropertyAssignment(prop, prop.name, initializer),
      );
      continue;
    }
  ...
  tracker.replaceNode(
    literal,
    ts__default['default'].factory.updateObjectLiteralExpression(
      literal,
      ts__default['default'].factory.createNodeArray(
        properties,
    ...
    ts__default['default'].EmitHint.Expression,
  );
}
...
function markDecoratorAsStandalone(node) {
  ...
  if (
    !standaloneProp ||
    standaloneProp.initializer.kind !== ts__default['default'].SyntaxKind.FalseKeyword
  ) {
    return node;
  }
  ...
  return ts__default['default'].factory.createDecorator(
...
function setPropertyOnAngularDecorator(node, name, initializer) {
  // Invalid decorator.
  if (
    !ts__default['default'].isCallExpression(node.expression) ||
  ...
  if (node.expression.arguments.length === 0) {
    ...
    if (existingProperty && ts__default['default'].isPropertyAssignment(existingProperty)) {
      ...
      literalProperties[literalProperties.indexOf(existingProperty)] = ts__default[
        'default'
      ].factory.updatePropertyAssignment(existingProperty, existingProperty.name, initializer);
    } else {
      literalProperties = [
        ...literal.properties,
        ts__default['default'].factory.createPropertyAssignment(name, initializer),
  ...
  return ts__default['default'].factory.createDecorator(
    ts__default['default'].factory.createCallExpression(
      ...
      [
        ts__default['default'].factory.createObjectLiteralExpression(
          ts__default['default'].factory.createNodeArray(literalProperties, hasTrailingComma),
...
function isNamedPropertyAssignment(node) {
  return (
    ts__default['default'].isPropertyAssignment(node) &&
    node.name &&
...
function findImportLocation(target, inContext, importMode, typeChecker) {
  ...
  for (const location of importLocations) {
    ...
    if (
      location.kind === checker.PotentialImportKind.NgModule &&
      !firstModuleImport &&
  ...
  if (imports.getImportSpecifier(sourceFile, '@angular/core', 'NgModule')) {
    sourceFile.forEachChild(function walk(node) {
      if (ts__default['default'].isClassDeclaration(node)) {
        const decorator = ng_decorators
          .getAngularDecorators(typeChecker, ts__default['default'].getDecorators(node) || [])
...
function findTemplateDependencies(decl, typeChecker) {
  ...
  if (usedPipes !== null) {
    ...
    for (const pipe of potentialPipes) {
      if (
        ts__default['default'].isClassDeclaration(pipe.ref.node) &&
...
function filterNonBootstrappedDeclarations(
  ...
  for (const el of bootstrapProp.initializer.elements) {
    const referencedClass = ts__default['default'].isIdentifier(el)
...
function migrateTestDeclarations(
  testObjects,
  declarationsOutsideOfTestFiles,
  tracker,
  templateTypeChecker,
  typeChecker,
) {
  ...
  for (const decorator of decorators) {
    const closestClass = nodes.closestNode(
      decorator.node,
    ...
    if (decorator.name === 'Pipe' || decorator.name === 'Directive') {
      ...
      if (importsToAdd && importsToAdd.size > 0) {
        ...
        const importsArray = ts__default['default'].factory.createNodeArray(
        ...
        tracker.replaceNode(
          ...
          setPropertyOnAngularDecorator(
            newDecorator,
            'imports',
            ts__default['default'].factory.createArrayLiteralExpression(importsArray),
          ),
        );
      } else {
...
function analyzeTestingModules(testObjects, typeChecker) {
  ...
  for (const obj of testObjects) {
    ...
    const importElements =
      ...
      ts__default['default'].isArrayLiteralExpression(importsProp.initializer)
        ? importsProp.initializer.elements.filter((el) => {
            ...
            return (
              !ts__default['default'].isCallExpression(el) &&
    ...
    for (const decl of declarations) {
      ...
      const [decorator] = ng_decorators.getAngularDecorators(
        typeChecker,
        ts__default['default'].getDecorators(decl) || [],
      );
      if (decorator) {
...
function pruneNgModules(
  ...
  sourceFiles.forEach(function walk(node) {
    if (ts__default['default'].isClassDeclaration(node) && canRemoveClass(node, typeChecker)) {
      ...
      ts__default['default'].isExportDeclaration(node) &&
      !node.exportClause &&
...
function collectChangeLocations(
  ...
  for (const node of nodes$1) {
    ...
    if (closestArray) {
      const closestAssignment = nodes.closestNode(
        closestArray,
        ts__default['default'].isPropertyAssignment,
      );
      if (closestAssignment && isInImportsArray(closestAssignment, closestArray)) {
        const closestCall = nodes.closestNode(
          closestAssignment,
          ts__default['default'].isCallExpression,
        );
        if (closestCall) {
          const closestDecorator = nodes.closestNode(
            closestCall,
            ts__default['default'].isDecorator,
          );
...
function replaceModulesInImportsArray(
  ...
  tracker.replaceNode(
    array,
    ts__default['default'].factory.updateArrayLiteralExpression(array, newElements),
  );
}
/**
...
function removeArrayReferences(locations, tracker) {
  for (const [array, toRemove] of locations.getEntries()) {
    ...
    tracker.replaceNode(
      array,
      ts__default['default'].factory.updateArrayLiteralExpression(
        array,
        ts__default['default'].factory.createNodeArray(
          newElements,
  ...
  for (const [namedImports, toRemove] of locations.getEntries()) {
    ...
    if (newElements.length === 0) {
      ...
      if (importClause && importClause.name) {
        tracker.replaceNode(
          importClause,
          ts__default['default'].factory.updateImportClause(
            importClause,
        ...
        const declaration = nodes.closestNode(
          namedImports,
          ts__default['default'].isImportDeclaration,
        );
        if (declaration) {
      ...
      tracker.replaceNode(
        namedImports,
        ts__default['default'].factory.updateNamedImports(namedImports, newElements),
      );
    }
  }
}
...
function removeExportReferences(locations, tracker) {
  for (const [namedExports, toRemove] of locations.getEntries()) {
    ...
    if (newElements.length === 0) {
      const declaration = nodes.closestNode(
        namedExports,
        ts__default['default'].isExportDeclaration,
      );
      if (declaration) {
        tracker.removeNode(declaration);
      }
      ...
      tracker.replaceNode(
        namedExports,
        ts__default['default'].factory.updateNamedExports(namedExports, newElements),
      );
    }
  }
}
  ...
  for (const node of sourceFile.statements) {
    ...
    if (
      ...
      (ts__default['default'].canHaveModifiers(node) &&
        ts__default['default']
          .getModifiers(node)
          ?.some((m) => m.kind === ts__default['default'].SyntaxKind.ExportKeyword))
    ) {
      return false;
    }
  }
...
function findNgModuleDecorator(node, typeChecker) {
  const decorators = ng_decorators.getAngularDecorators(
    typeChecker,
...
function isInImportsArray(closestAssignment, closestArray) {
  return (
    ...
    (ts__default['default'].isIdentifier(closestAssignment.name) ||
      ts__default['default'].isStringLiteralLike(closestAssignment.name)) &&
...
function analyzeBootstrapCall(call, typeChecker, templateTypeChecker) {
  ...
  const decorator = ng_decorators
    .getAngularDecorators(typeChecker, ts__default['default'].getDecorators(declaration) || [])
  ...
  if (providers && ts__default['default'].isPropertyAssignment(providers)) {
    ...
    if (ts__default['default'].isArrayLiteralExpression(providers.initializer)) {
      ...
      providersInNewCall.push(
        ts__default['default'].factory.createSpreadElement(providers.initializer),
      );
    }
...
function replaceBootstrapCallExpression(analysis, providers, modules, tracker) {
  ...
  if (providers.length > 0 || modules.length > 0) {
    ...
    if (modules.length > 0) {
      ...
      combinedProviders.push(
        ts__default['default'].factory.createCallExpression(importProvidersExpression, [], modules),
      );
    }
    // Push the providers after `importProvidersFrom` call for better readability.
    combinedProviders.push(...providers);
    const providersArray = ts__default['default'].factory.createNodeArray(
      combinedProviders,
    ...
    const initializer = remapDynamicImports(
      ...
      ts__default['default'].factory.createArrayLiteralExpression(
        providersArray,
        combinedProviders.length > 1,
      ),
    );
    args.push(
      ts__default['default'].factory.createObjectLiteralExpression(
        [ts__default['default'].factory.createPropertyAssignment('providers', initializer)],
        true,
      ),
    );
  }
  tracker.replaceNode(
    analysis.call,
    ts__default['default'].factory.createCallExpression(bootstrapExpression, [], args),
...
function migrateImportsForBootstrapCall(
  ...
  for (const element of imports.initializer.elements) {
    ...
    if (
      ts__default['default'].isCallExpression(element) &&
    ...
    const decorators = classDeclaration
      ? ng_decorators.getAngularDecorators(
          typeChecker,
          ts__default['default'].getDecorators(classDeclaration) || [],
        )
      : undefined;
    if (
      !decorators ||
...
function getRouterModuleForRootFeatures(sourceFile, options, tracker) {
  ...
  for (const prop of options.properties) {
    ...
    if (
      ...
      (!ts__default['default'].isIdentifier(prop.name) &&
        !ts__default['default'].isStringLiteralLike(prop.name))
    ) {
      return null;
    }
  ...
  if (inMemoryScrollingOptions.length > 0) {
    features.track(
      ...
      ts__default['default'].factory.createObjectLiteralExpression(inMemoryScrollingOptions),
    );
  }
  if (configOptions.length > 0) {
    features.track(
      'withRouterConfig',
      ts__default['default'].factory.createObjectLiteralExpression(configOptions),
    );
  }
...
function addNodesToCopy(targetFile, rootNode, nodeLookup, tracker, nodesToCopy, referenceResolver) {
  ...
  for (const ref of refs) {
    ...
    if (
      variableDeclaration &&
      variableStatement &&
      ts__default['default'].isIdentifier(variableDeclaration.name)
    ) {
...
function findAllSameFileReferences(rootNode, nodeLookup, referenceResolver) {
  ...
  (function walk(node) {
    ...
    for (const ref of refs) {
      ...
      if (
        !ts__default['default'].isImportDeclaration(closestTopLevel) &&
        isOutsideRange(
          excludeStart,
          excludeEnd,
...
function remapDynamicImports(targetFileName, rootNode) {
  ...
  const transformer = (context) => {
    return (sourceFile) =>
      ts__default['default'].visitNode(sourceFile, function walk(node) {
        if (
          ts__default['default'].isCallExpression(node) &&
...
function isReferenceIdentifier(node) {
  return (
    ...
    ((!ts__default['default'].isPropertyAssignment(node.parent) &&
      !ts__default['default'].isParameter(node.parent)) ||
...
function remapRelativeImport(targetFileName, specifier) {
  return getRelativeImportPath(
    targetFileName,
    p.join(p.dirname(specifier.getSourceFile().fileName), specifier.text),
  );
}
/**
...
function isExported(node) {
  return ts__default['default'].canHaveModifiers(node) && node.modifiers
    ? node.modifiers.some(
        (modifier) => modifier.kind === ts__default['default'].SyntaxKind.ExportKeyword,
      )
    : false;
}
/**
...
function hasImport(program, rootFileNames, moduleName) {
  ...
  for (const fileName of rootFileNames) {
    ...
    for (const statement of sourceFile.statements) {
      if (
        ts__default['default'].isImportDeclaration(statement) &&
...
function standaloneMigration(
  ...
  const {host, options, rootNames} = compiler_host.createProgramOptions(
    tree,
    tsconfigPath,
    basePath,
    undefined,
