'use strict';
/**
 * @license Angular v20.0.0-next.1+sha-639c717
    ...
    messages.forEach((message) => {
      ...
      message.sources.forEach((source) => {
        ...
        contextGroupTag.children.push(
          new checker.CR(10),
          new checker.Tag(_CONTEXT_TAG, {'context-type': 'sourcefile'}, [
            new checker.Text$1(source.filePath),
          ]),
          new checker.CR(10),
          new checker.Tag(_CONTEXT_TAG, {'context-type': 'linenumber'}, [
      ...
      transUnit.children.push(
        new checker.CR(8),
        new checker.Tag(_SOURCE_TAG$1, {}, visitor.serialize(message.nodes)),
        ...contextTags,
      );
      if (message.description) {
        transUnit.children.push(
          new checker.CR(8),
          new checker.Tag('note', {priority: '1', from: 'description'}, [
      ...
      if (message.meaning) {
        transUnit.children.push(
          new checker.CR(8),
          new checker.Tag('note', {priority: '1', from: 'meaning'}, [
            new checker.Text$1(message.meaning),
          ]),
        );
      }
...
class _WriteVisitor$1 {
  ...
  visitTagPlaceholder(ph, context) {
    ...
    if (ph.isVoid) {
      ...
      return [
        new checker.Tag(_PLACEHOLDER_TAG$1, {
          id: ph.startName,
          ctype,
          'equiv-text': `<${ph.tag}/>`,
        }),
    ...
    messages.forEach((message) => {
      ...
      if (message.description || message.meaning) {
        if (message.description) {
          notes.children.push(
            new checker.CR(8),
            new checker.Tag('note', {category: 'description'}, [
              new checker.Text$1(message.description),
            ]),
          );
        }
        if (message.meaning) {
          notes.children.push(
            new checker.CR(8),
            new checker.Tag('note', {category: 'meaning'}, [new checker.Text$1(message.meaning)]),
          );
        }
      }
      message.sources.forEach((source) => {
        notes.children.push(
          new checker.CR(8),
          new checker.Tag('note', {category: 'location'}, [
            new checker.Text$1(
      ...
      segment.children.push(
        new checker.CR(8),
        new checker.Tag(_SOURCE_TAG, {}, visitor.serialize(message.nodes)),
        new checker.CR(6),
      );
    ...
    switch (el.name) {
      ...
      case _PLACEHOLDER_SPANNING_TAG:
        ...
        if (!startAttr) {
          ...
          const nodes = [];
          return nodes.concat(
            new checker.Placeholder('', startId, el.sourceSpan),
            ...el.children.map((node) => node.visit(this, null)),
            new checker.Placeholder('', endId, el.sourceSpan),
          );
        }
        break;
      case _MARKER_TAG:
...
class MessageBundle {
  ...
  updateFromTemplate(source, url, interpolationConfig) {
    ...
    const rootNodes = this._preserveWhitespace
      ...
      : checker.visitAllWithSiblings(
          new checker.WhitespaceVisitor(/* preserveSignificantWhitespace */ false),
          htmlParserResult.rootNodes,
        );
    const i18nParserResult = checker.extractMessages(
      rootNodes,
      interpolationConfig,
      this._implicitTags,
      this._implicitAttrs,
...
class MapPlaceholderNames extends checker.CloneVisitor {
  ...
  visitTagPlaceholder(ph, mapper) {
    ...
    return new checker.TagPlaceholder(
      ph.tag,
      ph.attrs,
      startName,
      closeName,
  ...
  visitBlockPlaceholder(ph, mapper) {
    ...
    const closeName = ph.closeName ? mapper.toPublicName(ph.closeName) : ph.closeName;
    const children = ph.children.map((n) => n.visit(this, mapper));
    return new checker.BlockPlaceholder(
      ph.name,
      ph.parameters,
      startName,
      closeName,
...
function internalCompileClassMetadata(metadata) {
  return checker
    .importExpr(checker.Identifiers.setClassMetadata)
    .callFn([
      metadata.type,
      metadata.decorators,
...
function compileComponentClassMetadata(metadata, dependencies) {
  ...
  return internalCompileSetClassMetadataAsync(
    metadata,
    dependencies.map((dep) => new checker.FnParam(dep.symbolName, checker.DYNAMIC_TYPE)),
...
function internalCompileSetClassMetadataAsync(metadata, wrapperParams, dependencyResolverFn) {
  ...
  const setClassMetaAsync = checker
    .importExpr(checker.Identifiers.setClassMetadataAsync)
    .callFn([metadata.type, dependencyResolverFn, setClassMetaWrapper]);
  return checker
    .arrowFn([], [checker.devOnlyGuardedExpression(setClassMetaAsync).toStmt()])
    .callFn([]);
}
/**
...
function compileComponentMetadataAsyncResolver(dependencies) {
  const dynamicImports = dependencies.map(({symbolName, importPath, isDefaultImport}) => {
    ...
    const innerFn =
      ...
      checker.arrowFn(
        [new checker.FnParam('m', checker.DYNAMIC_TYPE)],
        checker.variable('m').prop(isDefaultImport ? 'default' : symbolName),
      );
...
function compileDeclareClassMetadata(metadata) {
  ...
  definitionMap.set('version', checker.literal('20.0.0-next.1+sha-639c717'));
  ...
  return checker
    .importExpr(checker.Identifiers.declareClassMetadata)
...
function compileComponentDeclareClassMetadata(metadata, dependencies) {
  ...
  return checker
    .importExpr(checker.Identifiers.declareClassMetadataAsync)
    .callFn([definitionMap.toLiteralMap()]);
}
...
function compileDeclareDirectiveFromMetadata(meta) {
  ...
  const expression = checker
    .importExpr(checker.Identifiers.declareDirective)
    .callFn([definitionMap.toLiteralMap()]);
...
function createHostDirectives(hostDirectives) {
  const expressions = hostDirectives.map((current) => {
    ...
    const inputsLiteral = current.inputs
      ? checker.createHostDirectivesMappingArray(current.inputs)
      : null;
        ...
        value: checker.literalMap([
          {
            ...
            quoted: false,
            value: checker.asLiteral(value.classPropertyName),
          },
          ...
          {key: 'isRequired', quoted: false, value: checker.asLiteral(value.required)},
          {
            ...
            quoted: false,
            value: value.transformFunction ?? checker.NULL_EXPR,
          },
        ]),
      };
    }),
...
function compileDeclareComponentFromMetadata(meta, template, additionalTemplateInfo) {
  ...
  const expression = checker
    .importExpr(checker.Identifiers.declareComponent)
    .callFn([definitionMap.toLiteralMap()]);
...
function createComponentDefinitionMap(meta, template, templateInfo) {
  ...
  if (meta.changeDetection !== null) {
    ...
    definitionMap.set(
      'changeDetection',
      checker
        .importExpr(checker.Identifiers.ChangeDetectionStrategy)
  ...
  if (meta.encapsulation !== checker.ViewEncapsulation.Emulated) {
    definitionMap.set(
      'encapsulation',
      checker
        .importExpr(checker.Identifiers.ViewEncapsulation)
        .prop(checker.ViewEncapsulation[meta.encapsulation]),
    );
  }
  if (meta.interpolation !== checker.DEFAULT_INTERPOLATION_CONFIG) {
    definitionMap.set(
      'interpolation',
      checker.literalArr([
        checker.literal(meta.interpolation.start),
        checker.literal(meta.interpolation.end),
      ]),
    );
  }
...
function compileDeclareFactoryFunction(meta) {
  ...
  definitionMap.set(
    'target',
    checker.importExpr(checker.Identifiers.FactoryTarget).prop(checker.FactoryTarget[meta.target]),
  );
  return {
    expression: checker
      .importExpr(checker.Identifiers.declareFactory)
      .callFn([definitionMap.toLiteralMap()]),
    statements: [],
...
function compileDeclareInjectableFromMetadata(meta) {
  ...
  const expression = checker
    .importExpr(checker.Identifiers.declareInjectable)
    .callFn([definitionMap.toLiteralMap()]);
...
function createInjectableDefinitionMap(meta) {
  ...
  if (meta.useExisting !== undefined) {
    definitionMap.set(
      'useExisting',
      checker.convertFromMaybeForwardRefExpression(meta.useExisting),
    );
  }
...
function compileDeclareInjectorFromMetadata(meta) {
  ...
  const expression = checker
    .importExpr(checker.Identifiers.declareInjector)
    .callFn([definitionMap.toLiteralMap()]);
...
function compileDeclareNgModuleFromMetadata(meta) {
  ...
  const expression = checker
    .importExpr(checker.Identifiers.declareNgModule)
    .callFn([definitionMap.toLiteralMap()]);
...
function createNgModuleDefinitionMap(meta) {
  ...
  if (meta.declarations.length > 0) {
    definitionMap.set(
      'declarations',
      checker.refsToArray(meta.declarations, meta.containsForwardDecls),
    );
  }
  if (meta.imports.length > 0) {
...
function compileDeclarePipeFromMetadata(meta) {
  ...
  const expression = checker
    .importExpr(checker.Identifiers.declarePipe)
    .callFn([definitionMap.toLiteralMap()]);
...
function compileClassDebugInfo(debugInfo) {
  ...
  const fnCall = checker
    .importExpr(checker.Identifiers.setClassDebugInfo)
...
function compileHmrInitializer(meta) {
  ...
  const replaceCall = checker
    .importExpr(checker.Identifiers.replaceMetadata)
    .callFn([
      meta.type,
      defaultRead,
  ...
  const replaceCallback = checker.arrowFn(
    [new checker.FnParam(moduleName)],
    defaultRead.and(replaceCall),
  );
  // '<url>?c=' + id + '&t=' + encodeURIComponent(t)
  const urlValue = checker
  ...
  const urlHref = new checker.InstantiateExpr(checker.variable('URL'), [urlValue, urlBase]).prop(
    'href',
  );
  // function Cmp_HmrLoad(t) {
  //   import(/* @vite-ignore */ urlHref).then((m) => m.default && replaceMetadata(...));
  // }
  const importCallback = new checker.DeclareFunctionStmt(
    importCallbackName,
    [new checker.FnParam(timestampName)],
    [
    ...
    null,
    checker.StmtModifier.Final,
  );
  // (d) => d.id === id && Cmp_HmrLoad(d.timestamp)
  const updateCallback = checker.arrowFn(
    [new checker.FnParam(dataName)],
    checker
      .variable(dataName)
      .prop('id')
      .identical(checker.variable(idName))
      .and(
        checker.variable(importCallbackName).callFn([checker.variable(dataName).prop('timestamp')]),
      ),
  );
  ...
  return checker
    .arrowFn(
      ...
      [
        // const id = <id>;
        new checker.DeclareVarStmt(
          idName,
          checker.literal(encodeURIComponent(`${meta.filePath}@${meta.className}`)),
          null,
          checker.StmtModifier.Final,
        ),
...
function compileHmrUpdateCallback(definitions, constantStatements, meta) {
  ...
  const params = [meta.className, namespaces].map(
    (name) => new checker.FnParam(name, checker.DYNAMIC_TYPE),
  );
  const body = [];
  ...
  for (let i = 0; i < meta.namespaceDependencies.length; i++) {
    body.push(
      new checker.DeclareVarStmt(
        ...
        checker.variable(namespaces).key(checker.literal(i)),
        checker.DYNAMIC_TYPE,
        checker.StmtModifier.Final,
      ),
    );
  }
  ...
  return new checker.DeclareFunctionStmt(
...
function relativePathBetween(from, to) {
  const relativePath = checker.stripExtension(
    checker.relative(checker.dirname(checker.resolve(from)), checker.resolve(to)),
  );
...
class ImportedSymbolsTracker {
  ...
  scanImports(sourceFile) {
    ...
    for (const stmt of sourceFile.statements) {
      if (
        !ts__default['default'].isImportDeclaration(stmt) ||
...
class ModuleResolver {
  ...
  resolveModule(moduleName, containingFile) {
    const resolved = checker.resolveModuleName(
      moduleName,
      containingFile,
    ...
    return checker.getSourceFileOrNull(
      this.program,
      checker.absoluteFrom(resolved.resolvedFileName),
    );
  }
}
        ...
        if (name === 'Inject') {
          if (dec.args === null || dec.args.length !== 1) {
            throw new checker.FatalDiagnosticError(
              checker.ErrorCode.DECORATOR_ARITY_WRONG,
              dec.node,
          ...
          if (ts__default['default'].isStringLiteralLike(attributeName)) {
            ...
            attributeNameType = new checker.WrappedNodeExpr(
              ts__default['default'].factory.createKeywordTypeNode(
          ...
          throw new checker.FatalDiagnosticError(
            checker.ErrorCode.DECORATOR_UNEXPECTED,
            dec.node,
...
function createUnsuitableInjectionTokenError(clazz, error) {
  ...
  switch (reason.kind) {
    case 5 /* ValueUnavailableKind.UNSUPPORTED */:
      ...
      hints = [
        checker.makeRelatedInformation(
          reason.typeNode,
  ...
  return new checker.FatalDiagnosticError(
    checker.ErrorCode.PARAM_MISSING_TOKEN,
    param.nameNode,
    chain,
    hints,
  );
...
class DtsMetadataReader {
  ...
  getNgModuleMetadata(ref) {
    ...
    const declarations = checker.extractReferencesFromType(
      this.checker,
      declarationMetadata,
      ref.bestGuessOwningModule,
    );
    const exports = checker.extractReferencesFromType(
      this.checker,
      exportMetadata,
      ref.bestGuessOwningModule,
    );
    const imports = checker.extractReferencesFromType(
      this.checker,
      importMetadata,
      ref.bestGuessOwningModule,
    );
  ...
  getDirectiveMetadata(ref) {
    ...
    const ngContentSelectors =
      def.type.typeArguments.length > 6
        ? checker.readStringArrayType(def.type.typeArguments[6])
        : null;
    ...
    const isStandalone =
      def.type.typeArguments.length > 7 &&
      (checker.readBooleanType(def.type.typeArguments[7]) ?? false);
    const inputs = ClassPropertyMapping.fromMappedObject(readInputsType(def.type.typeArguments[3]));
    const outputs = ClassPropertyMapping.fromMappedObject(
      checker.readMapType(def.type.typeArguments[4], checker.readStringType),
    );
    const hostDirectives =
      def.type.typeArguments.length > 8
        ...
        : null;
    const isSignal =
      def.type.typeArguments.length > 9 &&
      (checker.readBooleanType(def.type.typeArguments[9]) ?? false);
  ...
  getPipeMetadata(ref) {
    ...
    if (
      !ts__default['default'].isLiteralTypeNode(type) ||
      !ts__default['default'].isStringLiteral(type.literal)
    ) {
    ...
    const isStandalone =
      def.type.typeArguments.length > 2 &&
      (checker.readBooleanType(def.type.typeArguments[2]) ?? false);
    return {
    ...
    for (const member of type.members) {
      if (
        ...
        (!ts__default['default'].isStringLiteral(member.name) &&
          !ts__default['default'].isIdentifier(member.name))
      ) {
        continue;
      }
...
function readHostDirectivesType(checker$1, type, bestGuessOwningModule) {
  ...
  for (const hostDirectiveType of type.elements) {
    ...
    if (directive) {
      ...
      const ref = checker.extraReferenceFromTypeQuery(
        checker$1,
        directive,
        type,
...
class TraceDynamicValueVisitor {
  ...
  visitDynamicInput(value) {
    ...
    if (this.shouldTrace(value.node)) {
      const info = checker.makeRelatedInformation(
        value.node,
  ...
  visitSyntheticInput(value) {
    return [
      checker.makeRelatedInformation(value.node, 'Unable to evaluate this expression further.'),
    ];
  }
  ...
  visitInvalidExpressionType(value) {
    return [
      checker.makeRelatedInformation(value.node, 'Unable to evaluate an invalid expression.'),
    ];
  }
  visitUnknown(value) {
...
class PartialEvaluator {
  ...
  evaluate(expr, foreignFunctionResolver) {
    const interpreter = new checker.StaticInterpreter(
      this.host,
      this.checker,
...
class TraitCompiler {
  ...
  analyze(sf, preanalyze) {
    ...
    const priorWork =
      this.compilationMode !== checker.CompilationMode.LOCAL
  ...
  adopt(priorRecord) {
    ...
    for (const priorTrait of priorRecord.traits) {
      ...
      if (
        priorTrait.state === checker.TraitState.Analyzed ||
        priorTrait.state === checker.TraitState.Resolved
      ) {
  ...
  detectTraits(clazz, decorators) {
    ...
    const nonNgDecoratorsInLocalMode =
      this.compilationMode === checker.CompilationMode.LOCAL ? new Set(decorators) : null;
      ...
      if (record === null) {
        ...
        if (!isWeakHandler && record.hasWeakHandlers) {
          ...
          record.traits = record.traits.filter(
            (field) => field.handler.precedence !== checker.HandlerPrecedence.WEAK,
          );
  ...
  analyzeTrait(clazz, trait) {
    if (trait.state !== checker.TraitState.Pending) {
      throw new Error(
        `Attempt to analyze trait of ${clazz.name.text} in state ${checker.TraitState[trait.state]} (expected DETECTED)`,
      );
    }
  ...
  xi18n(bundle) {
    for (const clazz of this.classes.keys()) {
      ...
      for (const trait of record.traits) {
        if (
          trait.state !== checker.TraitState.Analyzed &&
          trait.state !== checker.TraitState.Resolved
        ) {
  ...
  updateResources(clazz) {
    ...
    for (const trait of record.traits) {
      if (
        trait.state !== checker.TraitState.Resolved ||
        trait.handler.updateResources === undefined
      ) {
        continue;
      }
  ...
  compile(clazz, constantPool) {
    ...
    for (const trait of record.traits) {
      ...
      if (this.compilationMode === checker.CompilationMode.LOCAL) {
        ...
        // `Readonly` works.
        if (
          this.compilationMode === checker.CompilationMode.PARTIAL &&
  ...
  decoratorsFor(node) {
    ...
    for (const trait of record.traits) {
      ...
      if (
        trait.detected.trigger !== null &&
        ts__default['default'].isDecorator(trait.detected.trigger)
      ) {
...
class IvyDeclarationDtsTransform {
  ...
  transformClass(clazz, members, reflector, refEmitter, imports) {
    ...
    const newMembers = fields.map((decl) => {
      const modifiers = [
        ts__default['default'].factory.createModifier(
          ts__default['default'].SyntaxKind.StaticKeyword,
        ),
      ];
      const typeRef = checker.translateType(
        decl.type,
...
class Visitor {
  ...
  _visit(node, context) {
    ...
    node = ts__default['default'].visitEachChild(
      node,
      (child) => child && this._visit(child, context),
      context,
    );
    if (ts__default['default'].isClassDeclaration(node)) {
    ...
    if (
      visitedNode &&
      (ts__default['default'].isBlock(visitedNode) ||
        ts__default['default'].isSourceFile(visitedNode))
    ) {
  ...
  _maybeProcessStatements(node) {
    ...
    const statementsArray = ts__default['default'].factory.createNodeArray(
      newStatements,
    ...
    if (ts__default['default'].isBlock(node)) {
      ...
      return ts__default['default'].factory.updateSourceFile(
        node,
        statementsArray,
...
function ivyTransformFactory(
  ...
  return (context) => {
    return (file) => {
      return perf.inPhase(checker.PerfPhase.Compile, () =>
...
class IvyTransformationVisitor extends Visitor {
  ...
  visitClassDeclaration(node) {
    ...
    for (const field of this.classCompilationMap.get(node)) {
      ...
      const exprNode = checker.translateExpression(
        sourceFile,
        field.initializer,
        this.importManager,
        translateOptions,
      ...
      const property = ts__default['default'].factory.createPropertyDeclaration(
        [
          ts__default['default'].factory.createToken(
            ts__default['default'].SyntaxKind.StaticKeyword,
          ),
        ],
        field.name,
      ...
      if (this.isClosureCompilerEnabled) {
        ...
        ts__default['default'].addSyntheticLeadingComment(
          property,
      ...
      field.statements
        .map((stmt) =>
          checker.translateStatement(sourceFile, stmt, this.importManager, translateOptions),
        )
    ...
    const filteredDecorators =
      ...
      maybeFilterDecorator(
        ts__default['default'].getDecorators(node),
        this.compilation.decoratorsFor(node),
      );
    ...
    node = ts__default['default'].factory.updateClassDeclaration(
      node,
      updatedModifiers,
      node.name,
      node.typeParameters,
  ...
  _stripAngularDecorators(node) {
    const modifiers = ts__default['default'].canHaveModifiers(node)
      ? ts__default['default'].getModifiers(node)
      : undefined;
    ...
    if (ts__default['default'].isParameter(node)) {
      ...
      node = ts__default['default'].factory.updateParameterDeclaration(
        node,
        combinedModifiers,
        node.dotDotDotToken,
        node.name,
      ...
      node = ts__default['default'].factory.updateMethodDeclaration(
        node,
        combinedModifiers,
        node.asteriskToken,
        node.name,
      ...
      node = ts__default['default'].factory.updatePropertyDeclaration(
        node,
        combinedModifiers,
        node.name,
        node.questionToken,
      ...
      node = ts__default['default'].factory.updateGetAccessorDeclaration(
        node,
        combinedModifiers,
        node.name,
        node.parameters,
      ...
      node = ts__default['default'].factory.updateSetAccessorDeclaration(
        node,
        combinedModifiers,
        node.name,
        node.parameters,
      ...
      node = ts__default['default'].factory.updateConstructorDeclaration(
        node,
        modifiers,
        parameters,
        node.body,
...
function transformIvySourceFile(
  ...
  const downlevelTranslatedCode =
    getLocalizeCompileTarget(context) < ts__default['default'].ScriptTarget.ES2015;
  const constants = constantPool.statements.map((stmt) =>
    checker.translateStatement(file, stmt, importManager, {
      recordWrappedNode,
...
function getLocalizeCompileTarget(context) {
  ...
  return target !== ts__default['default'].ScriptTarget.JSON
    ? target
    : ts__default['default'].ScriptTarget.ES2015;
}
...
function maybeFilterDecorator(decorators, toRemove) {
  ...
  const filtered = decorators.filter(
    (dec) =>
      toRemove.find(
        (decToRemove) => ts__default['default'].getOriginalNode(dec) === decToRemove,
      ) === undefined,
  );
  if (filtered.length === 0) {
    return undefined;
...
function makeDuplicateDeclarationError(node, data, kind) {
  ...
  return checker.makeDiagnostic(
    checker.ErrorCode.NGMODULE_DECLARATION_NOT_UNIQUE,
    node.name,
...
function createValueHasWrongTypeError(node, value, messageText) {
  ...
  if (value instanceof checker.DynamicValue) {
    ...
    relatedInformation = [
      checker.makeRelatedInformation(referenceNode, 'Reference is declared here.'),
    ];
  } else {
  ...
  return new checker.FatalDiagnosticError(
    checker.ErrorCode.VALUE_HAS_WRONG_TYPE,
    node,
    chain,
    relatedInformation,
  );
...
function getProviderDiagnostics(providerClasses, providersDeclaration, registry) {
  ...
  for (const provider of providerClasses) {
    ...
    diagnostics.push(
      checker.makeDiagnostic(
        checker.ErrorCode.UNDECORATED_PROVIDER,
        contextNode,
...
`,
        [
          checker.makeRelatedInformation(
            provider.node,
            `'${provider.node.name.text}' is declared here.`,
          ),
        ],
      ),
    );
...
function getDirectiveDiagnostics(
  node,
  injectableRegistry,
  evaluator,
  reflector,
...
function validateHostDirectives(origin, hostDirectives, metaReader) {
  ...
  for (const current of hostDirectives) {
    ...
    if (hostMeta === null) {
      diagnostics.push(
        checker.makeDiagnostic(
          checker.ErrorCode.HOST_DIRECTIVE_INVALID,
    ...
    if (!hostMeta.isStandalone) {
      diagnostics.push(
        checker.makeDiagnostic(
          checker.ErrorCode.HOST_DIRECTIVE_NOT_STANDALONE,
          current.directive.getOriginForDiagnostics(origin),
    ...
    if (hostMeta.isComponent) {
      diagnostics.push(
        checker.makeDiagnostic(
          checker.ErrorCode.HOST_DIRECTIVE_COMPONENT,
          current.directive.getOriginForDiagnostics(origin),
...
function validateHostDirectiveMappings(
  ...
  for (const publicName in hostDirectiveMappings) {
    if (hostDirectiveMappings.hasOwnProperty(publicName)) {
      ...
      if (bindings === null) {
        diagnostics.push(
          checker.makeDiagnostic(
            checker.ErrorCode.HOST_DIRECTIVE_UNDEFINED_BINDING,
      ...
      if (bindingsForPublicName !== null) {
        for (const binding of bindingsForPublicName) {
          if (binding.bindingPropertyName !== publicName) {
            diagnostics.push(
              checker.makeDiagnostic(
                checker.ErrorCode.HOST_DIRECTIVE_CONFLICTING_ALIAS,
                hostDirectiveMeta.directive.getOriginForDiagnostics(origin),
  ...
  if (requiredBindings !== null && requiredBindings.size !== exposedRequiredBindings.size) {
    ...
    diagnostics.push(
      checker.makeDiagnostic(
        checker.ErrorCode.HOST_DIRECTIVE_MISSING_REQUIRED_BINDING,
        hostDirectiveMeta.directive.getOriginForDiagnostics(origin),
...
function getUndecoratedClassWithAngularFeaturesDiagnostic(node) {
  return checker.makeDiagnostic(
    checker.ErrorCode.UNDECORATED_CLASS_USING_ANGULAR_FEATURES,
    node.name,
    `Class is using Angular features but is not decorated. Please add an explicit ` +
      `Angular decorator.`,
  );
}
function checkInheritanceOfInjectable(
  node,
  injectableRegistry,
  reflector,
  evaluator,
...
function getInheritedInvalidCtorDiagnostic(node, baseClass, kind) {
  ...
  return checker.makeDiagnostic(
    checker.ErrorCode.INJECTABLE_INHERITS_INVALID_CONSTRUCTOR,
    node.name,
...
function getInheritedUndecoratedCtorDiagnostic(node, baseClass, kind) {
  ...
  return checker.makeDiagnostic(
    checker.ErrorCode.DIRECTIVE_INHERITS_UNDECORATED_CTOR,
    node.name,
    `The ${kind.toLowerCase()} ${node.name.text} inherits its constructor from ${baseClassName}, ` +
 ...
 */
function assertLocalCompilationUnresolvedConst(
  compilationMode,
  value,
  nodeToHighlight,
  errorMessage,
  ...
  if (
    ...
    throw new checker.FatalDiagnosticError(
      checker.ErrorCode.LOCAL_COMPILATION_UNRESOLVED_CONST,
...
function resolveEnumValue(evaluator, metadata, field, enumSymbolName) {
  ...
  if (metadata.has(field)) {
    ...
    if (
      value instanceof checker.EnumValue &&
      checker.isAngularCoreReference(value.enumRef, enumSymbolName)
    ) {
...
function resolveLiteral(decorator, literalCache) {
  ...
  if (!ts__default['default'].isObjectLiteralExpression(meta)) {
    throw new checker.FatalDiagnosticError(
      checker.ErrorCode.DECORATOR_ARG_NOT_LITERAL,
      meta,
...
function extractClassMetadata(
  ...
  const metaDecorators = new checker.WrappedNodeExpr(
    ts__default['default'].factory.createArrayLiteralExpression(ngClassDecorators),
  );
  ...
  if (classCtorParameters !== null) {
    ...
    metaCtorParameters = new checker.ArrowFunctionExpr(
      [],
      new checker.LiteralArrayExpr(ctorParameters),
    );
  }
  ...
  if (decoratedMembers.length > 0) {
    metaPropDecorators = new checker.WrappedNodeExpr(
      ts__default['default'].factory.createObjectLiteralExpression(decoratedMembers),
    );
  }
  return {
...
function ctorParameterToMetadata(param, isCore) {
  ...
  if (param.decorators !== null) {
    ...
    const value = new checker.WrappedNodeExpr(
      ts__default['default'].factory.createArrayLiteralExpression(ngDecorators),
    );
...
function decoratorToMetadata(decorator, wrapFunctionsInParens) {
  ...
  if (decorator.args !== null && decorator.args.length > 0) {
    ...
    properties.push(
      ts__default['default'].factory.createPropertyAssignment(
        'args',
        ts__default['default'].factory.createArrayLiteralExpression(args),
      ),
    );
  }
...
function removeIdentifierReferences(node, names) {
  const result = ts__default['default'].transform(node, [
    (context) => (root) =>
      ts__default['default'].visitNode(root, function walk(current) {
...
function makeNotStandaloneDiagnostic(scopeReader, ref, rawExpr, kind) {
  ...
  if (scope !== null && scope.kind === checker.ComponentScopeKind.NgModule) {
    ...
    relatedInformation = [
      checker.makeRelatedInformation(scope.ngModule.name, relatedInfoMessageText),
    ];
  }
  ...
  return checker.makeDiagnostic(
    checker.ErrorCode.COMPONENT_IMPORT_NOT_STANDALONE,
...
function makeUnknownComponentImportDiagnostic(ref, rawExpr) {
  return checker.makeDiagnostic(
    checker.ErrorCode.COMPONENT_UNKNOWN_IMPORT,
    getDiagnosticNode(ref, rawExpr),
...
function makeUnknownComponentDeferredImportDiagnostic(ref, rawExpr) {
  return checker.makeDiagnostic(
    checker.ErrorCode.COMPONENT_UNKNOWN_DEFERRED_IMPORT,
    getDiagnosticNode(ref, rawExpr),
    ...
    for (const decl of ngModule.declarations) {
      ...
      if (directive !== null) {
        if (directive.isStandalone) {
          ...
          diagnostics.push(
            checker.makeDiagnostic(
              checker.ErrorCode.NGMODULE_DECLARATION_IS_STANDALONE,
        ...
        diagnostics.push(
          checker.makeDiagnostic(
            checker.ErrorCode.NGMODULE_INVALID_DECLARATION,
            errorNode,
            ...
            [
              checker.makeRelatedInformation(
                decl.node.name,
                `'${decl.node.name.text}' is declared here.`,
              ),
            ],
          ),
        );
      ...
      if (!ts__default['default'].isClassDeclaration(ref.node)) {
        ...
        const code =
          type === 'import'
            ...
            : checker.ErrorCode.NGMODULE_INVALID_EXPORT;
        diagnostics.push(
          checker.makeDiagnostic(
            code,
            checker.identifierOfNode(ref.node) || ref.node,
            `Appears in the NgModule.${type}s of ${checker.nodeNameForError(ownerForErrors)}, but could not be resolved to an NgModule`,
          ),
        );
        return 'invalid';
      }
    ...
    const addReexport = (exportRef) => {
      ...
      if (!reexportMap.has(exportName)) {
        if (exportRef.alias && exportRef.alias instanceof checker.ExternalExpr) {
          ...
          if (
            !(expr instanceof checker.ExternalExpr) ||
...
function invalidReexport(decl, rawExpr, isStandalone) {
  ...
  return checker.makeDiagnostic(
    checker.ErrorCode.NGMODULE_INVALID_REEXPORT,
    getDiagnosticNode(decl, rawExpr),
    message,
  );
}
...
function reexportCollision(module, refA, refB) {
  ...
  return checker.makeDiagnostic(
    checker.ErrorCode.NGMODULE_REEXPORT_NAME_COLLISION,
    module.name,
    `
    ...
    for (const meta of allDependencies) {
      if (meta.kind === checker.MetaKind.Directive && meta.selector !== null) {
        ...
        if (!ts__default['default'].isClassDeclaration(meta.ref.node)) {
          throw new Error(
            `Unexpected non-class declaration ${ts__default['default'].SyntaxKind[meta.ref.node.kind]} for pipe ${meta.ref.debugName}`,
          );
        }
...
const EMPTY_OBJECT = {};
const queryDecoratorNames = ['ViewChild', 'ViewChildren', 'ContentChild', 'ContentChildren'];
...
function extractDirectiveMetadata(
  ...
  if (directive.has('queries')) {
    ...
    const checkAndUnwrapQuery = (q) => {
      if (signalQueryFields.has(q.metadata.propertyName)) {
        throw new checker.FatalDiagnosticError(
          checker.ErrorCode.INITIALIZER_API_DECORATOR_METADATA_COLLISION,
          q.expr,
  ...
  if (directive.has('selector')) {
    ...
    if (!selector) {
      throw new checker.FatalDiagnosticError(
        checker.ErrorCode.DIRECTIVE_MISSING_SELECTOR,
        expr,
  ...
  const isStructural =
    ...
    ctorDeps.some(
      (dep) =>
        dep.token instanceof checker.ExternalExpr &&
  ...
  if (directive.has('standalone')) {
    ...
    if (!isStandalone && strictStandalone) {
      throw new checker.FatalDiagnosticError(
        checker.ErrorCode.NON_STANDALONE_NOT_ALLOWED,
        expr,
...
function extractDecoratorQueryMetadata(exprNode, name, args, propertyName, reflector, evaluator) {
  ...
  if (arg instanceof checker.Reference || arg instanceof checker.DynamicValue) {
    ...
    predicate = checker.createMayBeForwardRefExpression(
      new checker.WrappedNodeExpr(node),
  ...
  checker
    ...
    .forEach(({member, decorators}) => {
      decorators.forEach((decorator) => {
        ...
        bindings.properties[hostPropertyName] = checker.getSafePropertyAccessString(
          'this',
          member.name,
        );
      });
  ...
  if (compilationMode === checker.CompilationMode.LOCAL) {
    ...
    if (Array.isArray(value)) {
      const entry = value.find(
        (e) => e instanceof checker.DynamicValue && e.isFromUnknownIdentifier(),
      );
...
function tryGetQueryFromFieldDecorator(member, reflector, evaluator, isCore) {
  ...
  if (queryDecorators.length !== 1) {
    throw new checker.FatalDiagnosticError(
      checker.ErrorCode.DECORATOR_COLLISION,
...
function isPropertyTypeMember(member) {
  return (
    member.kind === checker.ClassMemberKind.Getter ||
  ...
  for (let i = 0; i < inputsArray.length; i++) {
    ...
    if (typeof value === 'string') {
      ...
      if (value.has('transform')) {
        ...
        if (
          !(transformValue instanceof checker.DynamicValue) &&
          !(transformValue instanceof checker.Reference)
        ) {
          throw createValueHasWrongTypeError(
            inputsField,
            transformValue,
...
function tryParseInputFieldMapping(
  ...
  if (decorator !== null && signalInputMapping !== null) {
    throw new checker.FatalDiagnosticError(
      checker.ErrorCode.INITIALIZER_API_WITH_DISALLOWED_DECORATOR,
      decorator.node,
...
function parseInputFields(
  ...
  for (const member of members) {
    ...
    if (member.isStatic) {
      throw new checker.FatalDiagnosticError(
        checker.ErrorCode.INCORRECTLY_DECLARED_ON_STATIC_MEMBER,
        member.node ?? clazz,
 ...
 *
 */
function parseDecoratorInputTransformFunction(
  clazz,
  classPropertyName,
  value,
  reflector,
  ...
  if (compilationMode === checker.CompilationMode.LOCAL) {
    const node =
      value instanceof checker.Reference ? value.getIdentityIn(clazz.getSourceFile()) : value.node;
  ...
  for (const member of members) {
    ...
    if (member.name === conflictingName && member.isStatic) {
      throw new checker.FatalDiagnosticError(
        checker.ErrorCode.CONFLICTING_INPUT_TRANSFORM,
        value.node,
...
function assertEmittableInputType(type, contextFile, reflector, refEmitter) {
  (function walk(node) {
    if (
      ts__default['default'].isTypeReferenceNode(node) &&
      ...
      if (declaration !== null) {
        ...
        if (declaration.node.getSourceFile() !== contextFile) {
          const emittedType = refEmitter.emit(
            new checker.Reference(
              declaration.node,
              declaration.viaModule === checker.AmbientImport ? checker.AmbientImport : null,
            ),
            contextFile,
            checker.ImportFlags.NoAliasing |
          ...
          throw new checker.FatalDiagnosticError(
            checker.ErrorCode.SYMBOL_NOT_EXPORTED,
            type,
            `Symbol must be exported in order to be used as the type of an Input transform function`,
            [checker.makeRelatedInformation(declaration.node, `The symbol is declared here.`)],
          );
        }
      }
    }
...
function parseQueriesOfClassFields(members, reflector, importTracker, evaluator, isCore) {
  ...
  for (const member of members) {
    ...
    const signalQuery = checker.tryParseSignalQueryFromInitializer(
      member,
      reflector,
      importTracker,
    );
...
function parseOutputFields(
  ...
  for (const member of members) {
    ...
    const initializerOutput = checker.tryParseInitializerBasedOutput(
      member,
      reflector,
      importTracker,
    );
...
function evaluateHostExpressionBindings(hostExpr, evaluator) {
  ...
  if (errors.length > 0) {
    throw new checker.FatalDiagnosticError(
      checker.ErrorCode.HOST_BINDING_PARSE_ERROR,
...
function getHostBindingErrorNode(error, hostExpr) {
  ...
  if (
    ts__default['default'].isObjectLiteralExpression(hostExpr) &&
    error.relatedError instanceof checker.ParserError
  ) {
    for (const prop of hostExpr.properties) {
      if (
        ts__default['default'].isPropertyAssignment(prop) &&
...
function extractHostDirectives(rawHostDirectives, evaluator, compilationMode) {
  ...
  return resolved.map((value) => {
    ...
    if (
      compilationMode === checker.CompilationMode.LOCAL &&
      hostReference instanceof checker.DynamicValue
    ) {
      ...
      if (
        !ts__default['default'].isIdentifier(hostReference.node) &&
        !ts__default['default'].isPropertyAccessExpression(hostReference.node)
      ) {
        throw new checker.FatalDiagnosticError(
          checker.ErrorCode.LOCAL_COMPILATION_UNSUPPORTED_EXPRESSION,
          hostReference.node,
...
function toHostDirectiveMetadata(hostDirective, context, refEmitter) {
  ...
  if (hostDirective.directive instanceof checker.Reference) {
    directive = checker.toR3Reference(
...
function toR3InputMetadata(mapping) {
  return {
    ...
    transformFunction:
      mapping.transform !== null ? new checker.WrappedNodeExpr(mapping.transform.node) : null,
...
class DirectiveDecoratorHandler {
  ...
  analyze(node, decorator) {
    ...
    if (directiveResult !== undefined && directiveResult.decorator.has('providers')) {
      providersRequiringFactory = checker.resolveProvidersRequiringFactory(
    ...
    return {
      analysis: {
        ...
        typeCheckMeta: checker.extractDirectiveTypeCheckMeta(
          node,
          directiveResult.inputs,
          this.reflector,
        ),
  ...
  compileFull(node, analysis, resolution, pool) {
    const fac = compileNgFactoryDefField(
      checker.toFactoryMetadata(analysis.meta, checker.FactoryTarget.Directive),
    );
    const def = checker.compileDirectiveFromMetadata(
      analysis.meta,
      pool,
      checker.makeBindingParser(),
    );
  ...
  compilePartial(node, analysis, resolution) {
    const fac = compileDeclareFactory(
      checker.toFactoryMetadata(analysis.meta, checker.FactoryTarget.Directive),
    );
      ...
      if (member.decorators) {
        return member.decorators.some((decorator) =>
          FIELD_DECORATORS.some((decoratorName) =>
            checker.isAngularDecorator(decorator, decoratorName, this.isCore),
          ),
        );
      }
      return false;
...
function createModuleWithProvidersResolver(reflector, isCore) {
  ...
  function _reflectModuleFromTypeParam(type, node) {
    ...
    if (type.typeArguments === undefined || type.typeArguments.length !== 1) {
      ...
      throw new checker.FatalDiagnosticError(
        checker.ErrorCode.NGMODULE_MODULE_WITH_PROVIDERS_MISSING_GENERIC,
        type,
    ...
    for (const t of type.types) {
      if (ts__default['default'].isTypeLiteralNode(t)) {
        for (const m of t.members) {
          const ngModuleType =
            (ts__default['default'].isPropertySignature(m) &&
  ...
  return (fn, callExpr, resolve, unresolvable) => {
    ...
    if (
      !(ngModule instanceof checker.Reference) ||
      !checker.isNamedClassDeclaration(ngModule.node)
    ) {
      return unresolvable;
    }
  ...
  analyze(node, decorator) {
          ...
          diagnostics.push(
            checker.makeDiagnostic(
              ...
              [
                checker.makeRelatedInformation(
                  ref.node.name,
                  `'${ref.node.name.text}' is declared here.`,
                ),
              ],
            ),
          );
    ...
    if (ngModule.has('id')) {
      ...
      if (!isModuleIdExpression(idExpr)) {
        ...
        const diag = checker.makeDiagnostic(
          checker.ErrorCode.WARN_NGMODULE_ID_UNNECESSARY,
          idExpr,
    ...
    const isForwardReference = (ref) =>
      checker.isExpressionForwardReference(ref.value, node.name, valueContext);
    ...
    if (this.compilationMode === checker.CompilationMode.LOCAL) {
      ngModuleMetadata = {
        ...
        declarationsExpression: rawDeclarations
          ? new checker.WrappedNodeExpr(rawDeclarations)
          : null,
    ...
    if (
      rawProviders !== null &&
      (!ts__default['default'].isArrayLiteralExpression(rawProviders) ||
        rawProviders.elements.length > 0)
    ) {
      wrappedProviders = new checker.WrappedNodeExpr(
        this.annotateForClosureCompiler
    ...
    if (this.compilationMode === checker.CompilationMode.LOCAL) {
      ...
      for (const exp of [rawImports, rawExports]) {
        ...
        if (ts__default['default'].isArrayLiteralExpression(exp)) {
          ...
          if (exp.elements) {
            injectorMetadata.imports.push(
              ...exp.elements.map((n) => new checker.WrappedNodeExpr(n)),
            );
          }
        } else {
...
function isModuleIdExpression(expr) {
  return (
    ts__default['default'].isPropertyAccessExpression(expr) &&
...
function makeStandaloneBootstrapDiagnostic(ngModuleClass, bootstrappedClassRef, rawBootstrapExpr) {
  ...
  const relatedInformation = [
    checker.makeRelatedInformation(
      ngModuleClass,
      `The 'bootstrap' array is present on this NgModule.`,
    ),
  ];
  return checker.makeDiagnostic(
    checker.ErrorCode.NGMODULE_BOOTSTRAP_IS_STANDALONE,
...
function extractTemplate(
  ...
  if (template.isInline) {
    ...
    if (
      ts__default['default'].isStringLiteral(template.expression) ||
    ...
    if (depTracker !== null) {
      depTracker.addResourceDependency(
        node.getSourceFile(),
        checker.absoluteFrom(template.resolvedTemplateUrl),
      );
    }
    return {
      ...parseExtractedTemplate(
...
function parseTemplateDeclaration(
  ...
  if (component.has('templateUrl')) {
    ...
    throw new checker.FatalDiagnosticError(
      checker.ErrorCode.COMPONENT_MISSING_TEMPLATE,
      decorator.node,
      'component is missing a template',
    );
  }
}
function preloadAndParseTemplate(
  evaluator,
  resourceLoader,
  depTracker,
...
function getTemplateRange(templateExpr) {
  ...
  const {line, character} = ts__default['default'].getLineAndCharacterOfPosition(
...
function makeResourceNotFoundError(file, nodeForError, resourceType) {
  ...
  return new checker.FatalDiagnosticError(
    checker.ErrorCode.COMPONENT_RESOURCE_NOT_FOUND,
    nodeForError,
    errorText,
  );
}
...
function transformDecoratorResources(dec, component, styles, template) {
  ...
  if (metadata.has('styleUrls') || metadata.has('styleUrl') || metadata.has('styles')) {
    ...
    if (styles.length > 0) {
      ...
      if (styleNodes.length > 0) {
        metadata.set(
          'styles',
          ts__default['default'].factory.createArrayLiteralExpression(styleNodes),
        );
      }
    }
  }
  ...
  return {
    ...dec,
    args: [ts__default['default'].factory.createObjectLiteralExpression(newMetadataFields)],
  };
}
function extractComponentStyleUrls(evaluator, component) {
  ...
  if (styleUrlsExpr !== undefined && styleUrlExpr !== undefined) {
    throw new checker.FatalDiagnosticError(
      checker.ErrorCode.COMPONENT_INVALID_STYLE_URLS,
      styleUrlExpr,
...
class ShimAdapter {
  ...
  maybeGenerate(fileName) {
    ...
    for (const record of this.generators) {
      ...
      let inputFile = this.delegate.getSourceFile(
        baseFileName,
      ...
      if (inputFile === undefined) {
        ...
        inputFile = this.delegate.getSourceFile(
          baseFileName,
          ts__default['default'].ScriptTarget.Latest,
        );
      }
...
class TsCreateProgramDriver {
  ...
  updateFiles(contents, updateMode) {
    ...
    for (const [filePath, {newText, originalFile}] of contents.entries()) {
      const sf = ts__default['default'].createSourceFile(
        filePath,
        newText,
        ts__default['default'].ScriptTarget.Latest,
        true,
...
class PotentialTopLevelReadsVisitor extends checker.RecursiveAstVisitor {
  ...
  isTopLevelIdentifierReference(identifier) {
    ...
    if (
      ts__default['default'].isExpressionStatement(parent) ||
    ...
    if (
      ts__default['default'].isPropertyAssignment(parent) ||
    ...
    if (
      ts__default['default'].isClassDeclaration(parent) ||
    ...
    if (
      ts__default['default'].isImportSpecifier(parent) ||
      ts__default['default'].isExportSpecifier(parent)
    ) {
...
function isConstEnumReference(node, reflection) {
  ...
  return (
    ...
    !!declaration.node.modifiers?.some(
      (m) => m.kind === ts__default['default'].SyntaxKind.ConstKeyword,
    )
  );
}
...
function getHmrUpdateDeclaration(compilationResults, constantStatements, meta, sourceFile) {
  ...
  return ts__default['default'].factory.updateFunctionDeclaration(
    node,
    [
...
class ComponentDecoratorHandler {
  ...
  analyze(node, decorator) {
    ...
    const relativeContextFilePath = this.rootDirs.reduce((previous, rootDir) => {
      const candidate = checker.relative(
        checker.absoluteFrom(rootDir),
        checker.absoluteFrom(containingFile),
      );
    ...
    if (component.has('viewProviders')) {
      ...
      viewProvidersRequiringFactory = checker.resolveProvidersRequiringFactory(
        viewProviders,
        this.reflector,
        this.evaluator,
      );
      wrappedViewProviders = new checker.WrappedNodeExpr(
        this.annotateForClosureCompiler
    ...
    if ((rawImports || rawDeferredImports) && !metadata.isStandalone) {
      ...
      diagnostics.push(
        checker.makeDiagnostic(
          checker.ErrorCode.COMPONENT_NOT_STANDALONE,
    ...
    if (this.preanalyzeTemplateCache.has(node)) {
      ...
      if (
        ...
        diagnostics.push(
          ...checker.getTemplateDiagnostics(
            template.errors,
    ...
    const relativeTemplatePath = getProjectRelativePath(
      templateResource.path ??
        ts__default['default'].getOriginalNode(node).getSourceFile().fileName,
      this.rootDirs,
      this.compilerHost,
    );
      ...
      try {
        ...
        if (this.depTracker !== null) {
          this.depTracker.addResourceDependency(
            node.getSourceFile(),
            checker.absoluteFrom(resourceUrl),
          );
        }
      } catch {
        if (this.depTracker !== null) {
    ...
    if (encapsulation === checker.ViewEncapsulation.ShadowDom && metadata.selector !== null) {
      ...
      if (selectorError !== null) {
        ...
        diagnostics.push(
          checker.makeDiagnostic(
            checker.ErrorCode.COMPONENT_INVALID_SHADOW_DOM_SELECTOR,
  ...
  index(context, node, analysis) {
    ...
    if (scope !== null) {
      let {dependencies, isPoisoned} =
        scope.kind === checker.ComponentScopeKind.NgModule ? scope.compilation : scope;
      if (
        (isPoisoned ||
          (scope.kind === checker.ComponentScopeKind.NgModule && scope.exported.isPoisoned)) &&
  ...
  resolve(node, analysis, symbol) {
    ...
    if (nonRemovableImports.length > 0) {
      for (const importDecl of nonRemovableImports) {
        const diagnostic = checker.makeDiagnostic(
          checker.ErrorCode.DEFERRED_DEPENDENCY_IMPORTED_EAGERLY,
          importDecl,
    ...
    if (scope !== null) {
        ...
        switch (dep.kind) {
          case checker.MetaKind.Directive:
            if (
              ...
              dep.matchSource !== checker.MatchSource.Selector
            ) {
              continue;
            }
            const dirType = this.refEmitter.emit(dep.ref, context);
            checker.assertSuccessfulReferenceEmit(
              dirType,
              node.name,
      ...
      const eagerDeclarations = Array.from(declarations.values()).filter(
        (decl) =>
          decl.kind === checker.R3TemplateDependencyKind.NgModule || eagerlyUsed.has(decl.ref.node),
      );
      ...
      if (!cycleDetected) {
        ...
        const declarationIsForwardDeclared = eagerDeclarations.some((decl) =>
          checker.isExpressionForwardReference(decl.type, node.name, context),
        );
        if (
          this.compilationMode !== checker.CompilationMode.LOCAL &&
        ...
        if (this.cycleHandlingStrategy === 0 /* CycleHandlingStrategy.UseRemoteScoping */) {
          ...
          throw new checker.FatalDiagnosticError(
            checker.ErrorCode.IMPORT_CYCLE_DETECTED,
            node,
  ...
  xi18n(ctx, node, analysis) {
    ctx.updateFromTemplate(
      ...
      analysis.template.interpolationConfig ?? checker.DEFAULT_INTERPOLATION_CONFIG,
    );
  }
  ...
  compileFull(node, analysis, resolution, pool) {
    ...
    const fac = compileNgFactoryDefField(
      checker.toFactoryMetadata(meta, checker.FactoryTarget.Component),
    );
  ...
  compileHmrUpdateDeclaration(node, analysis, resolution) {
    ...
    const hmrMeta = this.enableHmr
      ...
      : null;
    const res = checker.compileResults(
      fac,
      def,
      classMetadata,
      'ɵcmp',
  ...
  resolveDeferBlocks(
    ...
    for (const [deferBlock, bound] of deferBlocks) {
      ...
      for (const decl of Array.from(deferrableDecls.values())) {
        ...
        if (
          decl.kind === checker.R3TemplateDependencyKind.Directive &&
      ...
      for (const [block, dependencies] of perBlockDeps) {
        blocks.set(
          ...
          dependencies.length === 0
            ? null
            : checker.compileDeferResolverFunction({mode, dependencies}),
        );
      }
      return {mode, blocks};
    }
...
function getExplicitlyDeferredDeps(scope) {
  return scope.kind === checker.ComponentScopeKind.NgModule ? [] : scope.deferredDependencies;
}
...
function validateNoImportOverlap(eagerDeps, deferredDeps, rawDeferredImports) {
  ...
  for (const deferredDep of deferredDeps) {
    if (eagerDepsSet.has(deferredDep.ref.node)) {
      ...
      diagnostic = checker.makeDiagnostic(
        checker.ErrorCode.DEFERRED_DEPENDENCY_IMPORTED_EAGERLY,
...
class InjectableDecoratorHandler {
    ...
    return {
      analysis: {
        ...
        needsFactory:
          !decorators ||
          decorators.every(
            (current) => !checker.isAngularCore(current) || current.name === 'Injectable',
          ),
      },
    };
  }
  ...
  compileFull(node, analysis) {
    return this.compile(
      ...
      (meta) => checker.compileInjectable(meta, false),
  ...
  compile(compileFactoryFn, compileInjectableFn, compileClassMetadataFn, node, analysis) {
    ...
    if (analysis.needsFactory) {
      ...
      const factoryRes = compileFactoryFn(
        checker.toFactoryMetadata(
          {...meta, deps: analysis.ctorDeps},
          checker.FactoryTarget.Injectable,
        ),
      );
    ...
    if (ɵprov !== undefined && this.errorOnDuplicateProv) {
      throw new checker.FatalDiagnosticError(
        checker.ErrorCode.INJECTABLE_DUPLICATE_PROV,
...
function extractInjectableMetadata(clazz, decorator, reflector) {
  ...
  if (decorator.args === null) {
    throw new checker.FatalDiagnosticError(
      checker.ErrorCode.DECORATOR_NOT_CALLED,
      decorator.node,
  ...
  if (decorator.args.length === 0) {
    return {
      ...
      type,
      typeArgumentCount,
      providedIn: checker.createMayBeForwardRefExpression(
        new checker.LiteralExpr(null),
        0 /* ForwardRefHandling.None */,
      ),
    };
  } else if (decorator.args.length === 1) {
    ...
    const providedIn = meta.has('providedIn')
      ...
      : checker.createMayBeForwardRefExpression(
          new checker.LiteralExpr(null),
          0 /* ForwardRefHandling.None */,
        );
    let deps = undefined;
    if ((meta.has('useClass') || meta.has('useFactory')) && meta.has('deps')) {
      ...
      if (!ts__default['default'].isArrayLiteralExpression(depsExpr)) {
        throw new checker.FatalDiagnosticError(
          checker.ErrorCode.VALUE_NOT_LITERAL,
          depsExpr,
...
function getProviderExpression(expression, reflector) {
  ...
  return checker.createMayBeForwardRefExpression(
    new checker.WrappedNodeExpr(forwardRefValue ?? expression),
  ...
  if (ts__default['default'].isArrayLiteralExpression(dep)) {
    dep.elements.forEach((el) => {
      ...
      if (ts__default['default'].isIdentifier(el)) {
        ...
        ts__default['default'].isNewExpression(el) &&
        ts__default['default'].isIdentifier(el.expression)
      ) {
...
class PipeDecoratorHandler {
  ...
  analyze(clazz, decorator) {
    ...
    if (!pipe.has('name')) {
      throw new checker.FatalDiagnosticError(
        checker.ErrorCode.PIPE_MISSING_NAME,
        meta,
  ...
  compileFull(node, analysis) {
    const fac = compileNgFactoryDefField(
      checker.toFactoryMetadata(analysis.meta, checker.FactoryTarget.Pipe),
    );
  ...
  switch (expr.kind) {
    case ts__default['default'].SyntaxKind.Identifier:
      ...
      metadataProperties.push(
        ts__default['default'].factory.createPropertyAssignment('type', expr),
      );
      break;
    case ts__default['default'].SyntaxKind.CallExpression:
      ...
      const call = expr;
      metadataProperties.push(
        ts__default['default'].factory.createPropertyAssignment('type', call.expression),
      );
      if (call.arguments.length) {
        ...
        const argsArrayLiteral = ts__default['default'].factory.createArrayLiteralExpression(
          ts__default['default'].factory.createNodeArray(args, true),
        );
        metadataProperties.push(
          ts__default['default'].factory.createPropertyAssignment('args', argsArrayLiteral),
        );
      }
      break;
    default:
...
function createCtorParametersClassProperty(
  ...
  for (const ctorParam of ctorParameters) {
    ...
    const members = [
      ts__default['default'].factory.createPropertyAssignment(
        'type',
        paramType || ts__default['default'].factory.createIdentifier('undefined'),
      ),
    ];
    const decorators = [];
    ...
    if (decorators.length) {
      members.push(
        ts__default['default'].factory.createPropertyAssignment(
          'decorators',
          ts__default['default'].factory.createArrayLiteralExpression(decorators),
        ),
      );
    }
    params.push(ts__default['default'].factory.createObjectLiteralExpression(members));
  }
  const initializer = ts__default['default'].factory.createArrowFunction(
    undefined,
    undefined,
    [],
    undefined,
    ts__default['default'].factory.createToken(
      ts__default['default'].SyntaxKind.EqualsGreaterThanToken,
    ),
    ts__default['default'].factory.createArrayLiteralExpression(params, true),
  );
  const ctorProp = ts__default['default'].factory.createPropertyDeclaration(
    [ts__default['default'].factory.createToken(ts__default['default'].SyntaxKind.StaticKeyword)],
    'ctorParameters',
    undefined,
    undefined,
    initializer,
  ...
  switch (kind) {
    ...
    case ts__default['default'].SyntaxKind.UnionType:
      const childTypeNodes = node.types.filter(
        (t) =>
          !(
            ts__default['default'].isLiteralTypeNode(t) &&
            t.literal.kind === ts__default['default'].SyntaxKind.NullKeyword
          ),
      );
...
function symbolIsRuntimeValue(typeChecker, symbol) {
  ...
  return (
    (symbol.flags &
      ts__default['default'].SymbolFlags.Value &
      ts__default['default'].SymbolFlags.ConstEnumExcludes) !==
    0
  );
}
/**
  ...
  function createPropDecoratorsClassProperty(diagnostics, properties) {
    ...
    for (const [name, decorators] of properties.entries()) {
      entries.push(
        ts__default['default'].factory.createPropertyAssignment(
          name,
          ts__default['default'].factory.createArrayLiteralExpression(
    ...
    const prop = ts__default['default'].factory.createPropertyDeclaration(
      [ts__default['default'].factory.createToken(ts__default['default'].SyntaxKind.StaticKeyword)],
      'propDecorators',
      undefined,
      undefined,
  ...
  return (context) => {
    ...
    function entityNameToExpression(name) {
      ...
      if (ts__default['default'].isQualifiedName(name)) {
        ...
        return ts__default['default'].factory.createPropertyAccessExpression(
          containerExpr,
          name.right,
        );
      }
      ...
      if (checker.isAliasImportDeclaration(decl)) {
        ...
        if (decl.name !== undefined) {
          return ts__default['default'].setOriginalNode(
            ts__default['default'].factory.createIdentifier(decl.name.text),
            decl.name,
          );
        }
      }
      ...
      return ts__default['default'].setOriginalNode(
        ts__default['default'].factory.createIdentifier(name.text),
        name,
      );
    }
    /**
    ...
    function transformClassElement(element) {
      ...
      const elementModifiers = ts__default['default'].canHaveModifiers(element)
        ? ts__default['default'].getModifiers(element)
        : undefined;
      let modifiers;
      if (decoratorsToKeep.length || elementModifiers?.length) {
        modifiers = ts__default['default'].setTextRange(
    ...
    function transformConstructor(ctor) {
      ...
      for (const param of oldParameters) {
        ...
        const newParam = ts__default['default'].factory.updateParameterDeclaration(
          param,
          modifiers,
      ...
      const updated = ts__default['default'].factory.updateConstructorDeclaration(
        ctor,
        ts__default['default'].getModifiers(ctor),
        newParameters,
        ctor.body,
      );
    ...
    function transformClassDeclaration(classDecl) {
      ...
      for (const member of classDecl.members) {
        ...
        newMembers.push(
          ts__default['default'].visitEachChild(member, decoratorDownlevelVisitor, context),
        );
      }
      ...
      const members = ts__default['default'].setTextRange(
        ts__default['default'].factory.createNodeArray(
          newMembers,
...
function cloneClassElementWithModifiers(node, modifiers) {
  ...
  if (ts__default['default'].isMethodDeclaration(node)) {
    clone = ts__default['default'].factory.createMethodDeclaration(
      modifiers,
      node.asteriskToken,
      node.name,
      node.questionToken,
    ...
    clone = ts__default['default'].factory.createPropertyDeclaration(
      modifiers,
      node.name,
      node.questionToken,
      node.type,
    ...
    clone = ts__default['default'].factory.createGetAccessorDeclaration(
      modifiers,
      node.name,
      node.parameters,
      node.type,
    ...
    clone = ts__default['default'].factory.createSetAccessorDeclaration(
      modifiers,
      node.name,
      node.parameters,
      node.body,
    ...
    throw new Error(
      `Unsupported decorated member with kind ${ts__default['default'].SyntaxKind[node.kind]}`,
    );
  }
 ...
 */
function createSyntheticAngularCoreDecoratorAccess(
  factory,
  importManager,
  ngClassDecorator,
  sourceFile,
  ...
  return factory.createPropertyAccessExpression(
    ...
    ts__default['default'].setOriginalNode(
...
function castAsAny(factory, expr) {
  return factory.createAsExpression(
    expr,
    factory.createKeywordTypeNode(ts__default['default'].SyntaxKind.AnyKeyword),
  );
}
...
const signalModelTransform = (
  ...
  if (
    host.getDecoratorsOfDeclaration(member.node)?.some((d) => {
      return (
        checker.isAngularDecorator(d, 'Input', isCore) ||
        checker.isAngularDecorator(d, 'Output', isCore)
      );
    })
  ) {
    return member.node;
...
const queryFunctionsTransforms = (
  ...
  const queryDecorators =
    decorators && checker.getAngularDecorators(decorators, queryDecoratorNames, isCore);
...
function getInitializerApiJitTransform(host, importTracker, isCore, shouldTransformClass) {
  return (ctx) => {
    return (sourceFile) => {
      const importManager = new checker.ImportManager();
      sourceFile = ts__default['default'].visitNode(
        sourceFile,
        ...
        ts__default['default'].isSourceFile,
      );
...
function createTransformVisitor(
  ...
  const visitor = (node) => {
    if (ts__default['default'].isClassDeclaration(node) && node.name !== undefined) {
      const originalNode = ts__default['default'].getOriginalNode(
        node,
        ts__default['default'].isClassDeclaration,
      );
      ...
      const angularDecorator = host
        ...
        ?.find((d) =>
          decoratorsWithInputs.some((name) => checker.isAngularDecorator(d, name, isCore)),
        );
      if (
...
function i18nExtract(
  ...
  options,
  bundle,
  pathResolve = p__namespace.resolve,
) {
  ...
  scanImports(sf) {
    return this.perf.inPhase(checker.PerfPhase.CycleDetection, () => {
      ...
      for (const stmt of sf.statements) {
        if (
          (!ts__default['default'].isImportDeclaration(stmt) &&
            !ts__default['default'].isExportDeclaration(stmt)) ||
  ...
  return ts__default['default'].getJSDocTags(escapedNode).map((t) => {
    return {
      ...
      comment: unescapeAngularDecorators(
        ts__default['default'].getTextOfJSDocComment(t.comment) ?? '',
      ),
    };
  });
}
...
function extractJsDocDescription(node) {
  ...
  const description =
    typeof comment === 'string'
      ? comment
      : (ts__default['default'].getTextOfJSDocComment(comment) ?? '');
...
function extractRawJsDoc(node) {
  ...
  const comment =
    ts__default['default']
      ...
      .find(ts__default['default'].isJSDoc)
      ?.getFullText() ?? '';
...
function getEscapedNode(node) {
  ...
  const file = ts__default['default'].createSourceFile(
    'x.ts',
    `${escaped}class X {}`,
    ts__default['default'].ScriptTarget.ES2020,
    true,
  );
...
function extractResolvedTypeString(node, checker) {
  return checker.typeToString(
    ...
    undefined,
    ts__default['default'].TypeFormatFlags.NoTruncation,
  );
}
  ...
  extract() {
    ...
    const returnType = signature
      ? this.typeChecker.typeToString(
          ...
          ts__default['default'].TypeFormatFlags.NoTypeReduction |
            ts__default['default'].TypeFormatFlags.NoTruncation,
        )
      : 'unknown';
...
function findImplementationOfFunction(node, typeChecker) {
  ...
  const implementation = symbol?.declarations?.find(
    (s) => ts__default['default'].isFunctionDeclaration(s) && s.body !== undefined,
  );
...
function hasLeadingInternalComment(member) {
  ...
  return (
    ts__default['default'].reduceEachLeadingCommentRange(
      memberText,
      member.getFullStart(),
...
class ClassExtractor {
  ...
  extractSignature(signature) {
    ...
    const functionExtractor = new FunctionExtractor(
      ts__default['default'].isConstructSignatureDeclaration(signature) ? 'new' : '',
      signature,
      this.typeChecker,
    );
    return {
  ...
  extractGetterSetter(accessor) {
    return {
      ...
      memberType: ts__default['default'].isGetAccessor(accessor)
        ? MemberType.Getter
        : MemberType.Setter,
    };
  }
  ...
  extractInterfaceConformance(declaration) {
    const implementClause = declaration.heritageClauses?.find(
      (clause) => clause.token === ts__default['default'].SyntaxKind.ImplementsKeyword,
    );
  ...
  filterMethodOverloads(declarations) {
    return declarations.filter((declaration, index) => {
      ...
      if (
        ts__default['default'].isFunctionDeclaration(declaration) ||
    ...
    return (
      ...
      (!ts__default['default'].isCallSignatureDeclaration(member) &&
        member.modifiers?.some(
          (mod) => mod.kind === ts__default['default'].SyntaxKind.PrivateKeyword,
        )) ||
  ...
  isPublicConstructorParameterProperty(node) {
    if (
      ts__default['default'].isParameterPropertyDeclaration(node, node.parent) &&
      node.modifiers
    ) {
      return node.modifiers.some(
        (modifier) => modifier.kind === ts__default['default'].SyntaxKind.PublicKeyword,
      );
    }
    return false;
  }
  ...
  isProperty(member) {
    ...
    return (
      ts__default['default'].isPropertyDeclaration(member) ||
      ts__default['default'].isPropertySignature(member) ||
  ...
  isMethod(member) {
    // Classes have declarations, interface have signatures
    return (
      ts__default['default'].isMethodDeclaration(member) ||
      ts__default['default'].isMethodSignature(member)
    );
  }
  /** Gets whether the given signature declaration is documentable. */
  isDocumentableSignature(signature) {
    return (
      ts__default['default'].isConstructSignatureDeclaration(signature) ||
      ts__default['default'].isCallSignatureDeclaration(signature)
    );
  }
  ...
  hasPrivateComputedProperty(property) {
    return (
      ts__default['default'].isComputedPropertyName(property.name) &&
      property.name.expression.getText().startsWith('ɵ')
    );
  }
}
...
function extractLiteralPropertiesAsEnumMembers(declaration) {
  ...
  while (
    initializer &&
    (ts__default['default'].isAsExpression(initializer) ||
      ts__default['default'].isParenthesizedExpression(initializer))
  ) {
    initializer = initializer.expression;
  }
  if (initializer === undefined || !ts__default['default'].isObjectLiteralExpression(initializer)) {
    throw new Error(
      `Declaration tagged with "${LITERAL_AS_ENUM_TAG}" must be initialized to an object literal, but received ${initializer ? ts__default['default'].SyntaxKind[initializer.kind] : 'undefined'}`,
    );
  }
  return initializer.properties.map((prop) => {
    ...
    if (
      !ts__default['default'].isNumericLiteral(prop.initializer) &&
      !ts__default['default'].isStringLiteralLike(prop.initializer)
    ) {
      throw new Error(
...
function isDecoratorOptionsInterface(declaration) {
  return declaration
    ...
    .statements.some(
      (s) =>
        ts__default['default'].isVariableStatement(s) &&
...
function getDecoratorOptions(declaration, typeChecker) {
  ...
  if (ts__default['default'].isTypeAliasDeclaration(optionsDeclaration)) {
    ...
    optionsInterface = (aliasedType.getSymbol()?.getDeclarations() ?? []).find((d) =>
      ts__default['default'].isInterfaceDeclaration(d),
    );
  } else {
...
function getDecoratorJsDocNode(declaration) {
  ...
  const decoratorInterface = declaration.getSourceFile().statements.find((s) => {
    return (
      ts__default['default'].isInterfaceDeclaration(s) && s.name.getText() === `${name}Decorator`
    );
  });
...
function getEnumMemberValue(memberNode) {
  ...
  const literal = memberNode.getChildren().find((n) => {
    return (
      ts__default['default'].isNumericLiteral(n) ||
...
function isInitializerApiFunction(node, typeChecker) {
  ...
  let tagContainer = ts__default['default'].isFunctionDeclaration(node)
    ? node
    : getContainerVariableStatement(node);
...
function extractInitializerApiFunction(node, typeChecker) {
  ...
  const container = ts__default['default'].isFunctionDeclaration(node)
    ? node
    : getContainerVariableStatement(node);
  if (container === null) {
...
class DocsExtractor {
    ...
    for (const [exportName, node] of exportedDeclarations) {
      ...
      if (entry && !isIgnoredDocEntry(entry)) {
        ...
        entry.source = {
          ...
          startLine:
            ts__default['default'].getLineAndCharacterOfPosition(realSourceFile, node.getStart())
              .line + 1,
          endLine:
            ts__default['default'].getLineAndCharacterOfPosition(realSourceFile, node.getEnd())
              .line + 1,
        };
  ...
  getExportedDeclarations(sourceFile) {
    ...
    let exportedDeclarations = Array.from(exportedDeclarationMap?.entries() ?? []).map(
      ([exportName, declaration]) => [exportName, declaration.node],
    );
...
`;
    const genFile = ts__default['default'].createSourceFile(
      this.flatIndexPath,
      contents,
      ts__default['default'].ScriptTarget.ES2015,
      true,
...
function findFlatIndexEntryPoint(rootFiles) {
  ...
  if (tsFiles.length === 1) {
    ...
    for (const tsFile of tsFiles) {
      if (
        checker.getFileSystem().basename(tsFile) === 'index.ts' &&
...
function getIdentifierOfDeclaration(decl) {
  if (
    (ts__default['default'].isClassDeclaration(decl) ||
...
function generateAnalysis(context) {
  ...
  context.components.forEach(({declaration, selector, boundTemplate, templateMeta}) => {
    ...
    const componentFile = new checker.ParseSourceFile(
...
class AdapterResourceLoader {
  ...
  getResolvedCandidateLocations(url, fromFile) {
    const failedLookup = ts__default['default'].resolveModuleName(
      url + RESOURCE_MARKER,
      fromFile,
      this.options,
...
function isSignalSymbol(symbol) {
  ...
  return (
    ...
    declarations.some((decl) => {
      ...
      return (
        (ts__default['default'].isInterfaceDeclaration(decl) ||
          ts__default['default'].isTypeAliasDeclaration(decl)) &&
...
class InvalidBananaInBoxCheck extends TemplateCheckWithVisitor {
  ...
  visitNode(ctx, component, node) {
    if (!(node instanceof checker.BoundEvent)) return [];
...
class MissingControlFlowDirectiveCheck extends TemplateCheckWithVisitor {
  ...
  visitNode(ctx, component, node) {
    if (!(node instanceof checker.Template)) return [];
...
class NullishCoalescingNotNullableCheck extends TemplateCheckWithVisitor {
  ...
  visitNode(ctx, component, node) {
    if (!(node instanceof checker.Binary) || node.operation !== '??') return [];
    ...
    if (
      typeLeft.flags &
      (ts__default['default'].TypeFlags.Any | ts__default['default'].TypeFlags.Unknown)
    ) {
...
class OptionalChainNotNullableCheck extends TemplateCheckWithVisitor {
  ...
  visitNode(ctx, component, node) {
    if (
      !(node instanceof checker.SafeCall) &&
...
class SuffixNotSupportedCheck extends TemplateCheckWithVisitor {
  ...
  visitNode(ctx, component, node) {
    if (!(node instanceof checker.BoundAttribute)) return [];
    if (
...
class TextAttributeNotBindingSpec extends TemplateCheckWithVisitor {
  ...
  visitNode(ctx, component, node) {
    if (!(node instanceof checker.TextAttribute)) return [];
    const name = node.name;
...
class UninvokedFunctionInEventBindingSpec extends TemplateCheckWithVisitor {
  ...
  visitNode(ctx, component, node) {
    ...
    if (
      node.type !== checker.ParsedEventType.Regular &&
      node.type !== checker.ParsedEventType.Animation
    )
      return [];
    if (!(node.handler instanceof checker.ASTWithSource)) return [];
...
function assertExpressionInvoked(expression, component, node, expressionText, ctx) {
  ...
  if (
    !(expression instanceof checker.PropertyRead) &&
    !(expression instanceof checker.SafePropertyRead)
  ) {
...
class NgSkipHydrationSpec extends TemplateCheckWithVisitor {
  ...
  visitNode(ctx, component, node) {
    ...
    if (
      node instanceof checker.TextAttribute &&
...
class ExpressionsSemanticsVisitor extends checker.RecursiveAstVisitor$1 {
  ...
  checkForIllegalWriteInEventBinding(ast, context) {
    if (
      !(context instanceof checker.BoundEvent) ||
  ...
  makeIllegalTemplateVarDiagnostic(target, expressionNode, errorMessage) {
    const span =
      target instanceof checker.Variable
        ? target.valueSpan || target.sourceSpan
        : target.sourceSpan;
    return this.templateTypeChecker.makeTemplateDiagnostic(
      ...
      checker.ngErrorCode(checker.ErrorCode.WRITE_TO_READ_ONLY_VARIABLE),
      errorMessage,
      [
        {
...
class InitializerApiUsageRule {
  ...
  checkNode(node) {
    ...
    while (
      node.parent &&
      (ts__default['default'].isParenthesizedExpression(node.parent) ||
        ts__default['default'].isAsExpression(node.parent))
    ) {
      node = node.parent;
    }
    ...
    const identifiedInitializer = checker.tryParseInitializerApi(
      APIS_TO_CHECK,
      node,
      this.reflector,
    ...
    if (
      ts__default['default'].isPropertyDeclaration(node.parent) &&
      ...
      if (closestClass && ts__default['default'].isClassDeclaration(closestClass)) {
        ...
        return isComponentOrDirective
          ? null
          : checker.makeDiagnostic(
              checker.ErrorCode.UNSUPPORTED_INITIALIZER_API_USAGE,
              node,
...
class UnusedStandaloneImportsRule {
  ...
  checkNode(node) {
    ...
    const propertyAssignment = closestNode(
      metadata.rawImports,
      ts__default['default'].isPropertyAssignment,
    );
    const category =
    ...
    if (unused.length === metadata.imports.length && propertyAssignment !== null) {
      return checker.makeDiagnostic(
        checker.ErrorCode.UNUSED_STANDALONE_IMPORTS,
    ...
    while (current !== null) {
      if (ts__default['default'].isVariableStatement(current)) {
        return !!current.modifiers?.some(
          (m) => m.kind === ts__default['default'].SyntaxKind.ExportKeyword,
        );
      }
...
function coreVersionSupportsFeature(coreVersion, minVersion) {
  // A version of `20.0.0-next.1+sha-639c717` usually means that core is at head so it supports
...
class NgCompiler {
  ...
  constructor(
    ...
    this.entryPoint =
      adapter.entryPoint !== null
        ? checker.getSourceFileOrNull(inputProgram, adapter.entryPoint)
        : null;
    const moduleResolutionCache = ts__default['default'].createModuleResolutionCache(
  ...
  prepareEmit() {
    ...
    if (compilation.supportJitMode && compilation.jitDeclarationRegistry.jitDeclarations.size > 0) {
      ...
      const jitDeclarationOriginalNodes = new Set(
        jitDeclarationsArray.map((d) => ts__default['default'].getOriginalNode(d)),
      );
      const sourceFilesWithJit = new Set(
        jitDeclarationsArray.map((d) => d.getSourceFile().fileName),
      );
      before.push((ctx) => {
        const reflectionHost = new checker.TypeScriptReflectionHost(
        ...
        const jitTransform = angularJitApplicationTransform(
          ...
          (node) => {
            ...
            node = ts__default['default'].getOriginalNode(
              node,
              ts__default['default'].isClassDeclaration,
            );
  ...
  getTypeCheckingConfig() {
    ...
    let allowSignalsInTwoWayBindings =
      coreHasSymbol(this.inputProgram, checker.Identifiers.unwrapWritableSignal) ??
      (this.angularCoreVersion === null ||
    ...
    if (this.fullTemplateTypeCheck) {
      typeCheckingConfig = {
        ...
        controlFlowPreventingContentProjection:
          this.options.extendedDiagnostics?.defaultCategory ||
          exports.DiagnosticCategoryLabel.Warning,
  ...
  getTemplateDiagnostics() {
    ...
    for (const sf of this.inputProgram.getSourceFiles()) {
      ...
      diagnostics.push(
        ...compilation.templateTypeChecker.getDiagnosticsForFile(
          sf,
          checker.OptimizeFor.WholeProgram,
        ),
      );
    }
  ...
  makeCompilation() {
    ...
    const reflector = new checker.TypeScriptReflectionHost(
      checker$1,
      compilationMode === checker.CompilationMode.LOCAL,
    );
    ...
    if (
      ...
      if (
        ...
        // imports.
        localImportStrategy = new checker.LogicalProjectStrategy(
          reflector,
          new checker.LogicalFileSystem([...this.adapter.rootDirs], this.adapter),
        );
      } else {
      ...
      refEmitter = new checker.ReferenceEmitter([
        ...
        new checker.AbsoluteModuleStrategy(
          this.inputProgram,
          checker$1,
          this.moduleResolver,
          reflector,
    ...
    const templateTypeChecker = new checker.TemplateTypeCheckerImpl(
      this.inputProgram,
      notifyingDriver,
      traitCompiler,
      typeCheckingConfig,
...
function isAngularCorePackage(program) {
  ...
  return r3Symbols.statements.some((stmt) => {
    ...
    return stmt.declarationList.declarations.some((decl) => {
      ...
      if (
        !ts__default['default'].isIdentifier(decl.name) ||
        decl.name.text !== 'ITS_JUST_ANGULAR'
      ) {
        return false;
      }
      ...
      if (
        ...
        decl.initializer.kind !== ts__default['default'].SyntaxKind.TrueKeyword
      ) {
        return false;
      }
  ...
  createCachedResolveModuleNamesFunction() {
    ...
    return (moduleNames, containingFile, reusedNames, redirectedReference, options) => {
      return moduleNames.map((moduleName) => {
        const module = ts__default['default'].resolveModuleName(
          moduleName,
          containingFile,
          options,
          this,
...
class NgtscProgram {
  ...
  constructor(rootNames, options, delegateHost, oldProgram) {
    ...
    this.tsProgram = perfRecorder.inPhase(checker.PerfPhase.TypeScriptProgramCreate, () =>
      ts__default['default'].createProgram(this.host.inputFiles, options, this.host, reuseProgram),
    );
  ...
  emit(opts) {
    ...
    if (this.options.tracePerformance !== undefined) {
      ...
      checker
        .getFileSystem()
        .writeFile(
          checker.getFileSystem().resolve(this.options.tracePerformance),
