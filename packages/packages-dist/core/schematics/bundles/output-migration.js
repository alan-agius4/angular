...
function getTargetPropertyDeclaration(targetSymbol) {
  ...
  if (
    valDeclaration !== undefined &&
    ts__default['default'].isPropertyDeclaration(valDeclaration)
  ) {
...
function getOutputDecorator(node, reflector) {
  ...
  const ngDecorators =
    decorators !== null
      ? checker.getAngularDecorators(decorators, ['Output'], /* isCore */ false)
      : [];
...
function calculateDeclarationReplacement(info, node, aliasParam) {
  ...
  const payloadTypes =
    node.initializer !== undefined && ts__default['default'].isNewExpression(node.initializer)
      ? node.initializer?.typeArguments
      : undefined;
  const outputCall = ts__default['default'].factory.createCallExpression(
    ts__default['default'].factory.createIdentifier('output'),
    payloadTypes,
          ...
          ts__default['default'].factory.createObjectLiteralExpression(
            [
              ts__default['default'].factory.createPropertyAssignment(
                'alias',
                ts__default['default'].factory.createStringLiteral(aliasParam, true),
              ),
            ],
            false,
          ),
  ...
  return prepareTextReplacementForNode(
    info,
    node,
    printer.printNode(ts__default['default'].EmitHint.Unspecified, updatedOutputDeclaration, sf),
  );
}
...
function calculatePipeCallReplacement(info, node) {
  if (ts__default['default'].isPropertyAccessExpression(node.expression)) {
    ...
    const toObsCallExp = ts__default['default'].factory.createCallExpression(
      outputToObservableIdent,
      undefined,
      [node.expression.expression],
    );
    const pipePropAccessExp = ts__default['default'].factory.updatePropertyAccessExpression(
      node.expression,
      toObsCallExp,
      node.expression.name,
    );
    const pipeCallExp = ts__default['default'].factory.updateCallExpression(
      node,
      pipePropAccessExp,
      [],
      node.arguments,
    ...
    const replacements = [
      prepareTextReplacementForNode(
        info,
        node,
        printer.printNode(ts__default['default'].EmitHint.Unspecified, pipeCallExp, sf),
      ),
    ];
...
class OutputMigration extends project_paths.TsurgeFunnelMigration {
  ...
  async analyze(info) {
    ...
    const outputMigrationVisitor = (node) => {
      ...
      if (
        isPotentialCompleteCallUsage(node) &&
        ts__default['default'].isPropertyAccessExpression(node.expression)
      ) {
        const propertyDeclaration = isTargetOutputDeclaration(
          node.expression.expression,
    ...
    const {visitor: templateHostRefVisitor} = index.createFindAllSourceFileReferencesVisitor(
      info,
      checker$1,
      reflector,
      resourceLoader,
