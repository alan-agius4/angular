...
function updateImportDeclaration(decl, removeCommonModule) {
  ...
  const updated = ts__default['default'].factory.updateImportDeclaration(
    decl,
    decl.modifiers,
    updatedClause,
...
function updateImportClause(clause, removeCommonModule) {
  if (clause.namedBindings && ts__default['default'].isNamedImports(clause.namedBindings)) {
    ...
    clause = ts__default['default'].factory.updateImportClause(
      clause,
      clause.isTypeOnly,
      clause.name,
      ts__default['default'].factory.createNamedImports(elements),
    );
  }
  return clause;
}
function updateClassImports(propAssignment, removeCommonModule) {
  ...
  const elements = importList.elements.filter(
    (el) => !ts__default['default'].isIdentifier(el) || !removals.includes(el.text),
  );
  if (elements.length === importList.elements.length) {
    // nothing changed
    return null;
  }
  const updatedElements = ts__default['default'].factory.updateArrayLiteralExpression(
    importList,
    elements,
  );
  const updatedAssignment = ts__default['default'].factory.updatePropertyAssignment(
    propAssignment,
    propAssignment.name,
    updatedElements,
  );
...
function analyzeDecorators(node, sourceFile, analyzedFiles) {
  ...
  const decorator = ts__default['default'].getDecorators(node)?.find((dec) => {
    return (
      ts__default['default'].isCallExpression(dec.expression) &&
...
function forEachClass(sourceFile, callback) {
  sourceFile.forEachChild(function walk(node) {
    if (
      ...
      ts__default['default'].isImportDeclaration(node)
    ) {
      callback(node);
    }
