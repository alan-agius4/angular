...
class UnusedImportsMigration extends project_paths.TsurgeFunnelMigration {
  ...
  analyzeUsages(sourceFile, locations) {
    ...
    const walk = (node) => {
      ...
      if (ts__default['default'].isImportDeclaration(node)) {
        ...
        if (
          namedBindings &&
          ts__default['default'].isNamedImports(namedBindings) &&
          moduleName !== null
        ) {
  ...
  generateReplacements(sourceFile, removalLocations, usages, info, replacements) {
    ...
    fullRemovals.forEach((node) => {
      replacements.push(
        new project_paths.Replacement(
          project_paths.projectFile(sourceFile, info),
    ...
    partialRemovals.forEach((toRemove, node) => {
      const newNode = ts__default['default'].factory.updateArrayLiteralExpression(
        node,
