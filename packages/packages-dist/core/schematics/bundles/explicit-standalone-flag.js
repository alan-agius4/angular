...
function migrateFile(sourceFile, rewriteFn) {
  ...
  ts__default['default'].forEachChild(sourceFile, function visit(node) {
    ...
    ts__default['default'].getDecorators(node)?.forEach((decorator) => {
      ...
      if (
        !decoratorArgument ||
        !ts__default['default'].isObjectLiteralExpression(decoratorArgument)
      ) {
        return;
      }
      ...
      if (!standaloneProp) {
        if (!hasImports) {
          const standaloneFalseProperty = ts__default['default'].factory.createPropertyAssignment(
            'standalone',
        ...
        standaloneProp.value === ts__default['default'].SyntaxKind.TrueKeyword &&
        hasImports
      ) {
      ...
      if (newProperties) {
        ...
        const newFirstArg = ts__default['default'].factory.createObjectLiteralExpression(
          newPropsArr,
          true,
        );
...
function decoratorHasImports(decoratorArgument) {
  for (const prop of decoratorArgument.properties) {
    if (
      ...
      if (
        prop.initializer.kind === ts__default['default'].SyntaxKind.ArrayLiteralExpression ||
