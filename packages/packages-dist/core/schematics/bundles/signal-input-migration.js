...
class KnownInputs {
  ...
  markFieldIncompatible(input, incompatibility) {
    ...
    if (
      existingIncompatibility !== null &&
      migrate_ts_type_references.isFieldIncompatibility(existingIncompatibility)
    ) {
      incompatibility = migrate_ts_type_references.pickFieldIncompatibility(
        existingIncompatibility,
        incompatibility,
      );
    }
    this.knownInputIds
...
function extractDtsInput(node, metadataReader) {
  if (
    !index.isInputContainerNode(node) ||
...
function parseTransformOfInput(evaluatedInputOpts, node, reflector) {
  ...
  try {
    return program.parseDecoratorInputTransformFunction(
      node.parent,
      node.name.text,
      transformValue,
      reflector,
      noopRefEmitter,
      checker.CompilationMode.FULL,
    );
  } catch (e) {
...
function prepareAndCheckForConversion(node, metadata, checker, options) {
  ...
  let isUndefinedInitialValue =
    ...
    (ts__default['default'].isIdentifier(node.initializer) &&
  ...
  if (node.questionToken !== undefined) {
    ...
    if (typeToAdd === undefined) {
      return {
        context: node,
        reason:
          migrate_ts_type_references.FieldIncompatibilityReason
  ...
  if (
    ...
    initialValue = ts__default['default'].factory.createNonNullExpression(
      ts__default['default'].factory.createIdentifier('undefined'),
    );
  }
...
function inferImportableTypeForInput(checker, node, initialValue) {
  ...
  if (
    ...
    return (
      checker.typeToTypeNode(
        propertyType,
        node,
        ts__default['default'].NodeBuilderFlags.NoTypeReduction,
      ) ?? null
    );
  }
  ...
  if (ts__default['default'].isIdentifier(initialValue)) {
    ...
    ts__default['default'].isPropertyAccessExpression(initialValue) &&
    ...
    return ts__default['default'].factory.createTypeQueryNode(
      ts__default['default'].factory.createQualifiedName(
        initialValue.name,
...
function isPrimitiveImportableTypeNode(type) {
  return !!(
    type.flags & ts__default['default'].TypeFlags.BooleanLike ||
...
function pass1__IdentifySourceFileAndDeclarationInputs(
  ...
  const visitor = (node) => {
    ...
    if (decoratorInput !== null) {
      assert__default['default'](
        index.isInputContainerNode(node),
        'Expected input to be declared on accessor or property.',
      );
    ...
    let importName = null;
    if (
      ts__default['default'].isImportSpecifier(node) &&
...
function pass3__checkIncompatiblePatterns(
  ...
  migrate_ts_type_references.checkIncompatiblePatterns(
    inheritanceGraph,
    checker$1,
    groupedTsAstVisitor,
    knownInputs,
  ...
  for (const input of knownInputs.knownInputIds.values()) {
    const hostBindingDecorators = checker.getAngularDecorators(
    ...
    if (hostBindingDecorators.length > 0) {
      knownInputs.markFieldIncompatible(input.descriptor, {
        ...
        reason:
          migrate_ts_type_references.FieldIncompatibilityReason.SignalIncompatibleWithHostBinding,
      });
    }
  }
}
...
function pass2_IdentifySourceFileReferences(
  ...
  groupedTsAstVisitor.register(
    index.createFindAllSourceFileReferencesVisitor(
      programInfo,
      checker,
      reflector,
      resourceLoader,
...
function executeAnalysisPhase(
  ...
  const inheritanceGraph = new migrate_ts_type_references.InheritanceGraph(
    typeChecker,
  ).expensivePopulate(sourceFiles);
  const pass2And3SourceFileVisitor = new migrate_ts_type_references.GroupedTsAstVisitor(
    sourceFiles,
  );
...
function pass4__checkInheritanceOfInputs(inheritanceGraph, metaRegistry, knownInputs) {
  migrate_ts_type_references.checkInheritanceOfKnownFields(
    inheritanceGraph,
    metaRegistry,
    knownInputs,
    {
      ...
      getFieldsForClass: (clazz) => {
        ...
        assert__default['default'](
          ...
          'Expected directive info to exist for input.',
        );
    ...
    for (const [key, info] of Object.entries(file.knownInputs)) {
      ...
      if (info.memberIncompatibility !== null) {
        if (existing.memberIncompatibility === null) {
          ...
          existing.memberIncompatibility = migrate_ts_type_references.pickFieldIncompatibility(
  ...
  for (const node of topologicalSort(inheritanceGraph).reverse()) {
    ...
    for (const parent of node.outgoing) {
      ...
      if (isNodeIncompatible(parent.data)) {
        node.data.info.memberIncompatibility = migrate_ts_type_references.pickFieldIncompatibility(
          {
    ...
    if (!info.seenAsSourceInput) {
      const existingMemberIncompatibility =
        info.memberIncompatibility !== null
          ? {reason: info.memberIncompatibility, context: null}
          : null;
      info.memberIncompatibility = migrate_ts_type_references.pickFieldIncompatibility(
        {
          reason: migrate_ts_type_references.FieldIncompatibilityReason.OutsideOfMigrationScope,
          context: null,
        },
        existingMemberIncompatibility,
      ).reason;
...
function convertToSignalInput(
    ...
    if (metadata.bindingPropertyName !== metadata.classPropertyName) {
      properties.push(
        ts__default['default'].factory.createPropertyAssignment(
          'alias',
          ts__default['default'].factory.createStringLiteral(metadata.bindingPropertyName),
        ),
      );
    }
  ...
  if (initialValue === undefined) {
    ...
    if (preferShorthandIfPossible === null) {
      ...
      if (ts__default['default'].isUnionTypeNode(resolvedType)) {
        resolvedType = migrate_ts_type_references.removeFromUnionIfPossible(
          resolvedType,
          (t) => t.kind !== ts__default['default'].SyntaxKind.UndefinedKeyword,
        );
      }
    }
  }
  ...
  if (resolvedType !== undefined) {
    ...
    if (metadata.transform !== null) {
      ...
      typeArguments.push(
        ts__default['default'].setSyntheticTrailingComments(
          metadata.transform.type.node,
          undefined,
        ),
      );
    }
  ...
  const inputInitializerFn = metadata.required
    ...
    : inputFnRef;
  const inputInitializer = ts__default['default'].factory.createCallExpression(
    inputInitializerFn,
    typeArguments,
    inputArgs,
  );
  ...
  if (
    !modifiersWithoutInputDecorator?.some(
      (s) => s.kind === ts__default['default'].SyntaxKind.ReadonlyKeyword,
    )
  ) {
    modifiersWithoutInputDecorator.push(
      ts__default['default'].factory.createModifier(
        ts__default['default'].SyntaxKind.ReadonlyKeyword,
      ),
    );
  }
  const newNode = ts__default['default'].factory.createPropertyDeclaration(
  ...
  if (leadingTodoText !== null) {
    replacements.push(
      migrate_ts_type_references.insertPrecedingLine(
        node,
        info,
        '// TODO: Notes from signal input migration:',
      ),
      ...migrate_ts_type_references
        .cutStringToLineLimit(leadingTodoText, 70)
        .map((line) => migrate_ts_type_references.insertPrecedingLine(node, info, `//  ${line}`)),
    );
  }
  replacements.push(
...
function extractTransformOfInput(transform, resolvedType, checker) {
  assert__default['default'](
    ts__default['default'].isExpression(transform.node),
    `Expected transform to be an expression.`,
  );
  ...
  if (resolvedType !== undefined && !ts__default['default'].isSyntheticExpression(resolvedType)) {
    ...
    assert__default['default'](
      ...
      'Expected transform to be an invoke-able.',
    );
    if (
      !checker.isTypeAssignableTo(
      ...
      transformFn = ts__default['default'].factory.createAsExpression(
        ts__default['default'].factory.createParenthesizedExpression(transformFn),
        ts__default['default'].factory.createKeywordTypeNode(
...
function pass6__migrateInputDeclarations(host, checker, result, knownInputs, importManager, info) {
  ...
  for (const [input, metadata] of result.sourceInputs) {
    ...
    if (inputInfo.isIncompatible()) {
      ...
      if (incompatibilityReason !== null && host.config.insertTodosForSkippedFields) {
        result.replacements.push(
          ...migrate_ts_type_references.insertTodoForIncompatibility(
            input.node,
            info,
    ...
    assert__default['default'](
      metadata !== null,
      `Expected metadata to exist for input isn't marked incompatible.`,
    );
    assert__default['default'](
      !ts__default['default'].isAccessor(input.node),
      'Accessor inputs are incompatible.',
    );
...
function pass10_applyImportManager(importManager, result, sourceFiles, info) {
  apply_import_manager.applyImportManagerChanges(
    importManager,
    result.replacements,
    sourceFiles,
    info,
...
function pass8__migrateHostBindings(host, references, info) {
  ...
  for (const reference of references) {
    ...
    host.replacements.push(
      new project_paths.Replacement(
        project_paths.projectFile(bindingField.getSourceFile(), info),
        new project_paths.TextUpdate({position: readEndPos, end: readEndPos, toInsert: appendText}),
      ),
    );
  }
}
    ...
    for (const [id, input] of Object.entries(globalMetadata.knownInputs)) {
      ...
      const isConsideredSourceInput =
        ...
        input.memberIncompatibility !==
          migrate_ts_type_references.FieldIncompatibilityReason.OutsideOfMigrationScope &&
        input.memberIncompatibility !==
          migrate_ts_type_references.FieldIncompatibilityReason.SkippedViaConfigFilter;
      ...
      if (input.memberIncompatibility !== null) {
        const reasonName =
          migrate_ts_type_references.FieldIncompatibilityReason[input.memberIncompatibility];
      ...
      if (input.owningClassIncompatibility !== null) {
        const reasonName =
          migrate_ts_type_references.ClassIncompatibilityReason[input.owningClassIncompatibility];
