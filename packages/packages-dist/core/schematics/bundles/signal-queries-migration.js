...
function computeReplacementsToMigrateQuery(
  ...
  if (metadata.queryInfo.read !== null) {
    ...
    optionProperties.push(
      ts__default['default'].factory.createPropertyAssignment('read', metadata.queryInfo.read.node),
    );
  }
  if (metadata.queryInfo.descendants !== defaultDescendants) {
    optionProperties.push(
      ts__default['default'].factory.createPropertyAssignment(
        'descendants',
        metadata.queryInfo.descendants
          ? ts__default['default'].factory.createTrue()
          : ts__default['default'].factory.createFalse(),
      ),
    );
  }
  ...
  if (isRequired && metadata.queryInfo.first) {
    ...
    newQueryFn = ts__default['default'].factory.createPropertyAccessExpression(
      newQueryFn,
      'required',
    );
  }
  ...
  if (!isRequired && type !== undefined && ts__default['default'].isUnionTypeNode(type)) {
    type = migrate_ts_type_references.removeFromUnionIfPossible(
      type,
      (v) => v.kind !== ts__default['default'].SyntaxKind.UndefinedKeyword,
    );
  }
  ...
  const updated = ts__default['default'].factory.createPropertyDeclaration(
    [
...
function getUniqueIDForClassProperty(property, info) {
  if (
    !ts__default['default'].isClassDeclaration(property.parent) ||
    property.parent.name === undefined
  ) {
    return null;
  }
  ...
  const id = project_paths
    .projectFile(property.getSourceFile(), info)
    .id.replace(/\.d\.ts$/, '.ts');
...
function extractSourceQueryDefinition(node, reflector, evaluator, info) {
  if (
    (!ts__default['default'].isPropertyDeclaration(node) &&
      !ts__default['default'].isAccessor(node)) ||
  ...
  const ngDecorators = checker.getAngularDecorators(
    decorators,
    program.queryDecoratorNames,
    /* isCore */ false,
  );
  ...
  let queryInfo = null;
  try {
    queryInfo = program.extractDecoratorQueryMetadata(
      node,
      decorator.name,
      decorator.args ?? [],
      node.name.text,
...
function markFieldIncompatibleInMetadata(data, id, reason) {
  ...
  if (existing === undefined) {
    ...
    existing.fieldReason = migrate_ts_type_references.pickFieldIncompatibility(
      {reason, context: null},
...
class KnownQueries {
  ...
  getAllClassesWithQueries() {
    return Array.from(this.classToQueryFields.keys()).filter((c) =>
      ts__default['default'].isClassDeclaration(c),
    );
  }
  ...
  getIncompatibilityTextForField(field) {
    ...
    if (incompatibilityInfo.fieldReason !== null) {
      return migrate_ts_type_references.getMessageForFieldIncompatibility(
        incompatibilityInfo.fieldReason,
        {
          single: 'query',
          plural: 'queries',
        },
    ...
    if (incompatibilityInfo.classReason !== null) {
      return migrate_ts_type_references.getMessageForClassIncompatibility(
        incompatibilityInfo.classReason,
        {
          single: 'query',
          plural: 'queries',
        },
...
function checkTsReferenceAccessesField(ref, fieldName) {
  ...
  if (
    !ts__default['default'].isPropertyAccessExpression(accessNode.parent) ||
...
function checkTsReferenceCallsField(ref, fieldName) {
  ...
  if (
    ts__default['default'].isCallExpression(propertyAccess.parent) &&
...
function removeQueryListToArrayCall(ref, info, globalMetadata, knownQueries, replacements) {
  if (
    !index.isHostBindingReference(ref) &&
    !index.isTemplateReference(ref) &&
    !index.isTsReference(ref)
  ) {
    return;
  }
  ...
  if (index.isTsReference(ref)) {
    ...
    replacements.push(
      new project_paths.Replacement(
        project_paths.projectFile(toArrayExpr.getSourceFile(), info),
        new project_paths.TextUpdate({
...
function replaceQueryListGetCall(ref, info, globalMetadata, knownQueries, replacements) {
  ...
  if (index.isTsReference(ref)) {
    ...
    replacements.push(
      new project_paths.Replacement(
        project_paths.projectFile(getExpr.getSourceFile(), info),
        new project_paths.TextUpdate({
...
function replaceQueryListFirstAndLastReferences(
  ...
  if (index.isTsReference(ref)) {
    ...
    replacements.push(
      new project_paths.Replacement(
        project_paths.projectFile(expr.getSourceFile(), info),
        new project_paths.TextUpdate({
...
class SignalQueriesMigration extends project_paths.TsurgeComplexMigration {
  ...
  async analyze(info) {
      ...
      if (extractedQuery !== null) {
        ...
        if (ts__default['default'].isAccessor(queryNode)) {
          markFieldIncompatibleInMetadata(
            ...
            extractedQuery.id,
            migrate_ts_type_references.FieldIncompatibilityReason.Accessor,
          );
        }
    ...
    const inheritanceGraph = new migrate_ts_type_references.InheritanceGraph(
      checker$1,
    ).expensivePopulate(info.sourceFiles);
    ...
    for (const ref of referenceResult.references) {
      if (index.isTsReference(ref) && ref.from.isWrite) {
        markFieldIncompatibleInMetadata(
          res.potentialProblematicQueries,
          ref.target.key,
          migrate_ts_type_references.FieldIncompatibilityReason.WriteAssignment,
        );
      }
      if (
        (index.isTemplateReference(ref) || index.isHostBindingReference(ref)) &&
        ref.from.isWrite
      ) {
        markFieldIncompatibleInMetadata(
          res.potentialProblematicQueries,
    ...
    for (const unit of [unitA, unitB]) {
      ...
      for (const [id, info] of Object.entries(unit.potentialProblematicQueries)) {
        ...
        if (info.classReason !== null) {
          ...
          combined.potentialProblematicQueries[id].classReason = info.classReason;
        }
      }
  ...
  async migrate(globalMetadata, info) {
    ...
    const queryWholeProgramVisitor = (node) => {
      ...
      if (
        ...
        node.text === 'QueryList' &&
        ts__default['default'].findAncestor(node, ts__default['default'].isImportDeclaration) ===
          undefined
      ) {
    ...
    migrate_ts_type_references.migrateTypeScriptReferences(
    ...
    migrate_ts_type_references.migrateTypeScriptTypeReferences(
      referenceMigrationHost,
      referenceResult.references,
      importManager,
      info,
