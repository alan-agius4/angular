...
function unwrapExpression(node) {
  if (
    ts__default['default'].isParenthesizedExpression(node) ||
    ts__default['default'].isAsExpression(node)
  ) {
  ...
  visitClassDeclaration(node) {
    ...
    metadata.node.properties.forEach((property) => {
      ...
      if (
        ...
        ts__default['default'].isStringLiteralLike(property.initializer)
      ) {
        ...
        this.resolvedTemplates.push({
          ...
          getCharacterAndLineOfPosition: (pos) =>
            ts__default['default'].getLineAndCharacterOfPosition(sourceFile, pos + start),
        });
      }
      if (
