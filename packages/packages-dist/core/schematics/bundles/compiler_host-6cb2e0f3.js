...
class ChangeTracker {
  ...
  replaceNode(
    oldNode,
    newNode,
    emitHint = ts__default['default'].EmitHint.Unspecified,
    sourceFileWhenPrinting,
  ) {
  ...
  _getQuoteKind(sourceFile) {
    ...
    for (const statement of sourceFile.statements) {
      if (
        ...
        ts__default['default'].isStringLiteral(statement.moduleSpecifier)
      ) {
        kind =
          statement.moduleSpecifier.getText()[0] === '"'
            ? 1 /* QuoteKind.DOUBLE */
            : 0 /* QuoteKind.SINGLE */;
  ...
  _recordImports() {
    ...
    for (const [sourceFile] of this._changes) {
      ...
      for (const decl of importsToAdd) {
        importLines.push(
          this._printer.printNode(ts__default['default'].EmitHint.Unspecified, decl, sourceFile),
        );
      }
      this.insertText(
        sourceFile,
...
function parseTsconfigFile(tsconfigPath, basePath) {
  const {config} = ts__default['default'].readConfigFile(
    tsconfigPath,
    ts__default['default'].sys.readFile,
  );
