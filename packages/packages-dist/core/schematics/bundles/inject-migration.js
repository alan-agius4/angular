...
function analyzeFile(sourceFile, localTypeChecker, options) {
  ...
  sourceFile.forEachChild(function walk(node) {
    ...
    if (ts__default['default'].isParameter(node)) {
      const closestConstructor = nodes.closestNode(
        node,
        ts__default['default'].isConstructorDeclaration,
      );
    ...
    if (ts__default['default'].isIdentifier(node) && importsToSpecifiers.size > 0) {
      ...
      const isAbstract = !!node.modifiers?.some(
        (m) => m.kind === ts__default['default'].SyntaxKind.AbstractKeyword,
      );
      ...
      const allParamsInjectable = !!constructorNode?.parameters.every((param) => {
        ...
        return ng_decorators
          .getAngularDecorators(localTypeChecker, ts__default['default'].getDecorators(param) || [])
          .some((dec) => dec.name === 'Inject' || dec.name === 'Attribute');
      });
    ...
    localTypeChecker.getSymbolAtLocation(node)?.declarations?.forEach((decl) => {
      ...
      if (ts__default['default'].isShorthandPropertyAssignment(decl)) {
        ...
        if (
          ...
          ts__default['default'].isParameter(symbol.valueDeclaration)
        ) {
...
function parameterDeclaresProperty(node) {
  return !!node.modifiers?.some(
    ({kind}) =>
      kind === ts__default['default'].SyntaxKind.PublicKeyword ||
...
function isNullableType(node) {
  ...
  if (
    node.kind === ts__default['default'].SyntaxKind.UndefinedKeyword ||
    node.kind === ts__default['default'].SyntaxKind.VoidKeyword
  ) {
    return true;
  }
...
function isAccessedViaThis(node) {
  return (
    ts__default['default'].isPropertyAccessExpression(node.parent) &&
...
function findSuperCall(root) {
  ...
  root.forEachChild(function find(node) {
    if (
      ...
      node.expression.kind === ts__default['default'].SyntaxKind.SuperKeyword
    ) {
      result = node;
    } else if (result === null) {
...
function shouldCombineInInitializationOrder(toCombine, constructor) {
  ...
  toCombine.forEach(({declaration: {name}}) => {
    if (
      ts__default['default'].isStringLiteralLike(name) ||
      ts__default['default'].isIdentifier(name)
    ) {
  ...
  const walkInitializer = (node) => {
    if (
      ...
      node.expression.kind === ts__default['default'].SyntaxKind.ThisKeyword
    ) {
...
function getMemberInitializers(constructor) {
  ...
  for (const node of constructor.body.statements) {
    ...
    if (
      !ts__default['default'].isExpressionStatement(node) ||
...
function isInsideInlineFunction(startNode, boundary) {
  ...
  while (current) {
    ...
    if (
      ts__default['default'].isFunctionDeclaration(current) ||
    ...
    const prop = ts__default['default'].factory.createPropertyDeclaration(
      cloneModifiers(
        node.modifiers?.filter((modifier) => {
          ...
          return (
            !ts__default['default'].isDecorator(modifier) &&
            modifier.kind !== ts__default['default'].SyntaxKind.PublicKeyword
          );
        }),
      ),
      name,
      ...
      node.modifiers?.some(
        (modifier) => modifier.kind === ts__default['default'].SyntaxKind.PrivateKeyword,
      )
        ? undefined
        : node.questionToken,
      canInitialize ? undefined : node.type,
      canInitialize ? ts__default['default'].factory.createIdentifier(PLACEHOLDER) : undefined,
    );
    propsToAdd.push(
      memberIndentation +
...
function createInjectReplacementCall(param, options, localTypeChecker, printer, tracker) {
  ...
  const decorators = ng_decorators.getAngularDecorators(
    localTypeChecker,
    ts__default['default'].getDecorators(param) || [],
  );
  const literalProps = [];
  ...
  if (type) {
    ...
    if (
      ...
      injectedType = (
        type.types.find((t) => !ts__default['default'].isLiteralTypeNode(t)) || type.types[0]
      ).getText();
    } else {
    ...
    switch (decorator.name) {
      ...
      case 'Attribute':
        if (firstArg) {
          ...
          const expression = ts__default['default'].factory.createNewExpression(
            constructorRef,
            undefined,
            [firstArg],
          );
  ...
  let expression = ts__default['default'].factory.createCallExpression(
    injectRef,
    typeArguments,
    args,
  );
  ...
  if (param.initializer) {
    expression = ts__default['default'].factory.createBinaryExpression(
      expression,
...
function migrateInjectDecorator(firstArg, type, localTypeChecker) {
  ...
  if (ts__default['default'].isStringLiteralLike(firstArg)) {
    ...
    ts__default['default'].isCallExpression(firstArg) &&
    ...
    if (
      ...
      if (ts__default['default'].isBlock(arrowFn.body)) {
        const returnStatement = arrowFn.body.statements.find((stmt) =>
          ts__default['default'].isReturnStatement(stmt),
        );
    ...
    (ts__default['default'].isTypeReferenceNode(type) ||
      ...
      (ts__default['default'].isUnionTypeNode(type) &&
        type.types.some(ts__default['default'].isTypeReferenceNode)))
  ) {
    typeArguments = [type];
  }
...
function cloneName(node) {
  switch (node.kind) {
    ...
    case ts__default['default'].SyntaxKind.StringLiteral:
      return ts__default['default'].factory.createStringLiteral(
        node.text,
        node.getText()[0] === `'`,
      );
    case ts__default['default'].SyntaxKind.NoSubstitutionTemplateLiteral:
      return ts__default['default'].factory.createNoSubstitutionTemplateLiteral(
        node.text,
        node.rawText,
      );
...
function applyInternalOnlyChanges(
  ...
  result.toCombine.forEach(({declaration, initializer}) => {
    ...
    const newProperty = ts__default['default'].factory.createPropertyDeclaration(
  ...
  result.toHoist.forEach((decl) => {
    prependToClass.push(
      memberIndentation +
        printer.printNode(ts__default['default'].EmitHint.Unspecified, decl, decl.getSourceFile()),
    );
