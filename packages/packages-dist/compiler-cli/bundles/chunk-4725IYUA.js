...
import {PerfCheckpoint, PerfEvent, PerfPhase} from './chunk-Q2WE7ECN.js';
import {
  absoluteFrom,
...
import {__publicField} from './chunk-KPQ72R34.js';
...
function isAbstractClassDeclaration(clazz) {
  return ts.canHaveModifiers(clazz) && clazz.modifiers !== void 0
...
var StaticInterpreter = class {
    ...
    if (lhs instanceof Map) {
      ...
      if (this.host.isClass(ref)) {
        ...
        const member = this.host
          ...
          .find((member2) => member2.isStatic && member2.name === strIndex);
        if (member !== void 0) {
  ...
  visitBindingElement(node, context) {
    ...
    while (
      ts2.isBindingElement(closestDeclaration) ||
      ts2.isArrayBindingPattern(closestDeclaration) ||
      ts2.isObjectBindingPattern(closestDeclaration)
    ) {
    ...
    if (
      !ts2.isVariableDeclaration(closestDeclaration) ||
      closestDeclaration.initializer === void 0
    ) {
      return DynamicValue.fromUnknown(node);
    }
...
function isFunctionOrMethodReference(ref) {
  return (
    ts2.isFunctionDeclaration(ref.node) ||
    ts2.isMethodDeclaration(ref.node) ||
    ts2.isFunctionExpression(ref.node)
  );
}
...
function isVariableDeclarationDeclared(node) {
  ...
  return (
    modifiers !== void 0 && modifiers.some((mod) => mod.kind === ts2.SyntaxKind.DeclareKeyword)
  );
}
var EMPTY = {};
        ...
        if (name === 'Inject') {
          ...
          if (ts4.isStringLiteralLike(attributeName)) {
            ...
            attributeNameType = new WrappedNodeExpr2(
              ts4.factory.createKeywordTypeNode(ts4.SyntaxKind.UnknownKeyword),
            );
          }
        } else {
          throw new FatalDiagnosticError(
...
function readMapType(type, valueTransform) {
  ...
  type.members.forEach((member) => {
    if (
      !ts5.isPropertySignature(member) ||
      member.type === void 0 ||
      member.name === void 0 ||
      (!ts5.isStringLiteral(member.name) && !ts5.isIdentifier(member.name))
    ) {
      return;
    }
...
function extractDirectiveTypeCheckMeta(node, inputs, reflector) {
  ...
  const coercedInputFields = new Set(
    staticMembers.map(extractCoercedInput).filter((inputName) => {
      ...
      if (
        inputName === null ||
        ((_a = inputs.getByClassPropertyName(inputName)) == null ? void 0 : _a.isSignal)
      ) {
        return false;
      }
      return true;
...
function isRestricted(node) {
  ...
  return (
    ...
    modifiers.some(({kind}) => {
      return (
        kind === ts5.SyntaxKind.PrivateKeyword ||
        kind === ts5.SyntaxKind.ProtectedKeyword ||
        kind === ts5.SyntaxKind.ReadonlyKeyword
      );
    })
  );
}
function extractTemplateGuard(member) {
  ...
  if (member.kind === ClassMemberKind.Property) {
    ...
    if (
      member.type !== null &&
      ts5.isLiteralTypeNode(member.type) &&
      ts5.isStringLiteral(member.type.literal)
    ) {
...
function hasInjectableFields(clazz, host) {
  ...
  return members.some(
    ({isStatic, name}) => isStatic && (name === '\u0275prov' || name === '\u0275fac'),
  );
}
...
var DtsMetadataReader = class {
  ...
  getNgModuleMetadata(ref) {
    ...
    if (ngModuleDef === void 0) {
      ...
      !ts6.isTypeReferenceNode(ngModuleDef.type) ||
      ngModuleDef.type.typeArguments === void 0 ||
  ...
  getDirectiveMetadata(ref) {
    ...
    if (def === void 0) {
      ...
      def.type === null ||
      !ts6.isTypeReferenceNode(def.type) ||
      def.type.typeArguments === void 0 ||
    ...
    const isStructural =
      ...
      ctorParams.some((param) => {
        return (
          param.typeValueReference.kind === 1 &&
    ...
    const isStandalone =
      ...
      ((_a = readBooleanType(def.type.typeArguments[7])) != null ? _a : false);
    ...
    const isSignal =
      ...
      ((_b = readBooleanType(def.type.typeArguments[9])) != null ? _b : false);
    const isPoisoned =
      hostDirectives !== null && (hostDirectives == null ? void 0 : hostDirectives.isIncomplete);
    return {
      ...
      outputs,
      hostDirectives:
        (_c = hostDirectives == null ? void 0 : hostDirectives.result) != null ? _c : null,
  ...
  getPipeMetadata(ref) {
    ...
    const isStandalone =
      def.type.typeArguments.length > 2 &&
      ((_a = readBooleanType(def.type.typeArguments[2])) != null ? _a : false);
    return {
      kind: MetaKind.Pipe,
      ref,
      name,
...
function readInputsType(type) {
  ...
  if (ts6.isTypeLiteralNode(type)) {
    for (const member of type.members) {
      if (
        !ts6.isPropertySignature(member) ||
        member.type === void 0 ||
        member.name === void 0 ||
        (!ts6.isStringLiteral(member.name) && !ts6.isIdentifier(member.name))
      ) {
        continue;
      }
  ...
  mayExportProviders(ref, dependencyCallback) {
    ...
    try {
      ...
      if (dirMeta !== null) {
        ...
        return ((_a = dirMeta.imports) != null ? _a : []).some((importRef) =>
...
var TraitCompiler = class {
  ...
  analyzeTrait(clazz, trait) {
    ...
    const symbol = this.makeSymbolForTrait(
      trait.handler,
      clazz,
      (_a = result.analysis) != null ? _a : null,
    );
    if (result.analysis !== void 0 && trait.handler.register !== void 0) {
      trait.handler.register(clazz, result.analysis);
    }
    trait = trait.toAnalyzed(
      (_b = result.analysis) != null ? _b : null,
      (_c = result.diagnostics) != null ? _c : null,
      symbol,
    );
  }
  resolve() {
    ...
    for (const clazz of classes) {
      ...
      for (let trait of record.traits) {
        ...
        trait = trait.toResolved(
          (_a = result.data) != null ? _a : null,
          (_b = result.diagnostics) != null ? _b : null,
        );
    ...
    for (const trait of record.traits) {
      ...
      if (this.compilationMode === CompilationMode.LOCAL) {
        ...
        if (
          ...
          trait.handler.compilePartial !== void 0
        ) {
  ...
  compileHmrUpdateCallback(clazz) {
    ...
    for (const trait of record.traits) {
      if (
        trait.state === TraitState.Resolved &&
        trait.handler.compileHmrUpdateDeclaration !== void 0 &&
        !containsErrors(trait.analysisDiagnostics) &&
        !containsErrors(trait.resolveDiagnostics)
      ) {
...
function containsErrors(diagnostics) {
  return (
    diagnostics !== null &&
    diagnostics.some((diag) => diag.category === ts8.DiagnosticCategory.Error)
  );
}
...
var IvyDeclarationDtsTransform = class {
  ...
  transformClass(clazz, members, reflector, refEmitter, imports) {
    ...
    const newMembers = fields.map((decl) => {
      ...
      return ts9.factory.createPropertyDeclaration(modifiers, decl.name, void 0, typeRef, void 0);
    });
...
var Visitor = class {
  ...
  _maybeProcessStatements(node) {
    ...
    const statementsArray = ts10.factory.createNodeArray(
      newStatements,
    ...
    if (ts10.isBlock(node)) {
      ...
      return ts10.factory.updateSourceFile(
        node,
        statementsArray,
...
var IvyTransformationVisitor = class extends Visitor {
  ...
  visitClassDeclaration(node) {
    ...
    for (const field of this.classCompilationMap.get(node)) {
      ...
      const property = ts11.factory.createPropertyDeclaration(
        [ts11.factory.createToken(ts11.SyntaxKind.StaticKeyword)],
        field.name,
        void 0,
        void 0,
        exprNode,
    ...
    const filteredDecorators = maybeFilterDecorator(
      ts11.getDecorators(node),
      this.compilation.decoratorsFor(node),
    );
    const nodeModifiers = ts11.getModifiers(node);
    let updatedModifiers;
    if (
      (filteredDecorators == null ? void 0 : filteredDecorators.length) ||
      (nodeModifiers == null ? void 0 : nodeModifiers.length)
    ) {
  ...
  _stripAngularDecorators(node) {
    ...
    if (ts11.isParameter(node)) {
      ...
      node = ts11.factory.updateMethodDeclaration(
        node,
        combinedModifiers,
        node.asteriskToken,
        node.name,
      ...
      node = ts11.factory.updatePropertyDeclaration(
        node,
        combinedModifiers,
        node.name,
        node.questionToken,
      ...
      node = ts11.factory.updateGetAccessorDeclaration(
        node,
        combinedModifiers,
        node.name,
        node.parameters,
      ...
      node = ts11.factory.updateSetAccessorDeclaration(
        node,
        combinedModifiers,
        node.name,
        node.parameters,
...
function insertFileOverviewComment(sf, fileoverview) {
  ...
  if (sf.statements.length > 0 && host !== sf.statements[0]) {
    ...
    return ts11.factory.updateSourceFile(
      sf,
...
function maybeFilterDecorator(decorators, toRemove) {
  ...
  const filtered = decorators.filter(
    (dec) => toRemove.find((decToRemove) => ts11.getOriginalNode(dec) === decToRemove) === void 0,
  );
  if (filtered.length === 0) {
    return void 0;
  }
...
function assertLocalCompilationUnresolvedConst(
  ...
  if (
    ...
    throw new FatalDiagnosticError(
      ...
      nodeToHighlight != null ? nodeToHighlight : value.node,
      errorMessage,
    );
  }
}
...
function extractClassMetadata(
  ...
  const ngClassDecorators = classDecorators
    .filter((dec) => isAngularDecorator2(dec, isCore))
    .map((decorator) =>
  ...
  const metaDecorators = new WrappedNodeExpr3(
    ts14.factory.createArrayLiteralExpression(ngClassDecorators),
  );
  ...
  const decoratedMembers = classMembers.map((member) => {
    ...
    return classMemberToMetadata(
      (_a = member.nameNode) != null ? _a : member.name,
      member.decorators,
      isCore,
    );
  });
  if (decoratedMembers.length > 0) {
    metaPropDecorators = new WrappedNodeExpr3(
      ts14.factory.createObjectLiteralExpression(decoratedMembers),
    );
  }
  return {
...
function ctorParameterToMetadata(param, isCore) {
  const type =
    param.typeValueReference.kind !== 2
      ? valueReferenceToExpression(param.typeValueReference)
      : new LiteralExpr2(void 0);
...
function decoratorToMetadata(decorator, wrapFunctionsInParens) {
  ...
  const properties = [ts14.factory.createPropertyAssignment('type', decorator.identifier)];
  if (decorator.args !== null && decorator.args.length > 0) {
    const args = decorator.args.map((arg) => {
      return wrapFunctionsInParens ? wrapFunctionExpressionsInParens(arg) : arg;
    });
    properties.push(
      ts14.factory.createPropertyAssignment(
        'args',
        ts14.factory.createArrayLiteralExpression(args),
      ),
    );
  }
...
function removeIdentifierReferences(node, names) {
  const result = ts14.transform(node, [
    (context) => (root) =>
      ts14.visitNode(root, function walk(current) {
        return ts14.isIdentifier(current) &&
          (typeof names === 'string' ? current.text === names : names.has(current.text))
          ? ts14.factory.createIdentifier(current.text)
          : ts14.visitEachChild(current, walk, context);
      }),
  ]);
...
import {
  ...
  ConstantPool as ConstantPool2,
  CssSelector as CssSelector4,
  DEFAULT_INTERPOLATION_CONFIG as DEFAULT_INTERPOLATION_CONFIG2,
  DomElementSchemaRegistry as DomElementSchemaRegistry3,
  ExternalExpr as ExternalExpr8,
  FactoryTarget as FactoryTarget3,
  makeBindingParser as makeBindingParser2,
  outputAst as o4,
  R3TargetBinder,
  R3TemplateDependencyKind,
  SelectorMatcher as SelectorMatcher3,
  ViewEncapsulation as ViewEncapsulation2,
} from '@angular/compiler';
...
function makeNotStandaloneDiagnostic(scopeReader, ref, rawExpr, kind) {
  ...
  if (scope !== null && scope.kind === ComponentScopeKind.NgModule) {
    ...
    const relatedInfoMessageText = isExported
      ...
      : `It's declared in the '${scope.ngModule.name.text}' NgModule, but is not exported. Consider exporting it and importing the NgModule instead.`;
      ...
      if (directive !== null) {
        ...
        diagnostics.push(
          makeDiagnostic(
            ...
            errorNode,
            `The class '${decl.node.name.text}' is listed in the declarations of the NgModule '${ngModule.ref.node.name.text}', but is not a directive, a component, or a pipe. Either remove it from the NgModule's declarations, or add an appropriate Angular decorator.`,
    ...
    for (const meta of allDependencies) {
      if (meta.kind === MetaKind.Directive && meta.selector !== null) {
        ...
        if (!ts18.isClassDeclaration(meta.ref.node)) {
          throw new Error(
            `Unexpected non-class declaration ${ts18.SyntaxKind[meta.ref.node.kind]} for pipe ${meta.ref.debugName}`,
          );
        }
...
function parseTopLevelRequiredCall(call, functions, importTracker) {
  const node = call.expression;
  if (
    !ts19.isPropertyAccessExpression(node) ||
    !ts19.isIdentifier(node.expression) ||
...
function parseTopLevelCallFromNamespace(call, functions, importTracker) {
  ...
  if (ts19.isIdentifier(node.expression) && ts19.isIdentifier(node.name)) {
    ...
    ts19.isPropertyAccessExpression(node.expression) &&
    ts19.isIdentifier(node.expression.expression) &&
    ts19.isIdentifier(node.expression.name) &&
    node.name.text === 'required'
  ) {
...
function tryParseSignalInputMapping(member, reflector, importTracker) {
  ...
  const options =
    optionsNode !== void 0 ? parseAndValidateInputAndOutputOptions(optionsNode) : null;
  const classPropertyName = member.name;
  return {
    isSignal: true,
    classPropertyName,
    bindingPropertyName:
      (_a = options == null ? void 0 : options.alias) != null ? _a : classPropertyName,
...
function tryParseSignalQueryFromInitializer(member, reflector, importTracker) {
  ...
  const read = (options == null ? void 0 : options.has('read'))
    ? parseReadOption(options.get('read'))
    : null;
  const descendants = (options == null ? void 0 : options.has('descendants'))
...
function parseReadOption(value) {
  if (
    ts21.isExpressionWithTypeArguments(value) ||
    ts21.isParenthesizedExpression(value) ||
    ts21.isAsExpression(value)
  ) {
    return parseReadOption(value.expression);
  }
  if (
    (ts21.isPropertyAccessExpression(value) && ts21.isIdentifier(value.expression)) ||
    ts21.isIdentifier(value)
  ) {
...
function extractDirectiveMetadata(
  ...
  const isStructural =
    ...
    ctorDeps.some(
      (dep) =>
        dep.token instanceof ExternalExpr4 &&
  ...
  const metadata = {
    ...
    hostDirectives:
      (hostDirectives == null
        ? void 0
        : hostDirectives.map((hostDir) =>
            toHostDirectiveMetadata(hostDir, sourceFile, refEmitter),
          )) || null,
  };
  return {
    jitForced: false,
...
function extractQueriesFromDecorator(queryData, reflector, evaluator, isCore) {
  ...
  reflectObjectLiteral(queryData).forEach((queryExpr, propertyName) => {
    ...
    const queryType = ts22.isPropertyAccessExpression(queryExpr.expression)
...
function tryGetQueryFromFieldDecorator(member, reflector, evaluator, isCore) {
  ...
  if (queryDecorators.length !== 1) {
    throw new FatalDiagnosticError(
      ...
      (_a = member.node) != null ? _a : queryDecorators[0].node,
  ...
  const name =
    (_c = (_b = decorator.import) == null ? void 0 : _b.name) != null ? _c : decorator.name;
  return {
    name,
    decorator,
...
function tryParseInputFieldMapping(
  ...
  if (decorator !== null) {
    ...
    const optionsNode =
      decorator.args !== null && decorator.args.length === 1 ? decorator.args[0] : void 0;
...
function parseDecoratorInputTransformFunction(
  ...
  const firstParam =
    ((_a = definition.parameters[0]) == null ? void 0 : _a.name) === 'this'
...
function parseQueriesOfClassFields(members, reflector, importTracker, evaluator, isCore) {
  ...
  for (const member of members) {
    ...
    const queryNode =
      (_a = decoratorQuery == null ? void 0 : decoratorQuery.decorator.node) != null
        ? _a
        : signalQuery == null
          ? void 0
          : signalQuery.call;
...
function parseOutputFields(
  ...
  for (const member of members) {
    ...
    const queryNode =
      (_b =
        (_a = decoratorOutput == null ? void 0 : decoratorOutput.decorator.node) != null
          ? _a
          : initializerOutput == null
            ? void 0
            : initializerOutput.call) != null
        ? _b
        : modelMapping == null
          ? void 0
          : modelMapping.call;
...
function getHostBindingErrorNode(error, hostExpr) {
  if (ts22.isObjectLiteralExpression(hostExpr) && error.relatedError instanceof ParserError) {
    for (const prop of hostExpr.properties) {
      if (
        ts22.isPropertyAssignment(prop) &&
        ts22.isStringLiteralLike(prop.initializer) &&
...
function extractHostDirectives(rawHostDirectives, evaluator, compilationMode) {
  ...
  return resolved.map((value) => {
    ...
    if (compilationMode === CompilationMode.LOCAL && hostReference instanceof DynamicValue) {
      if (
        !ts22.isIdentifier(hostReference.node) &&
        !ts22.isPropertyAccessExpression(hostReference.node)
      ) {
        throw new FatalDiagnosticError(
...
function toR3InputMetadata(mapping) {
  return {
    ...
    transformFunction:
      mapping.transform !== null ? new WrappedNodeExpr5(mapping.transform.node) : null,
...
var DirectiveDecoratorHandler = class {
  ...
  resolve(node, analysis, symbol) {
    ...
    if (
      ...
      analysis.meta.providers instanceof WrappedNodeExpr6
    ) {
...
function createModuleWithProvidersResolver(reflector, isCore) {
  function _reflectModuleFromTypeParam(type, node) {
    ...
    const typeName =
      (type &&
        ((ts23.isIdentifier(type.typeName) && type.typeName) ||
          (ts23.isQualifiedName(type.typeName) && type.typeName.right))) ||
      null;
    if (typeName === null) {
      return null;
    }
    ...
    if (type.typeArguments === void 0 || type.typeArguments.length !== 1) {
      const parent =
        ts23.isMethodDeclaration(node) && ts23.isClassDeclaration(node.parent) ? node.parent : null;
      const symbolName =
      ...
      throw new FatalDiagnosticError(
        ...
        type,
        `${symbolName} returns a ModuleWithProviders type without a generic type argument. Please add a generic type argument to the ModuleWithProviders type. If this occurrence is in library code you don't control, please contact the library authors.`,
      );
    }
    ...
    for (const t of type.types) {
      if (ts23.isTypeLiteralNode(t)) {
        for (const m of t.members) {
          const ngModuleType =
            (ts23.isPropertySignature(m) &&
              ts23.isIdentifier(m.name) &&
  ...
  return (fn, callExpr, resolve, unresolvable) => {
    ...
    const type =
      (_a = _reflectModuleFromTypeParam(rawType, fn.node)) != null
        ? _a
        : _reflectModuleFromLiteralType(rawType);
    if (type === null) {
      return unresolvable;
    }
...
var NgModuleSymbol = class extends SemanticSymbol {
  ...
  isEmitAffected(previousSymbol) {
    ...
    for (const transitiveImport of this.transitiveImportsFromStandaloneComponents) {
      const prevEntry = previousImports.find((prevEntry2) =>
        isSymbolEqual(prevEntry2, transitiveImport),
      );
      if (prevEntry === void 0) {
        return true;
      }
  ...
  analyze(node, decorator) {
    ...
    const meta =
      decorator.args.length === 1
        ...
        : ts24.factory.createObjectLiteralExpression([]);
    ...
    const bootstrap = bootstrapRefs.map((bootstrap2) =>
      this._toR3Reference(
        bootstrap2.getOriginForDiagnostics(meta, node.name),
        bootstrap2,
        valueContext,
      ),
    );
    ...
    if (
      rawProviders !== null &&
      (!ts24.isArrayLiteralExpression(rawProviders) || rawProviders.elements.length > 0)
    ) {
      wrappedProviders = new WrappedNodeExpr7(
        this.annotateForClosureCompiler
...
function isModuleIdExpression(expr) {
  return (
    ts24.isPropertyAccessExpression(expr) &&
    ts24.isIdentifier(expr.expression) &&
...
function extractTemplate(
  ...
  if (template.isInline) {
    ...
    let sourceMapUrl;
    if (
      ts25.isStringLiteral(template.expression) ||
      ts25.isNoSubstitutionTemplateLiteral(template.expression)
    ) {
...
function getTemplateRange(templateExpr) {
  ...
  const {line, character} = ts25.getLineAndCharacterOfPosition(
...
import {
  CssSelector as CssSelector3,
  DomElementSchemaRegistry as DomElementSchemaRegistry2,
  ExternalExpr as ExternalExpr7,
  WrappedNodeExpr as WrappedNodeExpr9,
} from '@angular/compiler';
...
function readSpanComment(node, sourceFile = node.getSourceFile()) {
  return (
    ts29.forEachTrailingCommentRange(sourceFile.text, node.getEnd(), (pos, end, kind) => {
...
function findFirstMatchingNode(tcb, opts) {
  ...
  const visitor = makeRecursiveVisitor((node) => {
    ...
    if (
      withExpressionIdentifier !== void 0 &&
  ...
  getExpressionCompletionLocation(expr) {
    ...
    if (expr instanceof PropertyRead || expr instanceof PropertyWrite) {
      ...
      if (ts30.isPropertyAccessExpression(whenTrue)) {
        ...
        ts30.isCallExpression(whenTrue) &&
        ts30.isPropertyAccessExpression(whenTrue.expression)
      ) {
...
function tsCastToAny(expr) {
  ...
  return ts32.factory.createParenthesizedExpression(
    ts32.factory.createAsExpression(
      expr,
      ts32.factory.createKeywordTypeNode(ts32.SyntaxKind.AnyKeyword),
    ),
  );
}
function tsCreateElement(tagName) {
  ...
  return ts32.factory.createCallExpression(createElement, void 0, [
    ts32.factory.createStringLiteral(tagName),
  ]);
}
function tsDeclareVariable(id, type) {
  addExpressionIdentifier(type, ExpressionIdentifier.VARIABLE_AS_EXPRESSION);
  const initializer = ts32.factory.createAsExpression(
    ts32.factory.createNonNullExpression(ts32.factory.createNull()),
    type,
  );
  const decl = ts32.factory.createVariableDeclaration(id, void 0, void 0, initializer);
  return ts32.factory.createVariableStatement(void 0, [decl]);
}
function tsCreateTypeQueryForCoercedInput(typeName, coercedInputName) {
  return ts32.factory.createTypeQueryNode(
    ts32.factory.createQualifiedName(typeName, `ngAcceptInputType_${coercedInputName}`),
  );
}
...
function tsCallMethod(receiver, methodName, args = []) {
  ...
  return ts32.factory.createCallExpression(methodAccess, void 0, args);
}
...
import {
  ExpressionType as ExpressionType2,
  R3Identifiers as R3Identifiers3,
...
var TypeParameterEmitter = class {
  ...
  emit(emitReference) {
    ...
    return this.typeParameters.map((typeParam) => {
      const constraint =
        typeParam.constraint !== void 0 ? emitter.emitType(typeParam.constraint) : void 0;
      const defaultType =
        typeParam.default !== void 0 ? emitter.emitType(typeParam.default) : void 0;
      return ts33.factory.updateTypeParameterDeclaration(
        typeParam,
        typeParam.modifiers,
        typeParam.name,
        constraint,
  ...
  resolveTypeReference(type) {
    ...
    return new Reference(
      declaration.node,
      declaration.viaModule === AmbientImport ? AmbientImport : owningModule2,
    );
  }
  translateTypeReference(type, emitReference) {
    ...
    if (!ts33.isTypeReferenceNode(typeNode)) {
      throw new Error(
        `Expected TypeReferenceNode for emitted reference, got ${ts33.SyntaxKind[typeNode.kind]}.`,
      );
    }
    return typeNode;
  }
...
// bazel-out/k8-fastbuild/bin/packages/compiler-cli/src/ngtsc/typecheck/src/tcb_util.mjs
var TCB_FILE_IMPORT_GRAPH_PREPARE_IDENTIFIERS = [R3Identifiers2.InputSignalBrandWriteType];
var TcbInliningRequirement;
(function (TcbInliningRequirement2) {
  TcbInliningRequirement2[(TcbInliningRequirement2['MustInline'] = 0)] = 'MustInline';
  TcbInliningRequirement2[(TcbInliningRequirement2['ShouldInlineForGenericBounds'] = 1)] =
...
function getTemplateId(node, sourceFile, isDiagnosticRequest) {
  ...
  return (
    ts34.forEachLeadingCommentRange(sourceFile.text, start, (pos, end, kind) => {
...
function generateTypeCtorDeclarationFn(env, meta, nodeTypeRef, typeParams) {
  ...
  if (meta.body) {
    const fnType = ts35.factory.createFunctionTypeNode(typeParameters, [initParam], rawType);
    ...
    return ts35.factory.createVariableStatement(void 0, declList);
  } else {
...
function generateInlineTypeCtor(env, node, meta) {
  const rawTypeArgs =
    node.typeParameters !== void 0 ? generateGenericArgs(node.typeParameters) : void 0;
  const rawType = ts35.factory.createTypeReferenceNode(node.name, rawTypeArgs);
  const initParam = constructTypeCtorParameter(env, meta, rawType);
  let body = void 0;
  if (meta.body) {
    body = ts35.factory.createBlock([
      ts35.factory.createReturnStatement(
        ts35.factory.createNonNullExpression(ts35.factory.createNull()),
      ),
    ]);
  }
...
function constructTypeCtorParameter(env, meta, rawType) {
  ...
  for (const {classPropertyName, transform, isSignal} of meta.fields.inputs) {
    if (isSignal) {
      signalInputKeys.push(
        ts35.factory.createLiteralTypeNode(ts35.factory.createStringLiteral(classPropertyName)),
      );
      ...
      coercedKeys.push(
        ts35.factory.createPropertySignature(void 0, classPropertyName, void 0, coercionType),
      );
    }
  }
  ...
  if (coercedKeys.length > 0) {
    ...
    initType =
      initType !== null
        ? ts35.factory.createIntersectionTypeNode([initType, coercedLiteral])
        : coercedLiteral;
  }
  if (signalInputKeys.length > 0) {
    ...
    const unwrapDirectiveSignalInputsExpr = env.referenceExternalType(
      R3Identifiers3.UnwrapDirectiveSignalInputs.moduleName,
      R3Identifiers3.UnwrapDirectiveSignalInputs.name,
      [
    ...
    initType =
      initType !== null
        ? ts35.factory.createIntersectionTypeNode([initType, unwrapDirectiveSignalInputsExpr])
        : unwrapDirectiveSignalInputsExpr;
  }
  if (initType === null) {
    initType = ts35.factory.createTypeLiteralNode([]);
  }
  return ts35.factory.createParameterDeclaration(void 0, void 0, 'init', void 0, initType, void 0);
}
...
function typeParametersWithDefaultTypes(params) {
  ...
  return params.map((param) => {
    if (param.default === void 0) {
      return ts35.factory.updateTypeParameterDeclaration(
        param,
        param.modifiers,
        param.name,
        param.constraint,
        ts35.factory.createKeywordTypeNode(ts35.SyntaxKind.AnyKeyword),
      );
    } else {
      return param;
    }
...
Consider enabling the 'strictTemplates' option in your tsconfig.json for better type inference within this template.`;
    this._diagnostics.push(
      makeTemplateDiagnostic(
        ...
        ts37.DiagnosticCategory.Suggestion,
...
var TypeCheckShimGenerator = class {
  ...
  generateShimForFile(sf, genFilePath, priorShimSf) {
    ...
    return ts38.createSourceFile(
      genFilePath,
      'export const USED_FOR_NG_TYPE_CHECKING = true;',
      ts38.ScriptTarget.Latest,
      true,
      ts38.ScriptKind.TS,
    );
  }
...
import {
  ...
  R3Identifiers as R3Identifiers4,
  SafeCall,
  ...
  TmplAstElement as TmplAstElement2,
  TmplAstForLoopBlock,
  TmplAstIcu,
  TmplAstIfBlock,
...
function translateDiagnostic(diagnostic, resolver) {
  ...
  const fullMapping = getSourceMapping(diagnostic.file, diagnostic.start, resolver, true);
  if (fullMapping === null) {
    return null;
  }
...
var ANY_EXPRESSION = ts40.factory.createAsExpression(
  ts40.factory.createNumericLiteral('0'),
  ts40.factory.createKeywordTypeNode(ts40.SyntaxKind.AnyKeyword),
);
...
var AstTranslator = class {
  ...
  visitConditional(ast) {
    ...
    const node = ts40.factory.createParenthesizedExpression(
      ts40.factory.createConditionalExpression(condExpr, void 0, trueExpr, void 0, falseExpr),
    );
    addParseSpanInfo(node, ast.sourceSpan);
    return node;
  }
  ...
  visitInterpolation(ast) {
    return ast.expressions.reduce(
      (lhs, ast2) =>
        ts40.factory.createBinaryExpression(
          lhs,
          ts40.SyntaxKind.PlusToken,
          wrapForTypeChecker(this.translate(ast2)),
        ),
      ts40.factory.createStringLiteral(''),
    );
  }
  visitKeyedRead(ast) {
  ...
  visitKeyedWrite(ast) {
    ...
    const node = wrapForDiagnostics(
      ts40.factory.createBinaryExpression(left, ts40.SyntaxKind.EqualsToken, right),
    );
    addParseSpanInfo(node, ast.sourceSpan);
    return node;
  }
  ...
  visitPropertyWrite(ast) {
    ...
    const right = wrapForTypeChecker(this.translate(ast.value));
    const node = wrapForDiagnostics(
      ts40.factory.createBinaryExpression(leftWithPath, ts40.SyntaxKind.EqualsToken, right),
    );
    addParseSpanInfo(node, ast.sourceSpan);
    return node;
  }
  visitSafePropertyRead(ast) {
    ...
    if (this.config.strictSafeNavigationTypes) {
      const expr = ts40.factory.createPropertyAccessExpression(
        ts40.factory.createNonNullExpression(receiver),
        ast.name,
      );
      addParseSpanInfo(expr, ast.nameSpan);
      node = ts40.factory.createParenthesizedExpression(
        ts40.factory.createConditionalExpression(ANY_EXPRESSION, void 0, expr, void 0, UNDEFINED),
      );
  ...
  visitSafeKeyedRead(ast) {
    ...
    let node;
    if (this.config.strictSafeNavigationTypes) {
      const expr = ts40.factory.createElementAccessExpression(
        ts40.factory.createNonNullExpression(receiver),
        key,
      );
  ...
  visitTemplateLiteral(ast) {
    ...
    if (length === 1) {
      ...
      result = ts40.factory.createTemplateExpression(
        ts40.factory.createTemplateHead(head.text),
        spans,
      );
    }
    return result;
  ...
  visitTaggedTemplateLiteral(ast) {
    return ts40.factory.createTaggedTemplateExpression(
  ...
  convertToSafeCall(ast, expr, args) {
    if (this.config.strictSafeNavigationTypes) {
      const call = ts40.factory.createCallExpression(
        ts40.factory.createNonNullExpression(expr),
        void 0,
        args,
      );
      return ts40.factory.createParenthesizedExpression(
        ts40.factory.createConditionalExpression(ANY_EXPRESSION, void 0, call, void 0, UNDEFINED),
      );
    }
    ...
    return tsCastToAny(
      ts40.factory.createCallExpression(ts40.factory.createNonNullExpression(expr), void 0, args),
    );
  }
};
    ...
    switch (genericContextBehavior) {
      case TcbGenericContextBehavior.UseEmitter:
        ...
        typeArguments = typeParameters.map((param) =>
          ts41.factory.createTypeReferenceNode(param.name),
        );
        break;
  ...
  execute() {
    ...
    if (directives !== null) {
      for (const dir of directives) {
        ...
        dir.ngTemplateGuards.forEach((guard2) => {
          const boundInput =
            this.template.inputs.find((i) => i.name === guard2.inputName) ||
            this.template.templateAttrs.find(
              (i) => i instanceof TmplAstBoundAttribute && i.name === guard2.inputName,
            );
    ...
    if (directiveGuards.length > 0) {
      guard = directiveGuards.reduce(
        (expr, dirGuard) =>
          ts41.factory.createBinaryExpression(
            expr,
    ...
    if (guard !== null) {
      tmplBlock = ts41.factory.createIfStatement(guard, tmplBlock);
    }
...
var TcbReferenceOp = class extends TcbOp {
  ...
  execute() {
    const id = this.tcb.allocateId();
    let initializer =
      this.target instanceof TmplAstTemplate || this.target instanceof TmplAstElement2
        ? this.scope.resolve(this.target)
        : this.scope.resolve(this.host, this.target);
    if (
      (this.target instanceof TmplAstElement2 && !this.tcb.env.config.checkTypeOfDomReferences) ||
      !this.tcb.env.config.checkTypeOfNonDomReferences
    ) {
      initializer = ts41.factory.createAsExpression(
        initializer,
        ts41.factory.createKeywordTypeNode(ts41.SyntaxKind.AnyKeyword),
      );
  ...
  execute() {
    ...
    for (const attr of boundAttrs) {
      if (
        ...
        attr.attribute instanceof TmplAstTextAttribute2
      ) {
        continue;
      }
    ...
    for (const attr of boundAttrs) {
      ...
      for (const {fieldName, required, transformType, isSignal, isTwoWayBinding} of attr.inputs) {
        ...
        if (this.dir.coercedInputFields.has(fieldName)) {
          ...
          const type = ts41.factory.createIndexedAccessTypeNode(
            ts41.factory.createTypeQueryNode(dirId),
            ts41.factory.createLiteralTypeNode(ts41.factory.createStringLiteral(fieldName)),
          );
          ...
          target = this.dir.stringLiteralInputFields.has(fieldName)
            ? ts41.factory.createElementAccessExpression(
                dirId,
                ts41.factory.createStringLiteral(fieldName),
              )
            : ts41.factory.createPropertyAccessExpression(
                dirId,
                ts41.factory.createIdentifier(fieldName),
              );
        }
        if (isSignal) {
          const inputSignalBrandWriteSymbol = this.tcb.env.referenceExternalSymbol(
            R3Identifiers4.InputSignalBrandWriteType.moduleName,
            R3Identifiers4.InputSignalBrandWriteType.name,
          );
          if (
            !ts41.isIdentifier(inputSignalBrandWriteSymbol) &&
            !ts41.isPropertyAccessExpression(inputSignalBrandWriteSymbol)
          ) {
            throw new Error(
              `Expected identifier or property access for reference to ${R3Identifiers4.InputSignalBrandWriteType.name}`,
            );
          }
        ...
        assignment = ts41.factory.createBinaryExpression(
          target,
          ts41.SyntaxKind.EqualsToken,
          assignment,
        );
      }
...
var TcbDirectiveCtorCircularFallbackOp = class extends TcbOp {
  ...
  execute() {
    ...
    const circularPlaceholder = ts41.factory.createCallExpression(typeCtor, void 0, [
  ...
  constructor(tcb, element, ngContentSelectors, componentName) {
    ...
    this.category =
      tcb.env.config.controlFlowPreventingContentProjection === 'error'
        ? ts41.DiagnosticCategory.Error
        : ts41.DiagnosticCategory.Warning;
  }
  optional = false;
  execute() {
    ...
    for (const binding of this.element.inputs) {
      ...
      if (this.tcb.env.config.checkTypeOfDomBindings && isPropertyBinding) {
        if (binding.name !== 'style' && binding.name !== 'class') {
          ...
          const propertyName = (_a = ATTR_TO_PROP.get(binding.name)) != null ? _a : binding.name;
          const prop = ts41.factory.createElementAccessExpression(
            elId,
            ts41.factory.createStringLiteral(propertyName),
          );
          const stmt = ts41.factory.createBinaryExpression(
            prop,
            ts41.SyntaxKind.EqualsToken,
...
var TcbDirectiveOutputsOp = class extends TcbOp {
  ...
  execute() {
    ...
    for (const output of this.node.outputs) {
      ...
      const outputField = ts41.factory.createElementAccessExpression(
        dirId,
        ts41.factory.createStringLiteral(field),
      );
      ...
      if (this.tcb.env.config.checkTypeOfOutputEvents) {
        ...
        const call = ts41.factory.createCallExpression(subscribeFn, void 0, [handler]);
  ...
  execute() {
    ...
    for (const output of this.element.outputs) {
      ...
      if (output.type === ParsedEventType.Animation) {
        ...
        const propertyAccess = ts41.factory.createPropertyAccessExpression(
          elId,
          'addEventListener',
        );
        addParseSpanInfo(propertyAccess, output.keySpan);
        const call = ts41.factory.createCallExpression(propertyAccess, void 0, [
          ts41.factory.createStringLiteral(output.name),
          handler,
        ]);
        addParseSpanInfo(call, output.sourceSpan);
        this.scope.addStatement(ts41.factory.createExpressionStatement(call));
...
var TcbIfOp = class extends TcbOp {
  ...
  generateBranch(index) {
    ...
    if (branch.expressionAlias !== null) {
      expression = ts41.factory.createBinaryExpression(
        ts41.factory.createParenthesizedExpression(expression),
        ts41.SyntaxKind.AmpersandAmpersandToken,
        outerScope.resolve(branch.expressionAlias),
      );
    }
    const bodyScope = this.getBranchScope(outerScope, branch, index);
    return ts41.factory.createIfStatement(
      expression,
      ts41.factory.createBlock(bodyScope.render()),
  ...
  generateBranchGuard(index) {
    ...
    for (let i = 0; i <= index; i++) {
      ...
      const comparisonExpression =
        i === index
          ? expression
          : ts41.factory.createPrefixUnaryExpression(
              ts41.SyntaxKind.ExclamationToken,
              ts41.factory.createParenthesizedExpression(expression),
            );
      guard =
        guard === null
...
var TcbSwitchOp = class extends TcbOp {
  ...
  execute() {
    ...
    const clauses = this.block.cases.map((current) => {
      ...
      return current.expression === null
        ? ts41.factory.createDefaultClause(statements)
        : ts41.factory.createCaseClause(
            tcbExpression(current.expression, this.tcb, clauseScope),
            statements,
          );
    });
    this.scope.addStatement(
      ts41.factory.createSwitchStatement(switchExpression, ts41.factory.createCaseBlock(clauses)),
    );
    return null;
  }
  generateGuard(node, switchValue) {
    if (node.expression !== null) {
      ...
      markIgnoreDiagnostics(expression);
      return ts41.factory.createBinaryExpression(
        switchValue,
    ...
    for (const current of this.block.cases) {
      ...
      markIgnoreDiagnostics(expression);
      const comparison = ts41.factory.createBinaryExpression(
        switchValue,
      ...
      if (guard === null) {
        ...
        guard = ts41.factory.createBinaryExpression(
          guard,
          ts41.SyntaxKind.AmpersandAmpersandToken,
          comparison,
        );
...
var TcbForOfOp = class extends TcbOp {
  ...
  execute() {
    ...
    const initializer = ts41.factory.createVariableDeclarationList(
      [ts41.factory.createVariableDeclaration(initializerId)],
      ts41.NodeFlags.Const,
    );
    addParseSpanInfo(initializer, this.block.item.keySpan);
    const expression = ts41.factory.createNonNullExpression(
    ...
    this.scope.addStatement(
      ts41.factory.createForOfStatement(
        void 0,
        initializer,
        expression,
        ts41.factory.createBlock(statements),
      ),
    );
    return null;
  }
};
var INFER_TYPE_FOR_CIRCULAR_OP_EXPR = ts41.factory.createNonNullExpression(
  ts41.factory.createNull(),
);
var Context = class {
  env;
  domSchemaChecker;
...
var _Scope = class {
  ...
  static forNodes(tcb, parentScope, scopedNode, children, guard) {
    ...
    if (scopedNode instanceof TmplAstTemplate) {
      ...
      for (const variable of scopedNode.contextVariables) {
        ...
        const type = ts41.factory.createKeywordTypeNode(
  ...
  resolveLocal(ref, directive) {
    if (ref instanceof TmplAstReference2 && this.referenceOpMap.has(ref)) {
      ...
      (ref instanceof TmplAstElement2 || ref instanceof TmplAstTemplate) &&
      directive !== void 0 &&
  ...
  appendContentProjectionCheckOp(root) {
    var _a;
    const meta =
      ((_a = this.tcb.boundTarget.getDirectivesOfNode(root)) == null
        ? void 0
        : _a.find((meta2) => meta2.isComponent)) || null;
...
var TcbExpressionTranslator = class {
  ...
  resolve(ast) {
    if (
      ast instanceof PropertyRead3 &&
      ast.receiver instanceof ImplicitReceiver2 &&
      ...
      const result = ts41.factory.createParenthesizedExpression(
        ts41.factory.createBinaryExpression(targetExpression, ts41.SyntaxKind.EqualsToken, expr),
      );
      ...
      if (!this.tcb.env.config.checkTypeOfPipes) {
        methodAccess = ts41.factory.createAsExpression(
          methodAccess,
          ts41.factory.createKeywordTypeNode(ts41.SyntaxKind.AnyKeyword),
        );
      }
      const result = ts41.factory.createCallExpression(methodAccess, void 0, [expr, ...args]);
      addParseSpanInfo(result, ast.sourceSpan);
      return result;
    } else if (
      (ast instanceof Call2 || ast instanceof SafeCall) &&
      (ast.receiver instanceof PropertyRead3 || ast.receiver instanceof SafePropertyRead3)
    ) {
      if (
        ast.receiver.receiver instanceof ImplicitReceiver2 &&
        !(ast.receiver.receiver instanceof ThisReceiver) &&
        ast.receiver.name === '$any' &&
        ast.args.length === 1
      ) {
        const expr = this.translate(ast.args[0]);
        const exprAsAny = ts41.factory.createAsExpression(
          expr,
          ts41.factory.createKeywordTypeNode(ts41.SyntaxKind.AnyKeyword),
        );
...
function tcbCallTypeCtor(dir, tcb, inputs) {
  ...
  const members = inputs.map((input) => {
    ...
    if (input.type === 'binding') {
      ...
      const assignment = ts41.factory.createPropertyAssignment(
        propertyName,
        wrapForDiagnostics(expr),
      );
  ...
  return ts41.factory.createCallExpression(typeCtor, void 0, [
...
function unwrapWritableSignal(expression, tcb) {
  const unwrapRef = tcb.env.referenceExternalSymbol(
    R3Identifiers4.unwrapWritableSignal.moduleName,
    R3Identifiers4.unwrapWritableSignal.name,
  );
...
function tcbCreateEventHandler(event, tcb, scope, eventType) {
  ...
  if (event.type === ParsedEventType.TwoWay && tcb.env.config.checkTwoWayBoundEvents) {
    ...
    const assignment = ts41.factory.createBinaryExpression(
      target,
      ts41.SyntaxKind.EqualsToken,
      ts41.factory.createIdentifier(EVENT_PARAMETER),
    );
    statements.push(
      tsCreateVariable(
        target,
      ...
      ts41.factory.createExpressionStatement(assignment),
    );
  } else {
...
var TypeCheckFile = class extends Environment {
  ...
  render(removeComments) {
    ...
    if (newImports !== void 0) {
      source += newImports
        .map((i) => printer.printNode(ts42.EmitHint.Unspecified, i, this.contextFile))
        .join('\n');
    }
    source += '\n';
...
var TypeCheckContextImpl = class {
  ...
  addDirective(ref, binder, schemas, templateContext, isStandalone) {
    ...
    const meta = {
      ...
      isStandalone,
      preserveWhitespaces:
        (_a = templateContext == null ? void 0 : templateContext.preserveWhitespaces) != null
          ? _a
          : false,
    };
...
import {
  ParseLocation as ParseLocation2,
  ParseSourceSpan as ParseSourceSpan2,
} from '@angular/compiler';
...
var SymbolBuilder = class {
  ...
  getDirectivesOfNode(element) {
    ...
    const isDirectiveDeclaration = (node) =>
      (ts44.isTypeNode(node) || ts44.isIdentifier(node)) &&
      ts44.isVariableDeclaration(node.parent) &&
    ...
    for (const node of nodes) {
      ...
      if (
        ...
        !ts44.isClassDeclaration(symbol.tsSymbol.valueDeclaration)
      ) {
        continue;
      }
  ...
  getDirectiveMeta(host, directiveDeclaration) {
    ...
    if (firstChild instanceof TmplAstElement3) {
      const isMicrosyntaxTemplate =
        host instanceof TmplAstTemplate2 && sourceSpanEqual(firstChild.sourceSpan, host.sourceSpan);
  ...
  getSymbolOfBoundEvent(eventBinding) {
    ...
    function filter(n2) {
      ...
      if (ts44.isPropertyAccessExpression(n2)) {
        ...
        return (
          ts44.isStringLiteral(n2.argumentExpression) &&
          n2.argumentExpression.text === expectedAccess
        );
      }
    }
  ...
  getDirectiveSymbolForAccessExpression(fieldAccessExpr, {isComponent, selector, isStructural}) {
    ...
    if (
      (tsSymbol == null ? void 0 : tsSymbol.declarations) === void 0 ||
    ...
    if (
      !ts44.isVariableDeclaration(declaration) ||
  ...
  getSymbolOfReference(ref) {
    ...
    const originalDeclaration =
      ts44.isParenthesizedExpression(node.initializer) &&
      ts44.isAsExpression(node.initializer.expression)
  ...
  getSymbolOfTemplateExpression(expression) {
    ...
    if (
      expression instanceof PropertyWrite3 ||
      (expression instanceof ASTWithName && !(expression instanceof SafePropertyRead4))
    ) {
      withSpan = expression.nameSpan;
    }
    let node = null;
...
function unwrapSignalInputWriteTAccessor(expr) {
  if (
    !ts44.isElementAccessExpression(expr) ||
    !ts44.isPropertyAccessExpression(expr.argumentExpression)
  ) {
    return null;
  }
  if (
    !ts44.isIdentifier(expr.argumentExpression.name) ||
    expr.argumentExpression.name.text !== R3Identifiers5.InputSignalBrandWriteType.name
  ) {
    return null;
  }
  if (
    !ts44.isPropertyAccessExpression(expr.expression) &&
    !ts44.isElementAccessExpression(expr.expression) &&
    !ts44.isIdentifier(expr.expression)
  ) {
...
var TemplateTypeCheckerImpl = class {
  ...
  getUsedDirectives(component) {
    var _a;
    return (
      ((_a = this.getLatestComponentState(component).data) == null
        ? void 0
        : _a.boundTarget.getUsedDirectives()) || null
    );
  }
  getUsedPipes(component) {
    ...
    return (
      ((_a = this.getLatestComponentState(component).data) == null
        ? void 0
        : _a.boundTarget.getUsedPipes()) || null
    );
  }
  ...
  getSourceMappingAtTcbLocation(tcbLocation) {
    ...
    return getSourceMapping(shimSf, tcbLocation.positionInFile, fileRecord.sourceManager, false);
  }
  ...
  getExpressionTarget(expression, clazz) {
    var _a;
    return (
      ((_a = this.getLatestComponentState(clazz).data) == null
        ? void 0
        : _a.boundTarget.getExpressionTarget(expression)) || null
    );
  }
  ...
  getPotentialTemplateDirectives(component) {
    ...
    const inScopeDirectives =
      (_b = (_a = this.getScopeData(component)) == null ? void 0 : _a.directives) != null ? _b : [];
  ...
  getPotentialPipes(component) {
    ...
    const typeChecker = this.programDriver.getProgram().getTypeChecker();
    const inScopePipes =
      (_b = (_a = this.getScopeData(component)) == null ? void 0 : _a.pipes) != null ? _b : [];
  ...
  emit(kind, refTo, inContext) {
    ...
    if (emitted instanceof WrappedNodeExpr9) {
      ...
      if (emitted.node.getStart() > inContext.getStart()) {
        const declaration =
          (_b =
            (_a = this.programDriver
              .getProgram()
              .getTypeChecker()
              .getTypeAtLocation(emitted.node)
              .getSymbol()) == null
              ? void 0
              : _a.declarations) == null
            ? void 0
            : _b[0];
      ...
      emitted instanceof ExternalExpr7 &&
  ...
  getPotentialImportsFor(toImport, inContext, importMode) {
    ...
    const imports = [];
    const meta =
      (_a = this.metaReader.getDirectiveMetadata(toImport)) != null
        ? _a
        : this.metaReader.getPipeMetadata(toImport);
    if (meta === null) {
      return imports;
    }
...
function extractHmrDependencies(
  ...
  if (deferBlockMetadata.mode === 0) {
    deferBlockMetadata.blocks.forEach((loader) =>
      loader == null ? void 0 : loader.visitExpression(visitor, null),
    );
  } else {
...
function getTopLevelDeclarationNames(sourceFile) {
  ...
  for (const node of sourceFile.statements) {
    if (
      ts45.isClassDeclaration(node) ||
      ts45.isFunctionDeclaration(node) ||
      ts45.isEnumDeclaration(node)
    ) {
      if (node.name) {
...
var PotentialTopLevelReadsVisitor = class extends o2.RecursiveAstVisitor {
  ...
  isTopLevelIdentifierReference(identifier) {
    ...
    if (
      ts45.isExpressionStatement(parent) ||
      ts45.isPropertyAccessExpression(parent) ||
      ts45.isComputedPropertyName(parent) ||
      ts45.isTemplateSpan(parent) ||
      ts45.isSpreadAssignment(parent) ||
      ts45.isSpreadElement(parent) ||
      ts45.isAwaitExpression(parent) ||
      ts45.isNonNullExpression(parent) ||
      ts45.isIfStatement(parent) ||
      ts45.isDoStatement(parent) ||
      ts45.isWhileStatement(parent) ||
      ts45.isSwitchStatement(parent) ||
      ts45.isCaseClause(parent) ||
      ts45.isThrowStatement(parent) ||
      ts45.isNewExpression(parent)
    ) {
    ...
    if (
      ts45.isPropertyAssignment(parent) ||
      ts45.isParameter(parent) ||
      ts45.isBindingElement(parent) ||
      ts45.isPropertyDeclaration(parent) ||
      ts45.isEnumMember(parent)
    ) {
    ...
    if (
      ts45.isClassDeclaration(parent) ||
      ts45.isFunctionDeclaration(parent) ||
      ts45.isShorthandPropertyAssignment(parent)
    ) {
...
function isConstEnumReference(node, reflection) {
  ...
  return (
    declaration !== null &&
    ts45.isEnumDeclaration(declaration.node) &&
    !!((_a = declaration.node.modifiers) == null
      ? void 0
      : _a.some((m) => m.kind === ts45.SyntaxKind.ConstKeyword))
  );
}
...
function getHmrUpdateDeclaration(compilationResults, constantStatements, meta, sourceFile) {
  ...
  return ts46.factory.updateFunctionDeclaration(
    node,
    [
...
var ComponentDecoratorHandler = class {
  ...
  analyze(node, decorator) {
    ...
    const encapsulation =
      (_a =
        this.compilationMode !== CompilationMode.LOCAL
          ...
          : resolveEncapsulationEnumValueLocally(component.get('encapsulation'))) != null
        ? _a
        : ViewEncapsulation2.Emulated;
    ...
    const relativeContextFilePath = this.rootDirs.reduce(
      (previous, rootDir) => {
    ...
    if (component.has('viewProviders')) {
      ...
      wrappedViewProviders = new o4.WrappedNodeExpr(
        this.annotateForClosureCompiler
    ...
    if (this.preanalyzeTemplateCache.has(node)) {
      ...
      if (
        ...
        diagnostics.push(...getTemplateDiagnostics(template.errors, '', template.sourceMapping));
      }
    }
    ...
    const relativeTemplatePath = getProjectRelativePath(
      (_d = templateResource.path) != null
        ? _d
        : ts47.getOriginalNode(node).getSourceFile().fileName,
      this.rootDirs,
      this.compilerHost,
    );
    let styles = [];
    ...
    const output = {
      analysis: {
        ...
        meta: {
          ...
          changeDetection,
          interpolation:
            (_e = template.interpolationConfig) != null ? _e : DEFAULT_INTERPOLATION_CONFIG2,
          styles,
          externalStyles,
          animations,
  ...
  resolve(node, analysis, symbol) {
    ...
    if (this.compilationMode === CompilationMode.LOCAL) {
      data = {
        ...
        declarationListEmitMode:
          !analysis.meta.isStandalone || analysis.rawImports !== null ? 3 : 0,
    ...
    if (scope !== null) {
      ...
      if (isModuleScope && context.fileName !== getSourceFile(scope.ngModule).fileName) {
        (_b = this.localCompilationExtraImportsTracker) == null
          ? void 0
          : _b.markFileForExtraImportGeneration(context);
      }
      if (
    ...
    if (this.compilationMode !== CompilationMode.LOCAL) {
      ...
      if (
        analysis.providersRequiringFactory !== null &&
        analysis.meta.providers instanceof o4.WrappedNodeExpr
      ) {
        const providerDiagnostics = getProviderDiagnostics(
          analysis.providersRequiringFactory,
          analysis.meta.providers.node,
      ...
      if (
        ...
        analysis.meta.viewProviders instanceof o4.WrappedNodeExpr
      ) {
  ...
  xi18n(ctx, node, analysis) {
    ...
    ctx.updateFromTemplate(
      ...
      (_a = analysis.template.interpolationConfig) != null ? _a : DEFAULT_INTERPOLATION_CONFIG2,
    );
  }
    ...
    const templateInfo = {
      ...
      inlineTemplateLiteralExpression:
        analysis.template.sourceMapping.type === 'direct'
          ? new o4.WrappedNodeExpr(analysis.template.sourceMapping.node)
          : null,
    };
    const perComponentDeferredDeps = this.canDeferDeps
      ? this.resolveAllDeferredDependencies(resolution)
  ...
  resolveAllDeferredDependencies(resolution) {
    ...
    for (const [_, deps] of resolution.deferPerBlockDependencies) {
      for (const deferBlockDep of deps) {
        ...
        const importDecl =
          (_a = resolution.deferrableDeclToImportDecl.get(node)) != null ? _a : null;
  ...
  for (const deferredDep of deferredDeps) {
    if (eagerDepsSet.has(deferredDep.ref.node)) {
      ...
      diagnostic = makeDiagnostic(
        ...
        `\`${classInfo}\` is imported via both \`@Component.imports\` and \`@Component.deferredImports\`. To fix this, make sure that dependencies are imported only once.`,
      );
      break;
    }
  }
...
import {
  ...
  compileInjectable,
  createMayBeForwardRefExpression as createMayBeForwardRefExpression3,
  FactoryTarget as FactoryTarget4,
  LiteralExpr as LiteralExpr3,
  WrappedNodeExpr as WrappedNodeExpr10,
} from '@angular/compiler';
  ...
  compile(compileFactoryFn, compileInjectableFn, compileClassMetadataFn, node, analysis) {
    ...
    if (analysis.needsFactory) {
      ...
      const factoryRes = compileFactoryFn(
        toFactoryMetadata({...meta, deps: analysis.ctorDeps}, FactoryTarget4.Injectable),
      );
...
function extractInjectableMetadata(clazz, decorator, reflector) {
  ...
  if (decorator.args.length === 0) {
    ...
    const providedIn = meta.has('providedIn')
      ...
      : createMayBeForwardRefExpression3(new LiteralExpr3(null), 0);
    let deps = void 0;
...
function getProviderExpression(expression, reflector) {
  ...
  return createMayBeForwardRefExpression3(
    new WrappedNodeExpr10(forwardRefValue != null ? forwardRefValue : expression),
...
function requiresValidCtor(meta) {
  return (
    meta.useValue === void 0 &&
    meta.useExisting === void 0 &&
    meta.useClass === void 0 &&
    meta.useFactory === void 0
  );
}
...
var PipeDecoratorHandler = class {
  ...
  compileFull(node, analysis) {
    ...
    const classMetadata =
      analysis.classMetadata !== null
        ? compileClassMetadata4(analysis.classMetadata).toStmt()
        : null;
  ...
  compilePartial(node, analysis) {
    ...
    const classMetadata =
      analysis.classMetadata !== null
        ? compileDeclareClassMetadata4(analysis.classMetadata).toStmt()
        : null;
    return compileResults(fac, def, classMetadata, '\u0275pipe', null, null);
  }
  compileLocal(node, analysis) {
