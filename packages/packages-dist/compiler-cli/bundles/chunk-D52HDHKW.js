...
import {angularJitApplicationTransform} from './chunk-FL3TJEGV.js';
import {
  CompilationMode,
...
function extractGenerics(declaration) {
  var _a, _b;
  return (_b =
    (_a = declaration.typeParameters) == null
      ? void 0
      : _a.map((typeParam) => {
          var _a2, _b2;
          return {
  ...
  return ts2.getJSDocTags(escapedNode).map((t) => {
    ...
    return {
      ...
      comment: unescapeAngularDecorators(
        (_a = ts2.getTextOfJSDocComment(t.comment)) != null ? _a : '',
      ),
    };
  });
}
function extractJsDocDescription(node) {
  ...
  const description =
    typeof comment === 'string'
      ? comment
      : (_b = ts2.getTextOfJSDocComment(comment)) != null
        ? _b
        : '';
...
function extractRawJsDoc(node) {
  ...
  const comment =
    (_b =
      (_a = ts2.getJSDocCommentsAndTags(node).find(ts2.isJSDoc)) == null
        ? void 0
        : _a.getFullText()) != null
      ? _b
      : '';
...
function extractResolvedTypeString(node, checker) {
  return checker.typeToString(
    ...
    void 0,
    ts3.TypeFormatFlags.NoTruncation,
  );
}
...
var FunctionExtractor = class {
  ...
  extract() {
    ...
    const implementation =
      (_a = findImplementationOfFunction(this.exportDeclaration, this.typeChecker)) != null
        ? _a
    ...
    return {
      ...
      implementation: {
        ...
        returnType,
        returnDescription:
          (_b = jsdocsTags.find((tag) => tag.name === 'returns')) == null ? void 0 : _b.comment,
...
function filterSignatureDeclarations(signatures) {
  ...
  for (const signature of signatures) {
    ...
    if (
      ts4.isFunctionDeclaration(decl) ||
      ts4.isCallSignatureDeclaration(decl) ||
      ts4.isMethodDeclaration(decl)
    ) {
...
function findImplementationOfFunction(node, typeChecker) {
  ...
  const implementation =
    (_a = symbol == null ? void 0 : symbol.declarations) == null
      ? void 0
      : _a.find((s) => ts4.isFunctionDeclaration(s) && s.body !== void 0);
...
var ClassExtractor = class {
  ...
  extract() {
    return {
      ...
      entryType: ts6.isInterfaceDeclaration(this.declaration)
        ? EntryType.Interface
  ...
  extractSignature(signature) {
    const functionExtractor = new FunctionExtractor(
      ts6.isConstructSignatureDeclaration(signature) ? 'new' : '',
      signature,
      this.typeChecker,
    );
    return {
  ...
  extractInterfaceConformance(declaration) {
    var _a, _b;
    const implementClause =
      (_a = declaration.heritageClauses) == null
        ? void 0
        : _a.find((clause) => clause.token === ts6.SyntaxKind.ImplementsKeyword);
    return (_b =
      implementClause == null ? void 0 : implementClause.types.map((m) => m.getText())) != null
      ? _b
      : [];
  }
  ...
  getMemberDeclarations() {
    ...
    for (const member of [...members, ...staticMembers]) {
      const memberDeclarations = this.filterMethodOverloads(
        (_a = member.getDeclarations()) != null ? _a : [],
      );
    ...
    return declarations.filter((declaration, index) => {
      ...
      if (ts6.isFunctionDeclaration(declaration) || ts6.isMethodDeclaration(declaration)) {
        ...
        const isNextAbstractMethodWithSameName =
          nextDeclaration &&
          ts6.isMethodDeclaration(nextDeclaration) &&
          nextDeclaration.name.getText() ===
            ((_a = declaration.name) == null ? void 0 : _a.getText());
  ...
  isMemberExcluded(member) {
    ...
    return (
      ...
      (!ts6.isCallSignatureDeclaration(member) &&
        ((_a = member.modifiers) == null
          ? void 0
          : _a.some((mod) => mod.kind === ts6.SyntaxKind.PrivateKeyword))) ||
  ...
  isProperty(member) {
    return (
      ts6.isPropertyDeclaration(member) ||
  ...
  isDocumentableSignature(signature) {
    return (
      ts6.isConstructSignatureDeclaration(signature) || ts6.isCallSignatureDeclaration(signature)
    );
  }
  isAbstract() {
    var _a;
  ...
  hasPrivateComputedProperty(property) {
    return (
      ts6.isComputedPropertyName(property.name) &&
...
var DirectiveExtractor = class extends ClassExtractor {
  ...
  getInputMetadata(prop) {
    ...
    return (_b =
      (_a = this.metadata.inputs) == null ? void 0 : _a.getByClassPropertyName(propName)) != null
      ? _b
      : void 0;
  }
  getOutputMetadata(prop) {
    ...
    const propName = prop.name.getText();
    return (_c =
      (_b = (_a = this.metadata) == null ? void 0 : _a.outputs) == null
        ? void 0
        : _b.getByClassPropertyName(propName)) != null
      ? _c
      : void 0;
  }
};
...
function extractLiteralPropertiesAsEnumMembers(declaration) {
  ...
  while (
    initializer &&
    (ts7.isAsExpression(initializer) || ts7.isParenthesizedExpression(initializer))
  ) {
    initializer = initializer.expression;
  }
  if (initializer === void 0 || !ts7.isObjectLiteralExpression(initializer)) {
    throw new Error(
      `Declaration tagged with "${LITERAL_AS_ENUM_TAG}" must be initialized to an object literal, but received ${initializer ? ts7.SyntaxKind[initializer.kind] : 'undefined'}`,
    );
  }
...
function getDecoratorType(declaration) {
  var _a, _b;
  const initializer =
    (_b = (_a = declaration.initializer) == null ? void 0 : _a.getFullText()) != null ? _b : '';
...
function getDecoratorOptions(declaration, typeChecker) {
  ...
  const optionsDeclaration = declaration.getSourceFile().statements.find((node) => {
    return (
      (ts8.isInterfaceDeclaration(node) || ts8.isTypeAliasDeclaration(node)) &&
  ...
  if (ts8.isTypeAliasDeclaration(optionsDeclaration)) {
    ...
    optionsInterface = (
      (_b = (_a = aliasedType.getSymbol()) == null ? void 0 : _a.getDeclarations()) != null
        ? _b
        : []
    ).find((d) => ts8.isInterfaceDeclaration(d));
  } else {
...
function getEnumMemberValue(memberNode) {
  ...
  const literal = memberNode.getChildren().find((n) => {
    return (
      ts9.isNumericLiteral(n) ||
      ts9.isStringLiteral(n) ||
      (ts9.isPrefixUnaryExpression(n) &&
        n.operator === ts9.SyntaxKind.MinusToken &&
        ts9.isNumericLiteral(n.operand))
    );
  });
...
function extractInitializerApiFunction(node, typeChecker) {
  ...
  if (metadataTag === void 0) {
    throw new Error(
      `Initializer API: Detected initializer API function does not have "@initializerApiFunction" tag: ${name}`,
    );
  }
...
var DocsExtractor = class {
  ...
  getExportedDeclarations(sourceFile) {
    ...
    let exportedDeclarations = Array.from(
      (_a = exportedDeclarationMap == null ? void 0 : exportedDeclarationMap.entries()) != null
        ? _a
        : [],
    ).map(([exportName, declaration]) => [exportName, declaration.node]);
...
function isIgnoredDocEntry(entry) {
  ...
  if (isDocsPrivate !== void 0 && isDocsPrivate.comment === '') {
    throw new Error(
      `Docs extraction: Entry "${entry.name}" is marked as "@docsPrivate" but without reasoning.`,
    );
  }
  ...
  scanImports(sf) {
    return this.perf.inPhase(PerfPhase.CycleDetection, () => {
      ...
      for (const stmt of sf.statements) {
        if (
          (!ts14.isImportDeclaration(stmt) && !ts14.isExportDeclaration(stmt)) ||
          stmt.moduleSpecifier === void 0
        ) {
          continue;
        }
        if (
          ts14.isImportDeclaration(stmt) &&
          stmt.importClause !== void 0 &&
...
function isTypeOnlyImportClause(node) {
  ...
  if (
    ...
    ts14.isNamedImports(node.namedBindings) &&
...
`;
    const genFile = ts15.createSourceFile(
      this.flatIndexPath,
      contents,
      ts15.ScriptTarget.ES2015,
      true,
      ts15.ScriptKind.TS,
    );
...
function getIdentifierOfDeclaration(decl) {
  if (
    (ts16.isClassDeclaration(decl) ||
      ts16.isVariableDeclaration(decl) ||
      ts16.isFunctionDeclaration(decl)) &&
    decl.name !== void 0 &&
    ts16.isIdentifier(decl.name)
  ) {
    return decl.name;
  } else {
    return null;
...
var IncrementalCompilation = class {
  ...
  static incremental(program, newVersions, oldProgram, oldState, modifiedResourceFiles, perf) {
    return perf.inPhase(PerfPhase.Reconciliation, () => {
      ...
      const changedResourceFiles = new Set(
        modifiedResourceFiles != null ? modifiedResourceFiles : [],
      );
      let priorAnalysis;
...
var NgModuleIndexImpl = class {
  ...
  indexTrait(ref, seenTypesWithReexports) {
    ...
    const meta =
      (_a = this.metaReader.getDirectiveMetadata(ref)) != null
        ? _a
        : this.metaReader.getNgModuleMetadata(ref);
    if (meta === null) {
      return;
    }
    ...
    if (meta.kind === MetaKind.NgModule) {
      ...
      for (const childRef of meta.exports) {
        ...
        const childMeta =
          (_c =
            (_b = this.metaReader.getDirectiveMetadata(childRef)) != null
              ? _b
              : this.metaReader.getPipeMetadata(childRef)) != null
            ? _c
...
var AdapterResourceLoader = class {
  ...
  resolve(url, fromFile) {
    ...
    if (this.adapter.resourceNameToFileName) {
      resolvedUrl = this.adapter.resourceNameToFileName(url, fromFile, (url2, fromFile2) =>
        this.fallbackResolve(url2, fromFile2),
      );
    } else {
  ...
  getResolvedCandidateLocations(url, fromFile) {
    const failedLookup = ts17.resolveModuleName(
      url + RESOURCE_MARKER,
      fromFile,
      this.options,
...
function isSignalReference(symbol) {
  return (
    ...
    ((symbol.tsType.symbol !== void 0 && isSignalSymbol(symbol.tsType.symbol)) ||
      (symbol.tsType.aliasSymbol !== void 0 && isSignalSymbol(symbol.tsType.aliasSymbol)))
  );
}
function isSignalSymbol(symbol) {
  ...
  return (
    ...
    declarations.some((decl) => {
      ...
      return (
        (ts18.isInterfaceDeclaration(decl) || ts18.isTypeAliasDeclaration(decl)) &&
...
import {
  ...
  TmplAstBoundDeferredTrigger as TmplAstBoundDeferredTrigger2,
} from '@angular/compiler';
  ...
  visitNode(ctx, component, node) {
    if (node instanceof Interpolation) {
      return node.expressions
        .filter((item) => item instanceof PropertyRead2)
      ...
      if (
        ...
        node.value instanceof ASTWithSource3 &&
        node.value.ast instanceof PropertyRead2
      ) {
...
var MissingControlFlowDirectiveCheck = class extends TemplateCheckWithVisitor {
  ...
  visitNode(ctx, component, node) {
    if (!(node instanceof TmplAstTemplate2)) return [];
...
var factory5 = {
  ...
  create: (options) => {
    const strictNullChecks =
      options.strictNullChecks === void 0 ? !!options.strict : !!options.strictNullChecks;
    if (!strictNullChecks) {
      return null;
    }
...
var SuffixNotSupportedCheck = class extends TemplateCheckWithVisitor {
  ...
  visitNode(ctx, component, node) {
    if (!(node instanceof TmplAstBoundAttribute2)) return [];
    if (
...
import {
  ...
  TmplAstBoundEvent as TmplAstBoundEvent2,
} from '@angular/compiler';
var UninvokedFunctionInEventBindingSpec = class extends TemplateCheckWithVisitor {
  ...
  visitNode(ctx, component, node) {
    ...
    if (!(node.handler instanceof ASTWithSource4)) return [];
  ...
  getAnalysis(node) {
    if (!this.analysis.has(node)) {
      this.analysis.set(node, {
        allLetDeclarations: /* @__PURE__ */ new Set(),
        usedLetDeclarations: /* @__PURE__ */ new Set(),
      });
    }
...
import {
  TmplAstBoundAttribute as TmplAstBoundAttribute3,
...
var NgSkipHydrationSpec = class extends TemplateCheckWithVisitor {
  ...
  visitNode(ctx, component, node) {
    ...
    if (
      node instanceof TmplAstTextAttribute2 &&
    ...
    for (const factory12 of templateCheckFactories) {
      const category = diagnosticLabelToCategory(
        (_e =
          (_d =
            (_b =
              (_a = options == null ? void 0 : options.extendedDiagnostics) == null
                ? void 0
                : _a.checks) == null
              ? void 0
              : _b[factory12.name]) != null
            ? _d
            : (_c = options == null ? void 0 : options.extendedDiagnostics) == null
              ? void 0
              : _c.defaultCategory) != null
          ? _e
...
import {
  ASTWithSource as ASTWithSource6,
  ImplicitReceiver as ImplicitReceiver2,
  ParsedEventType as ParsedEventType2,
  RecursiveAstVisitor as RecursiveAstVisitor3,
  TmplAstBoundEvent as TmplAstBoundEvent3,
  TmplAstLetDeclaration as TmplAstLetDeclaration2,
  TmplAstRecursiveVisitor as TmplAstRecursiveVisitor2,
...
var ExpressionsSemanticsVisitor = class extends RecursiveAstVisitor3 {
  ...
  checkForIllegalWriteInTwoWayBinding(ast, context) {
    if (
      !(context instanceof TmplAstBoundEvent3) ||
      context.type !== ParsedEventType2.TwoWay ||
...
var InitializerApiUsageRule = class {
  ...
  checkNode(node) {
    ...
    while (
      node.parent &&
      (ts23.isParenthesizedExpression(node.parent) || ts23.isAsExpression(node.parent))
    ) {
      node = node.parent;
    }
      ...
      if (closestClass && ts23.isClassDeclaration(closestClass)) {
        ...
        const isComponentOrDirective =
          ...
          decorators.some((decorator) => {
            var _a;
            return (
              ((_a = decorator.import) == null ? void 0 : _a.from) === '@angular/core' &&
        ...
        return isComponentOrDirective
          ...
          : makeDiagnostic(
              ...
              node,
              `Unsupported call to the ${functionName} function. This function can only be used as the initializer of a property on a @Component or @Directive class.`,
            );
      }
    }
  ...
  checkNode(node) {
    ...
    const category =
      this.typeCheckingConfig.unusedStandaloneImports === 'error'
        ...
        : ts24.DiagnosticCategory.Warning;
  ...
  isPotentialSharedReference(reference, rawImports) {
    ...
    while (current !== null) {
      if (ts24.isVariableStatement(current)) {
        return !!((_a = current.modifiers) == null
          ? void 0
          : _a.some((m) => m.kind === ts24.SyntaxKind.ExportKeyword));
      }
...
(function (CompilationTicketKind2) {
  ...
  CompilationTicketKind2[(CompilationTicketKind2['IncrementalTypeScript'] = 1)] =
    'IncrementalTypeScript';
  CompilationTicketKind2[(CompilationTicketKind2['IncrementalResource'] = 2)] =
    'IncrementalResource';
...
function incrementalFromStateTicket(
  oldProgram,
  oldState,
  newProgram,
  options,
...
var NgCompiler = class {
  ...
  getMeta(classDecl) {
    ...
    const meta =
      (_a = metaReader.getPipeMetadata(ref)) != null ? _a : metaReader.getDirectiveMetadata(ref);
    if (meta === null) {
      return null;
    }
    return meta;
  ...
  prepareEmit() {
    ...
    if (compilation.supportJitMode && compilation.jitDeclarationRegistry.jitDeclarations.size > 0) {
      ...
      const jitDeclarationOriginalNodes = new Set(
        jitDeclarationsArray.map((d) => ts25.getOriginalNode(d)),
      );
  ...
  getTypeCheckingConfig() {
    ...
    const checkTwoWayBoundEvents =
      (_a = this.options['_checkTwoWayBoundEvents']) != null ? _a : false;
    let allowSignalsInTwoWayBindings =
      (_b = coreHasSymbol(this.inputProgram, R3Identifiers.unwrapWritableSignal)) != null
        ? _b
        : this.angularCoreVersion === null ||
    ...
    if (this.fullTemplateTypeCheck) {
      typeCheckingConfig = {
        ...
        controlFlowPreventingContentProjection:
          ((_c = this.options.extendedDiagnostics) == null ? void 0 : _c.defaultCategory) ||
          DiagnosticCategoryLabel.Warning,
        unusedStandaloneImports:
          ((_d = this.options.extendedDiagnostics) == null ? void 0 : _d.defaultCategory) ||
          DiagnosticCategoryLabel.Warning,
      ...
      typeCheckingConfig = {
        ...
        controlFlowPreventingContentProjection:
          ((_e = this.options.extendedDiagnostics) == null ? void 0 : _e.defaultCategory) ||
          DiagnosticCategoryLabel.Warning,
        unusedStandaloneImports:
          ((_f = this.options.extendedDiagnostics) == null ? void 0 : _f.defaultCategory) ||
          DiagnosticCategoryLabel.Warning,
        allowSignalsInTwoWayBindings,
        checkTwoWayBoundEvents,
      };
    ...
    if (
      ((_h = (_g = this.options.extendedDiagnostics) == null ? void 0 : _g.checks) == null
        ? void 0
        : _h.controlFlowPreventingContentProjection) !== void 0
    ) {
    ...
    if (
      ((_j = (_i = this.options.extendedDiagnostics) == null ? void 0 : _i.checks) == null
        ? void 0
        : _j.unusedStandaloneImports) !== void 0
    ) {
  ...
  runAdditionalChecks(sf) {
    ...
    for (const sf2 of files) {
      ...
      if (templateSemanticsChecker !== null) {
        diagnostics.push(
          ...compilation.traitCompiler.runAdditionalChecks(sf2, (clazz, handler) => {
            var _a;
            return (
              ((_a = handler.templateSemanticsCheck) == null
                ? void 0
                : _a.call(handler, clazz, templateSemanticsChecker)) || null
            );
          }),
        );
      }
      if (this.options.strictTemplates && extendedTemplateChecker !== null) {
        diagnostics.push(
          ...compilation.traitCompiler.runAdditionalChecks(sf2, (clazz, handler) => {
            var _a;
            return (
              ((_a = handler.extendedTemplateCheck) == null
                ? void 0
                : _a.call(handler, clazz, extendedTemplateChecker)) || null
            );
          }),
        );
      }
  ...
  makeCompilation() {
    ...
    if (
      ...
      if (
        this.options.rootDir !== void 0 ||
        (this.options.rootDirs !== void 0 && this.options.rootDirs.length > 0)
      ) {
    ...
    const deferredSymbolsTracker = new DeferredSymbolTracker(
      this.inputProgram.getTypeChecker(),
      (_a = this.options.onlyExplicitDeferDependencyImports) != null ? _a : false,
    );
    ...
    const handlers = [
      new ComponentDecoratorHandler(
        ...
        localCompilationExtraImportsTracker,
        jitDeclarationRegistry,
        (_e = this.options.i18nPreserveWhitespaceForLegacyExtraction) != null ? _e : true,
      ...
      new NgModuleDecoratorHandler(
        ...
        refEmitter,
        this.closureCompilerEnabled,
        (_f = this.options.onlyPublishPublicTypingsForNgModules) != null ? _f : false,
        injectableRegistry,
        this.delegatingPerfRecorder,
        supportTestBed,
        supportJitMode,
...
function isAngularCorePackage(program) {
  ...
  return r3Symbols.statements.some((stmt) => {
    ...
    if (
      modifiers === void 0 ||
      !modifiers.some((mod) => mod.kind === ts25.SyntaxKind.ExportKeyword)
    ) {
      return false;
    }
...
${allowedCategoryLabels.join('\n')}
  ...
  for (const [checkName, category] of Object.entries(
    (_c = (_b = options.extendedDiagnostics) == null ? void 0 : _b.checks) != null ? _c : {},
  )) {
...
var NotifyingProgramDriverWrapper = class {
  ...
  constructor(delegate, notifyNewProgram) {
    ...
    this.getSourceFileVersion =
      (_a = this.delegate.getSourceFileVersion) == null ? void 0 : _a.bind(this);
  }
  ...
  createCachedResolveModuleNamesFunction() {
    ...
    return (moduleNames, containingFile, reusedNames, redirectedReference, options) => {
      return moduleNames.map((moduleName) => {
        const module = ts26.resolveModuleName(
          moduleName,
          containingFile,
          options,
          this,
...
var NgtscProgram = class {
  ...
  constructor(rootNames, options, delegateHost, oldProgram) {
    ...
    this.host = NgCompilerHost.wrap(
      ...
      rootNames,
      options,
      reuseProgram != null ? reuseProgram : null,
    );
    if (reuseProgram !== void 0) {
      retagAllTsFiles(reuseProgram);
    }
    this.tsProgram = perfRecorder.inPhase(PerfPhase.TypeScriptProgramCreate, () =>
      ts27.createProgram(this.host.inputFiles, options, this.host, reuseProgram),
    );
  ...
  emitXi18n() {
    ...
    const ctx = new MessageBundle(
      ...
      [],
      {},
      (_a = this.options.i18nOutLocale) != null ? _a : null,
      this.options.i18nPreserveWhitespaceForLegacyExtraction,
    );
    this.compiler.xi18n(ctx);
    i18nExtract(
      (_b = this.options.i18nOutFormat) != null ? _b : null,
      (_c = this.options.i18nOutFile) != null ? _c : null,
      this.host,
      this.options,
      ctx,
      resolve,
  ...
  emit(opts) {
    ...
    const res = this.compiler.perfRecorder.inPhase(PerfPhase.TypeScriptEmit, () => {
      ...
      const emitCallback =
        (_a2 = opts == null ? void 0 : opts.emitCallback) != null ? _a2 : defaultEmitCallback;
...
function formatDiagnostics(diags, host = defaultFormatHost) {
  if (diags && diags.length) {
    return diags
      .map((diagnostic) =>
        replaceTsWithNgInErrors(ts29.formatDiagnosticsWithColorAndContext([diagnostic], host)),
      )
      .join('');
  } else {
    return '';
...
function readConfiguration(project, existingOptions, host = getFileSystem()) {
  ...
  try {
    ...
    const readConfigFile = (configFile) =>
      ts29.readConfigFile(configFile, (file) => host.readFile(host.resolve(file)));
    const readAngularCompilerOptions = (configFile, parentOptions = {}) => {
      ...
      const angularCompilerOptions =
        (_b = config2.angularCompilerOptions) != null
          ? _b
          : (_a2 = config2.bazelOptions) == null
            ? void 0
            : _a2.angularCompilerOptions;
      let existingNgCompilerOptions = {...angularCompilerOptions, ...parentOptions};
      if (!config2.extends) {
        return existingNgCompilerOptions;
      }
      const extendsPaths =
        typeof config2.extends === 'string' ? [config2.extends] : config2.extends;
    ...
    const {
      ...
      fileNames: rootNames,
      projectReferences,
    } = ts29.parseJsonConfigFileContent(
      config,
      parseConfigHost,
      basePath,
...
function getExtendedConfigPathWorker(configFile, extendsValue, host, fs) {
  if (extendsValue.startsWith('.') || fs.isRooted(extendsValue)) {
    ...
    const parseConfigHost = createParseConfigHost(host, fs);
    const {resolvedModule} = ts29.nodeModuleNameResolver(
      extendsValue,
      configFile,
      {moduleResolution: ts29.ModuleResolutionKind.Node10, resolveJsonModule: true},
      parseConfigHost,
    );
    if (resolvedModule) {
