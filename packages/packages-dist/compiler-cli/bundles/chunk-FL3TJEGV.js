...
function getDownlevelDecoratorsTransform(
      ...
      entries.push(
        ts.factory.createPropertyAssignment(
          ...
          ts.factory.createArrayLiteralExpression(
            decorators.map((deco) => extractMetadataFromSingleDecorator(deco, diagnostics2)),
          ),
        ),
      );
    }
  ...
  return (context) => {
    ...
    function transformClassElement(element) {
      ...
      let modifiers;
      if (
        decoratorsToKeep.length ||
        (elementModifiers == null ? void 0 : elementModifiers.length)
      ) {
    ...
    function decoratorDownlevelVisitor(node) {
      if (
        ...
        (shouldTransformClass === void 0 || shouldTransformClass(node))
      ) {
...
function createSyntheticAngularCoreDecoratorAccess(
  ...
  const classDecoratorIdentifier = ts2.isIdentifier(ngClassDecorator.identifier)
...
var signalInputsTransform = (
  ...
  var _a, _b;
  if (
    (_a = host.getDecoratorsOfDeclaration(member.node)) == null
      ? void 0
      : _a.some((d) => isAngularDecorator(d, 'Input', isCore))
  ) {
    return member.node;
  }
  ...
  return factory.updatePropertyDeclaration(
    member.node,
    [newDecorator, ...((_b = member.node.modifiers) != null ? _b : [])],
    member.name,
...
var signalModelTransform = (
  ...
  return factory.updatePropertyDeclaration(
    member.node,
    [inputDecorator, outputDecorator, ...((_b = member.node.modifiers) != null ? _b : [])],
    member.node.name,
    member.node.questionToken,
    member.node.type,
    member.node.initializer,
...
var initializerApiOutputTransform = (
  member,
  sourceFile,
  host,
  factory,
  ...
  if (
    (_a = host.getDecoratorsOfDeclaration(member.node)) == null
      ? void 0
      : _a.some((d) => isAngularDecorator(d, 'Output', isCore))
  ) {
    return member.node;
  }
...
var queryFunctionsTransforms = (
  ...
  return factory.updatePropertyDeclaration(
    member.node,
    [newDecorator, ...((_a = member.node.modifiers) != null ? _a : [])],
    member.node.name,
    member.node.questionToken,
    member.node.type,
    member.node.initializer,
  ...
  const visitor = (node) => {
    ...
    if (ts4.isClassDeclaration(node) && node.name !== void 0) {
      ...
      const angularDecorator =
        (_a = host.getDecoratorsOfDeclaration(originalNode)) == null
          ? void 0
          : _a.find((d) =>
              decoratorsWithInputs.some((name) => isAngularDecorator(d, name, isCore)),
            );
      if (
        angularDecorator !== void 0 &&
        (shouldTransformClass === void 0 || shouldTransformClass(node))
      ) {
