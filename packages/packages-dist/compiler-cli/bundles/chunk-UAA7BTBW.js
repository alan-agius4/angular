...
import {SourceFileLoader} from './chunk-77D5CI2U.js';
import {Context, ExpressionTranslatorVisitor} from './chunk-QYORKQDN.js';
...
var PLACEHOLDER_VERSION = '20.0.0-next.1+sha-639c717';
...
${errors}`,
  ...
  tryExternalTemplate(range) {
    ...
    const templateContents = sourceFile.sources.find(
      (src) => (src == null ? void 0 : src.sourcePath) === pos.file,
    ).contents;
    return {
  ...
  templateFromPartialCode(templateNode, {startPos, endPos, startLine, startCol}) {
    if (!/["'`]/.test(this.code[startPos]) || this.code[startPos] !== this.code[endPos - 1]) {
      throw new FatalLinkerError(
        ...
        `Expected the template string to be wrapped in quotes but got: ${this.code.substring(startPos, endPos)}`,
      );
    }
    return {
      code: this.code,
...
function toR3InjectableMeta(metaObj) {
  ...
  const meta = {
    ...
    providedIn: metaObj.has('providedIn')
      ...
      : createMayBeForwardRefExpression2(o3.literal(null), 0),
  };
...
function toR3NgModuleMeta(metaObj, supportJit) {
  ...
  if (metaObj.has('bootstrap')) {
    ...
    } else meta.bootstrap = wrapReferences(bootstrap);
  }
  if (metaObj.has('declarations')) {
    ...
    } else meta.declarations = wrapReferences(declarations);
  }
  if (metaObj.has('imports')) {
    ...
    } else meta.imports = wrapReferences(imports);
  }
  if (metaObj.has('exports')) {
    ...
    } else meta.exports = wrapReferences(exports);
  }
...
var Translator = class {
  ...
  translateExpression(expression, imports, options = {}) {
    return expression.visitExpression(
      new ExpressionTranslatorVisitor(this.factory, imports, null, options),
      new Context(false),
    );
  }
...
var LinkerEnvironment = class {
  ...
  static create(fileSystem, logger, host, factory, options) {
    ...
    return new LinkerEnvironment(fileSystem, logger, host, factory, {
      sourceMapping:
        (_a = options.sourceMapping) != null ? _a : DEFAULT_LINKER_OPTIONS.sourceMapping,
      linkerJitMode:
        (_b = options.linkerJitMode) != null ? _b : DEFAULT_LINKER_OPTIONS.linkerJitMode,
      unknownDeclarationVersionHandling:
        (_c = options.unknownDeclarationVersionHandling) != null
          ? _c
