...
import {Diagnostics} from '../../chunk-P63CR46L.js';
    ...
    if (!sourceCode.includes('$localize')) {
      ...
      if (sourceLocale2 !== void 0) {
        this.translateFile(
          diagnostics2,
          ast,
          {locale: sourceLocale2, translations: {}},
          sourceRoot,
          relativeFilePath,
          outputPathFn2,
          this.sourceLocaleOptions,
  ...
  loadBundle(filePath, providedLocale) {
    ...
    for (const translationParser of this.translationParsers) {
      ...
      if (diagnostics2.hasErrors) {
        throw new Error(
          diagnostics2.formatDiagnostics(`The translation file "${filePath}" could not be parsed.`),
        );
      }
...
function translateFiles({
  sourceRootPath: sourceRootPath2,
  sourceFilePaths: sourceFilePaths2,
  translationFilePaths: translationFilePaths2,
  translationFileLocales: translationFileLocales2,
  outputPathFn: outputPathFn2,
  diagnostics: diagnostics2,
  missingTranslation: missingTranslation2,
  duplicateTranslation: duplicateTranslation2,
  ...
  const translationFilePathsArrays = translationFilePaths2.map((filePaths) =>
    Array.isArray(filePaths) ? filePaths.map((p) => fs2.resolve(p)) : [fs2.resolve(filePaths)],
  );
  const translations = translationLoader.loadBundles(
    translationFilePathsArrays,
    translationFileLocales2,
  );
  sourceRootPath2 = fs2.resolve(sourceRootPath2);
  resourceProcessor.translateFiles(
    sourceFilePaths2.map(relativeFrom),
    fs2.resolve(sourceRootPath2),
    outputPathFn2,
    translations,
    sourceLocale2,
  );
