...
function makeEs2015TranslatePlugin(
  diagnostics,
  translations,
  {missingTranslation = 'error', localizeName = '$localize'} = {},
  fs = getFileSystem(),
) {
  return {
    visitor: {
...
import {
  Element,
  ParseError,
  ParseErrorLevel as ParseErrorLevel2,
  XmlParser,
...
function canParseXml(filePath, contents, rootNodeName, attributes) {
  ...
  if (
    ...
    xml.errors.some((error) => error.level === ParseErrorLevel2.ERROR)
  ) {
...
import {
  Element as Element3,
  ParseErrorLevel as ParseErrorLevel4,
  visitAll as visitAll3,
} from '@angular/compiler';
